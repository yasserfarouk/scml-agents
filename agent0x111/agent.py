...
class ElaboratedMeanERPStrategy(ExecutionRatePredictionStrategy):
    ...
    @abstractmethod
    def check_breaches_strategy(self, breach_situation:bool):#update exe fraction if the rate of the breaching is big
...
class ElaboratedPredictionBasedTradingStrategy(FixedTradePredictionStrategy, ElaboratedMeanERPStrategy, TradingStrategy):
...
class ElaboratedStepNegotiationManager(ElaboratedMeanERPStrategy, NegotiationManager):
    ...
    @abstractmethod
    def get_reliable_partners(self,partners: List[str],threshold_of_suspects=0.8,ratio_between_partners_and_suspects=0.3):
...
class ASMASH(
        ...
        if (len(self.Contracts_situation) >= len_of_seq):
            ...
            if (self.awi.current_step == last_elements[-1][0]):
                ...
                for i in range(0, len(last_elements) - 1):
                    if (((last_elements[i + 1][0] - last_elements[i][0]) > 1) or (
                            "breach" not in last_elements[i][1]) or ("breach" not in last_elements[i + 1][1])):
                        temp = False
                        break
    ...
    def check_conseq_of_execution(self):
        ...
        if (len(self.Contracts_situation) >= size_of_conseq):
            ...
            if (self.awi.current_step == last_elements[-1][0]):
                ...
                for i in range(0, len(last_elements) - 1):
                    if (((last_elements[i + 1][0] - last_elements[i][0]) > 1) or (
                            "executed" not in last_elements[i][1]) or ("executed" not in last_elements[i + 1][1])):
                        temp = False
                        break
    ...
    def good_situation_partners(self,partners: List[str],):
        ...
        if (Breaches_strategy_partners != None):
            ...
            partners_after_filtering = list(set(partners_after_filtering) - set(Breaches_strategy_partners))

        #print(str("Good partners: ")+str(partners_after_filtering))
        return partners_after_filtering

    def get_reliable_partners(self,partners: List[str],threshold_of_suspects=0.85,ratio_between_partners_and_suspects=0.3):
        ...
        suspected_partners = self.get_suspected_partners(paramater=threshold_of_suspects)
        #print("Suspected patners: "+str(suspected_partners))
        partners_after_filtering = list(set(partners) - set(suspected_partners))
        ratio_of_partners=float(len(partners_after_filtering)/len(partners))#the ratio between the
    ...
    def get_ratio_of_suspects(self,partners: List[str]):
        ...
        suspected_partners = self.get_suspected_partners(paramater=threshold_of_suspects)
        partners_after_filtering = list(set(partners) - set(suspected_partners))
        ratio_of_partners = float(len(partners_after_filtering) / len(partners))  # the ratio between the suspects and all partners
    ...
    def recognize_breaches_strategy(self):
        ...
        self.my_breaches = [(x, y) for x, y in self.my_breaches if x > self.awi.current_step - 5 ]
        breachs=[]
        #i=0
    ...
    def get_bad_situation_partners(self):
        breach_dic = self.get_breach_dic()
        if breach_dic != -1:
            bad_situation_partners = [id for id, val in breach_dic.items() if (val > 0.75)]

            partners= set(bad_situation_partners).union(set(self.suspects_to_bankrupt))
            partners=partners-set(self.agent_bankrupt)# I dont want to negotiate with bankrupt agents
...
def run(competition='std',reveal_names=True,n_steps=15,n_configs=1,max_n_worlds_per_config=None,n_runs_per_world=2):
    """
    **Not needed for submission.** You can use this function to test your agent.
    ...
    competitors = [ASMASH, IndDecentralizingAgent,DecentralizingAgent,MovingRangeAgent]
    start = time.perf_counter()
    if competition == 'std':
        results = anac2020_std(competitors=competitors, verbose=True, n_steps=n_steps,n_configs=n_configs, n_runs_per_world=n_runs_per_world)
    elif competition == 'collusion':
        results = anac2020_collusion(competitors=competitors, verbose=True, n_steps=n_steps,n_configs=n_configs, n_runs_per_world=n_runs_per_world)
    else:
