...
from scml.scml2020.agents import DecentralizingAgent, BuyCheapSellExpensiveAgent, \
    IndDecentralizingAgent, RandomAgent
...
class OurNegotiationsManager(IndependentNegotiationsManager):
    def acceptable_unit_price(self, step: int, sell: bool) -> int:
        ...
        #ours
        production_cost = np.max(self.awi.profile.costs[:, self.awi.my_input_product])
        return self.awi.catalog_prices[self.awi.my_output_product] * 10 \
            if sell else production_cost
...
class BeerAgent(OurNegotiationsManager, DemandDrivenProductionStrategy, DecentralizingAgent):
    pass
