from negmas import Issue, AgentMechanismInterface, Negotiator, MechanismState, Contract, Breach, SAOController, \
...
class PrintingAgent(SCML2020Agent):
    ...
    def request_negotiations(
        ...
        self.print("<<<Outgoing {} Negotiation from {}: issue[quantity:{} unit_price:{}, time:{}]".format("Buy" if is_buy else "sell", self.id, quantity, unit_price, time))
        awi: AWI = self.awi
        result: bool = awi.request_negotiations(is_buy=is_buy, product=product, quantity=quantity,
                                                unit_price=unit_price, time=time, controller=controller,
                                                negotiators=negotiators, partners=partners, extra=extra)
        return result
    ...
    def on_neg_request_accepted(self, req_id: str, mechanism: AgentMechanismInterface):
        ...
        self.print(
            "~>>Outgoing Negotiation Accepted by {}: ID({}) issue{}".format(
                mechanism.annotation["seller"] if mechanism.annotation["buyer"] == self.id else mechanism.annotation["buyer"],
                mechanism["id"][:4], str(mechanism["issues"])))

    def on_neg_request_rejected(self, req_id: str, by: Optional[List[str]]):
        super().on_neg_request_rejected(req_id, by)
        for rejecter in by:
            self.print("~>|Outgoing Negotiation Rejected by {}: ID({})".format(rejecter, req_id[:4]))
        ...
        if self.awi.current_step >= 1:
            ...
            if fr_list is not None:
                for key, x in fr_list.items():
                    self.print("Agent: {},\tcash: {:>7},\tassets: {:>5},\tbreach prob: {:.5f},\tbreach level:{:.5f},"
                               "\tbankrupt:{},".format(x.agent_id, x.cash, x.assets, x.breach_prob, x.breach_level,
                                                       x.is_bankrupt))
    ...
    def on_accepting_negotiation_request(self,
        ...
        if self.awi.current_step == self.i_step_step_base:
            self.print("<<~Accept Incoming Negotiation from {}: ID({}) issue{}".format(initiator, mechanism["id"][:4], str(issues)))
        else:
            self.print("<<<<<~Accept Incoming Negotiation for next from {}: ID({}) issue{}".format(initiator, mechanism["id"][:4],str(issues)))

    def on_rejecting_negotiation_request(self,
                                         initiator: str,
                                         issues: List[Issue],
                                         annotation: Dict[str, Any],
                                         mechanism: AgentMechanismInterface
        ...
        if self.awi.current_step == self.i_step_step_base:
            self.print("|<~Reject Incoming Negotiation from {}: ID({}) issue{}".format(initiator, mechanism["id"][:4], str(issues)))
        else:
            self.print("|<<<<~Reject Incoming Negotiation for next from {}: ID({}) issue{}".format(initiator, mechanism["id"][:4],str(issues)))
    ...
    def respond_to_negotiation_request(self,
        ...
        if self.awi.current_step == self.i_step_step_base:
            self.print(">>>Incoming Negotiation from {}: ID({}) issue{}".format(initiator, mechanism["id"][:4], str(issues)))
        else:
            self.print(
                ">>>>>>Incoming Negotiation for next from {}: ID({}) issue{}".format(initiator, mechanism["id"][:4], str(issues)))
    ...
    def on_negotiation_failure(self,
        ...
        agreement"""
        self.print("<|>{} Negotiation failure w/ {}: ID({}) issue{}".format(
            "Sell" if annotation["seller"] == self.id else "Buy",
            list(set(partners)-set([self.id]))[0], mechanism["id"][:4], str(mechanism["issues"])))
    ...
    def on_negotiation_success(self,
        ...
        agreement"""
        self.print("|||{} Negotiation success w/ {}: ID({}) Agreement{} issue{}".format(
            "Sell" if contract.annotation["seller"] == self.id else "Buy",
            list(set(contract["partners"]) - set([self.id]))[0], mechanism["id"][:4],
            contract["agreement"], str(mechanism["issues"])))
...
class PrintingSAOController(SAOController):
    ...
    def before_join(self, negotiator_id: str, ami: AgentMechanismInterface, state: MechanismState, *,
                    ufun: Optional["UtilityFunction"] = None, role: str = "agent", **kwargs) -> bool:
        """
        ...
        """
        return super().before_join(negotiator_id=negotiator_id, ami=ami, state=state, ufun=ufun, role=role)

    def after_join(self, negotiator_id: str, ami: AgentMechanismInterface, state: MechanismState, *,
            ufun: Optional["UtilityFunction"] = None, role: str = "agent") -> None:
        """
        ...
        Args:
            ...
            role (str): role of the agent.
        """
        super().after_join(negotiator_id=negotiator_id, ami=ami, state=state, ufun=ufun, role=role)
