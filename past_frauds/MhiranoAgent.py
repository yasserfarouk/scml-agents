...
from negmas import (AgentMechanismInterface, Breach, Contract, Issue,
                    MechanismState, Negotiator, AgentWorldInterface, SAOController, Controller, SAONegotiator,
                    AspirationNegotiator, UtilityFunction, LinearUtilityFunction, PassThroughNegotiator,
                    PassThroughSAONegotiator, ResponseType)
from negmas.helpers import humanize_time, get_class
from scml.scml2020 import SCML2020Agent, SCML2020World, FactoryState, AWI, FinancialReport, TIME, TradingStrategy, \
    DemandDrivenProductionStrategy, PredictionBasedTradingStrategy, ReactiveTradingStrategy
from scml.scml2020.agents import (BuyCheapSellExpensiveAgent,
                                  DecentralizingAgent, DoNothingAgent, IndDecentralizingAgent, MovingRangeAgent)
...
class MhiranoAgent(DemandDrivenProductionStrategy, PrintingAgent):
    def __init__(
        ...
        self.controller: MhiranoController = MhiranoController(parent=self, default_negotiator_type=MhiranoNegotiator)
    ...
    def step(self):
        ...
        #####
        more_or_less: int = sum(self.outputs_signed[awi.current_step + 1: awi.current_step+10]) \
        ...
        if more_or_less > 0:
            ...
            self.request_negotiations(is_buy=True, product=awi.my_input_product, quantity=(1, min(more_or_less, 15)),
                                      unit_price=(0, awi.catalog_prices[awi.my_output_product]),
                                      time=(awi.current_step + 1, min(awi.current_step + 15, awi.n_steps -1)),
                                      controller=None,
        ...
        if more_or_less < 0:
            ...
            self.request_negotiations(is_buy=False, product=awi.my_output_product, quantity=(1, min(-more_or_less, 10)),
                                      unit_price=(awi.catalog_prices[awi.my_input_product], awi.catalog_prices[awi.my_output_product] * 2),
                                      time=(awi.current_step + 1, min(awi.current_step + 15, awi.n_steps -1)),
                                      controller=None,
                                      negotiators=[negotiator for _ in partners],
                                      partners=partners)
    ...
    def on_accepting_negotiation_request(self,
                                         initiator: str,
                                         issues: List[Issue],
                                         annotation: Dict[str, Any],
                                         mechanism: AgentMechanismInterface
                                         ) -> None:
        super().on_accepting_negotiation_request(initiator, issues, annotation, mechanism)
    ...
    def create_negotiator(self, is_seller:bool) -> Negotiator:
        ...
        negotiator: Negotiator = self.controller.create_negotiator(negotiator_type=MhiranoNegotiator, ufun=ufun)# MhiranoNegotiator(parent=self, ufun=ufun)
        return negotiator

    def processing_negotiation_request(self,
                                       initiator: str,
                                       issues: List[Issue],
                                       annotation: Dict[str, Any],
                                       mechanism: AgentMechanismInterface
        ...
        self.request_negotiations(is_buy=is_buy, product=product, quantity=(1, q),
                                  unit_price=(up_min, up_max), time=(t_start, t_end),
                                  controller=None,
                                  negotiators=[negotiator for _ in partners],
                                  partners=partners)
    ...
    def respond_to_negotiation_request(self,
        ...
        negotiator: Negotiator = self.processing_negotiation_request(initiator, issues, annotation, mechanism)

        if negotiator is None:
            self.on_rejecting_negotiation_request(initiator, issues, annotation, mechanism)
        else:
            self.on_accepting_negotiation_request(initiator, issues, annotation, mechanism)
        return negotiator
...
class MhiranoController(PrintingSAOController):
    ...
    def _get_best_outcome(self, is_seller: bool, outcomes: List["Outcome"], eval_outcomes: List[float]=None)\
        ...
        if eval_outcomes is None:
            ...
            if sum(np.array(eval_outcomes) == np.array(eval_outcomes).max()) > 1:
                np_candidates: np.ndarray = np.array(outcomes)[np.array(eval_outcomes) == np.array(eval_outcomes).max()]
                if is_seller:
                    # 売り手の場合 納期が遅い => 量が少ない => 価格が高い
                    np_candidates = np_candidates[np_candidates[:, 1] == np_candidates[:, 1].max()]
                    np_candidates = np_candidates[np_candidates[:, 0] == np_candidates[:, 0].min()]
                    np_candidates = np_candidates[np_candidates[:, 2] == np_candidates[:, 2].max()]
                    if len(np_candidates) == 1:
                        return tuple(np_candidates[0]), np.array(eval_outcomes).max()
                    else:
                        return tuple(np.choose(1, choices=np_candidates)), np.array(eval_outcomes).max()
                else:
                    # 買いての場合 納期が早い => 価格が安い => 量が多い
                    np_candidates = np_candidates[np_candidates[:, 1] == np_candidates[:, 1].min()]
                    np_candidates = np_candidates[np_candidates[:, 2] == np_candidates[:, 2].min()]
                    np_candidates = np_candidates[np_candidates[:, 0] == np_candidates[:, 0].max()]
                    if len(np_candidates) == 1:
                        return tuple(np_candidates[0]), np.array(eval_outcomes).max()
                    else:
                        return tuple(np.choose(1, choices=np_candidates)), np.array(eval_outcomes).max()
            return outcomes[np.array(eval_outcomes).argmax()], np.array(eval_outcomes).max()

    def _get_worst_outcome_over_thr(self, is_seller: bool, outcomes: List["Outcome"], eval_outcomes: List[float]=None,
                                    thr: float=None) -> Tuple["Outcome", Union[float, None]]:
        # 正方的なoutcomes spaceを想定する
        # (quantity, time, unit_price)のTupleのListがくる
        # 係数
        if len(outcomes) == 0:
    ...
    def _get_current_thr(self, relative_time:float) -> float:
        thr: float = (1 - (relative_time ** self.power)) * (self.ufun_max - self.ufun_min) + self.ufun_min
        return thr

    def _cut_outcomes(self, negotiator_id: str, offer: "Outcome", is_seller: bool, method: str = "worse") -> None:
        ...
        if is_seller:
            # 売り手の場合
            if method == "better":
                new_outcomes: List["Outcome"] = [outcome for outcome in
                                                 self._negotiating[negotiator_id]["current_space"]
    ...
    def _get_current_eval(self, negotiator_id: str, is_seller: bool, offer: "Outcome") -> float:
        return self._get_current_eval_all(negotiator_id=negotiator_id, is_seller=is_seller, offers=[offer])[0]

    def _time_price_probs(self, is_sell: bool, t_up_list: List[Tuple[int, int]]) -> List[float]:
        # 時間とpriceの出現確率
        # priceに対して指数分布を仮定
        # カタログプライスはわざと逆にしている
    ...
    def _mk_evaluation_for_tup(self, q_max:int, is_seller:bool, t_up_list: List[Tuple[int, int]],
                               input_diff_series: List[int], line_usage_list:List[int])\
            -> Dict[Tuple[int, int], List[float]]:

        def _get_production_end_time_of_buy(_q_max:int, _start_time:int, _line_usage_list: List[int]) -> np.ndarray:
            # 最も早い生産時間を考える
        ...
        def _get_production_start_time_of_sell(_q_max:int, _end_time:int, _line_usage_list: List[int]) -> np.ndarray:
            # 最も遅い生産時間を考える
        ...
        t_max: int = min(self.parent.awi.current_step + 10, self.parent.awi.n_steps) # endは含まない
        #####
        ...
        if not is_seller:
            ...
            for data in self._negotiating.values():
                if data["is_seller"]:
                    ...
                    all_q += max_q
                    negotiating_opponents.append((max_q, list(set([(t, up) for (q, t, up) in data["current_space"]]))))
        else:
            ...
            if all_q == 0:
                ...
                if not is_seller:
                    ...
                    if up <= awi.catalog_prices[awi.my_input_product]:
                        agree_tmp: List[int] = [agreements[i] if t_relative <= i else 0.0 for i in range(t_max - t_min)]
                    else:
                        agree_tmp: List[int] = [0 if t_relative <= i else 0.0 for i in range(t_max - t_min)]
                    evaluation_dict[(t, up)] = [min(sum(agree_tmp), i) / all_q for i in range(1, q_max + 1)]
                else:
                    # 評価する注文が売りの場合(agreementは買い注文)にはには
                    # outputのカタログプライス以上でないと計算しない
                    start_time: List[int] = _get_production_start_time_of_sell(_q_max=q_max, _end_time=t, _line_usage_list=line_usage_list)
        ...
        def _get_profit_prob(_t: int, _up: int, _is_sell: bool, _outcome_space: List[Tuple[int, int]]) -> float:
            # t, upを与えたら，ターゲットとなる注文1つのoutcome spaceの何割をカバーできるかを計算
            # is_sell　は (_t, _up)が売り注文かどうか
            _prob_list: List[float] = self._time_price_probs(is_sell=_is_sell, t_up_list=_outcome_space)
            _results: float = 0.0
            ...
            if not is_seller:
                # ターゲットは売り注文
                _results: float = sum((np_outcome_space[:, 0] >= _t) * (np_outcome_space[:, 1] > _up) * np_prob)
            else:
                # ターゲットは買い注文
                _results: float = sum((np_outcome_space[:, 0] <= _t) * (np_outcome_space[:, 1] < _up) * np_prob)
            assert _results >= 0.0
        ...
        if not is_seller:
            ...
            for (t, up), ev_list in (evaluation_dict.items()):
                products_available_time_at_q: List[int] = \
                    _get_production_end_time_of_buy(_q_max=q_max, _start_time=t, _line_usage_list=line_usage_list)
                ...
                for (q_target, outcome_space) in (negotiating_opponents):
                    ...
                    for _t in possible_time:
                        prob = _get_profit_prob(_t=t, _up=up, _is_sell=is_seller, _outcome_space=outcome_space)
            ...
            # 売りの場合は，買いを評価すればいいので，売りofferを最遅の生産を考える
            for (t, up), ev_list in (evaluation_dict.items()):
                products_need_time_at_q: List[int] = \
                    _get_production_start_time_of_sell(_q_max=q_max, _end_time=t, _line_usage_list=line_usage_list)
    ...
    def _get_current_eval_all(self, negotiator_id: str, is_seller: bool, offers: List["Outcome"]) -> List[float]:
        # offerは(q, t, uo)
        # 在庫確認
        awi: AWI = self.parent.awi
        ...
        # {(time, price): [ev for q=1, q=2, ..., q=q_max]}
        evaluation_dict: Dict[Tuple[int, int], List[float]] =\
            self._mk_evaluation_for_tup(q_max=q_max, is_seller=is_seller, t_up_list=t_up_list,
                                        input_diff_series=input_diff_series, line_usage_list=line_usage_list)
    ...
    def propose_(self, negotiator_id: str, state: MechanismState) -> Optional["Outcome"]:
        ...
        eval_outcomes: List[float] = self._get_current_eval_all(negotiator_id=negotiator_id, is_seller=is_seller, offers=outcomes)
        best_outcome, utility = self._get_best_outcome(is_seller=is_seller, outcomes=outcomes, eval_outcomes=eval_outcomes)
        ...
        propose_outcome, propose_outcom_eval = \
            self._get_worst_outcome_over_thr(is_seller=is_seller, outcomes=outcomes,
                                             eval_outcomes=eval_outcomes, thr=thr_eval)
        return propose_outcome

    def record_offer_(self, negotiator_id: str, state: MechanismState, offer: "Outcome") -> None:
    ...
    def respond_(self, negotiator_id: str, state: MechanismState, offer: "Outcome") -> "ResponseType":
        ...
        if self._negotiating[negotiator_id]["last_offer"] is not None:
            ...
            self._cut_outcomes(negotiator_id=negotiator_id, offer=self._negotiating[negotiator_id]["last_offer"],
                               is_seller=is_seller, method="better")
        else:
            ...
            if self._negotiating[negotiator_id]["max_utility"] < utility:
                self._negotiating[negotiator_id]["max_utility"] = utility
        self._cut_outcomes(negotiator_id=negotiator_id, offer=offer,
                           is_seller=self._negotiating[negotiator_id]["is_seller"], method="worse")
        # spaceのカット終了
        offer_eval: float = self._get_current_eval(negotiator_id=negotiator_id, is_seller=is_seller, offer=offer)
        if offer_eval <= 1e-9:
    ...
    def join_(self, negotiator_id: str, ami: AgentMechanismInterface, state: MechanismState,
                              issues: List[Issue], outcomes: List["Outcome"]) -> None:
        ...
        self._negotiating[negotiator_id] = {"state": state, "ami": ami, "is_seller": is_seller, "issues": issues,
                                            "initial_space": outcomes, "current_space": outcomes, "last_offer": None,
                                            "last_opponent_offer": None, "max_utility": 0.0}

    def on_negotiation_start_(self, negotiator_id: str, ami: AgentMechanismInterface, state: MechanismState,
                              issues: List[Issue], outcomes: List["Outcome"]) -> None:
        if negotiator_id not in self._negotiating:
            is_seller = (ami.annotation["seller"] == self.parent.id)
            ######
            ...
            outcomes = self._slim_outcomes(outcomes=outcomes)
            self._negotiating[negotiator_id] = {"state": state, "ami": ami, "is_seller": is_seller, "issues": issues,
                                                "initial_space": outcomes, "current_space": outcomes,
                                                "last_offer": None, "last_opponent_offer": None,  "max_utility": 0.0}
            return None
        pass
    ...
    def _slim_outcomes(self, outcomes: List["Outcome"]):
        ...
        if (t_max - t_min) >= 10:
            if t_min < self.parent.awi.current_step + 10:
                _outcomes = [(q, t, up) for (q, t, up) in _outcomes if t < self.parent.awi.current_step + 10]
            else:
                interval: int = int(math.sqrt(t_max - t_min)) + 1
                _outcomes = [(q, t, up) for (q, t, up) in _outcomes if (t - t_min) % interval == 0]
        if q_max >= 10:
        ...
        if (up_max - up_min) >= 10:
            ...
            _outcomes = [(q, t, up) for (q, t, up) in _outcomes if (up - up_min) % interval == 0]
...
class MhiranoNegotiator(AspirationNegotiator):
    ...
    def join(
        ...
        *,
        ...
        if outcomes is None:
            return None
        self.__parent.join_(negotiator_id=self.name, ami=ami, state=state, issues=issues,
                                            outcomes=outcomes)
        return result
    ...
    def on_negotiation_start(self, state: MechanismState) -> None:
        ...
        issues = ami.issues
        outcomes = ami.outcomes
        self.__parent.on_negotiation_start_(negotiator_id=self.name, ami=ami, state=state, issues=issues, outcomes=outcomes)
    ...
    def propose(self, state: MechanismState) -> Optional["Outcome"]:
        ...
        if offer is not None:
            self.__parent.record_offer_(negotiator_id=self.name, state=state, offer=offer)
        return offer
...
def run(competition='std',
    ...
    **Not needed for submission.** You can use this function to test your agent.
    ...
    result_dict = {}
    for (name, score) in zip(results.total_scores["agent_type"].apply(lambda x: x.split(".")[-1]),results.total_scores["score"]):
        result_dict[name] = score
    json.dump(result_dict, open("_output.json", "w"))


def debug():
    agent_types = [MhiranoAgent, DecentralizingAgent, BuyCheapSellExpensiveAgent] \
