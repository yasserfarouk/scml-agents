import numpy as np
from scml.scml2020 import TIME, QUANTITY, UNIT_PRICE, TradingStrategy, PredictionBasedTradingStrategy, \
    ReactiveTradingStrategy, DemandDrivenProductionStrategy, AWI, FactoryState
from negmas import ResponseType, outcome_is_valid, Outcome, MechanismState, UtilityFunction, Negotiator, SAOController
...
class IntegratedNegotiationManager(ReactiveTradingStrategy):
    ...
    def request_negotiations(
        ...
        return None
        return self.awi.request_negotiations(is_buy, product, quantity, unit_price, time, controller, negotiators, partners, extra)
    ...
    def step(self):
        ...
        current_marginal_material_inventory: int = awi.state.inventory[awi.my_input_product]
        current_marginal_product_inventory: int = awi.state.inventory[awi.my_output_product]
        for i_step, demand_quantity in enumerate(input_demand):
            ...
            if i_step == awi.current_step:
                ...
                marginal_product: int = input_demand[awi.current_step] + current_marginal_product_inventory
                if marginal_product > 0:
                    self.request_negotiations(is_buy=False, product=awi.my_input_product, quantity=(1, marginal_product),
                                              unit_price=(0, awi.catalog_prices[awi.my_output_product]*10), time=i_step,
                                              controller=self.controllers[True], partners=awi.my_consumers)
                continue
            if i_step == awi.current_step + 1:
                # 次のターンに出したい => lineが空いているか確認してから発注
                current_blank_line: int = min(sum(factory_state.commands[awi.current_step] == -1),
                                              current_marginal_material_inventory)
                assert current_blank_line >= 0
                self.request_negotiations(is_buy=False, product=awi.my_input_product, quantity=(1, current_blank_line),
                                          unit_price=(awi.catalog_prices[awi.my_input_product]+production_cost,
                                                      awi.catalog_prices[awi.my_output_product] * 10), time=i_step,
                                          controller=self.controllers[True], partners=awi.my_consumers)
            ...
            if demand_quantity < 0:
                ...
                current_blank_line: int = min(sum(factory_state.commands[i_step - 1] == -1),
                                              current_marginal_material_inventory)
                assert current_blank_line >= 0
                self.request_negotiations(is_buy=False, product=awi.my_output_product, quantity=(1, selling_quantity + current_blank_line),
                                          unit_price=(awi.catalog_prices[awi.my_input_product]+production_cost,
                                                      awi.catalog_prices[awi.my_output_product]*4), time=i_step,
                                          controller=self.controllers[True], partners=awi.my_consumers)
                ...
                continue
            if demand_quantity > 0:
                self.request_negotiations(is_buy=True, product=awi.my_input_product, quantity=(1, demand_quantity),
                                          unit_price=(0, awi.catalog_prices[awi.my_input_product]*2),
                                          time=i_step, controller=self.controllers[False], partners=awi.my_suppliers)
                ...
                #                           time=i_step, controller=self.controllers[False], partners=awi.my_suppliers)
                continue
            self.request_negotiations(is_buy=True, product=awi.my_input_product, quantity=(1, 10),
                                      unit_price=(0, awi.catalog_prices[awi.my_input_product] * 2),
                                      time=i_step, controller=self.controllers[False], partners=awi.my_suppliers)
            self.request_negotiations(is_buy=False, product=awi.my_output_product, quantity=(1, 10),
                                      unit_price=(awi.catalog_prices[awi.my_input_product]+production_cost,
