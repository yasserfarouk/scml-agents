from scml.scml2020 import SCML2020Agent, SCML2020World, RandomAgent, DecentralizingAgent, IndDecentralizingAgent
import matplotlib.pyplot as plt
from scml.scml2020.components.production import DemandDrivenProductionStrategy, ProductionStrategy, \
    SupplyDrivenProductionStrategy, TradeDrivenProductionStrategy
from scml.scml2020.components.trading import PredictionBasedTradingStrategy
from scml.scml2020.components.negotiation import IndependentNegotiationsManager, MovingRangeNegotiationManager, StepNegotiationManager
...
class SyncController(SAOSyncController):
    ...
    def utility(self, offer) -> float:
        ...
        if self._is_seller:
            return price * q / (abs(q - self.__parent.outputs_needed[self.__parent.awi.current_step]) + 0.1)
...
class MyDrorAgent(MyNegotiationManager, PredictionBasedTradingStrategy, SupplyDrivenProductionStrategy, SCML2020Agent):
    ...
    def acceptable_unit_price(self, step: int, sell: bool) -> int:
        ...
        return 1.2 * self.awi.catalog_prices[self.awi.my_output_product] if sell else 0.8 * self.awi.catalog_prices[self.awi.my_input_product]
...
class DrorStepAgent(MovingRangeNegotiationManager,
    ...
    def __init__(
        ...
        super().__init__(*args, **kwargs)
        MovingRangeNegotiationManager.__init__(self, *args,
                                               price_weight=0.7,
                                               utility_threshold=0.9,
                                               time_threshold=0.9,
                                               time_horizon=0.1,
...
for output_step in range(0,2):
    for n_simulations in range(10):
        ...
        if scores['DecentralizingAgent'].get(output_step):
            scores['DecentralizingAgent'][output_step] += returned_scores['DecentralizingAgent']
        else:
            scores['DecentralizingAgent'][output_step] = returned_scores['DecentralizingAgent']
