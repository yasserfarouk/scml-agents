...
from abc import ABC
import itertools
from scml.scml2020 import SCML2020Agent, SCML2020World, RandomAgent, DecentralizingAgent, DoNothingAgent, \
    NegotiationManager, Simulation
...
from negmas import (Contract, Issue, AgentMechanismInterface, Negotiator, SAONegotiator,
                    AspirationNegotiator, ToughNegotiator)
import numpy as np
...
class ToughAspirationNegotiator(AspirationNegotiator):
    ...
    def __init__(self, brother=False, threshold=0.8, behave="natural", all_money=10000, exec_time=1, *args, **kwargs):
...
class GryffindorIndependentNegotiationsManager(IndependentNegotiationsManager):
    ...
    def step(self):
        ...
        price_range = (int(self.awi.catalog_prices[self.awi.my_output_product] * 0),
        ...
        # (my_brother_suppliers)
        if len(my_brother_suppliers) > 0 and same_level_brother() and not check_profit() and self.awi.current_step > 3:
            if self.awi.state.inventory[self.awi.my_input_product] + self.awi.state.inventory[
                self.awi.my_output_product] < 10:
                self.neg_extras = {'threshold': 0.8, 'behave': 'help_me', 'exec_time': self.awi.current_step + 1,
                                   'brother': True,
                                   'all_money': int(self.awi.state.balance * 0.8) // (len(my_brother_suppliers))}
                self._start_negotiations(self.awi.my_input_product, False, self.awi.current_step + 1, (2, 2),
                                         (1, 1),
                                         (self.awi.current_step + 1, self.awi.current_step + 2),
                                         my_brother_suppliers)
            else:
                self._start_negotiations(self.awi.my_output_product, True, self.awi.current_step + 1, (1, 2),
        ...
        if len(self.brothers) < 2 or test_proximity():
            ...
            self._start_negotiations(self.awi.my_input_product, False, self.awi.current_step + 1, (1, 100),
                                     (1, 2),
                                     (self.awi.current_step + 1, self.awi.n_steps),
                                     self.awi.my_suppliers)

            self._start_negotiations(self.awi.my_output_product, True, self.awi.current_step + 1, (1, 2),
                                     (500, 1000), (self.awi.current_step, self.awi.n_steps),
                                     self.awi.my_consumers)

            for _ in range(4):
                if self.did_buy == False:
                    self._start_negotiations(self.awi.my_input_product, False, self.awi.current_step + 1, (1, 10),
                                             (int(self.awi.catalog_prices[self.awi.my_output_product] * 0),
                                              int(self.awi.catalog_prices[self.awi.my_output_product] * 2)),
                                             (self.awi.current_step + 1, self.awi.n_steps),
                                             self.awi.my_suppliers)
                if self.did_buy == False:
                    if self.awi.current_step + 2 < self.awi.n_steps - 2:
                        self._start_negotiations(self.awi.my_output_product, True, self.awi.current_step + 1, (1, 10),
                                                 (int(self.awi.catalog_prices[self.awi.my_output_product] * 0.5),
                                                  int(self.awi.catalog_prices[self.awi.my_output_product] * 2.5)),
                                                 (self.awi.current_step + 2,
                                                  self.awi.n_steps - 2),
                                                 self.awi.my_consumers)

            for _ in range(7):
                self._start_negotiations(self.awi.my_input_product, False, self.awi.current_step + 1, (1, 5),
                                         (int(self.awi.catalog_prices[self.awi.my_output_product] * 0),
                ...
                if min(self.awi.current_step + 3 + _, self.awi.n_steps - 2) > self.awi.current_step + 1:
                    self._start_negotiations(self.awi.my_output_product, True, self.awi.current_step + 1, (1, 5),
                                             (int(self.awi.catalog_prices[self.awi.my_output_product] * 0.8),
                                              int(self.awi.catalog_prices[self.awi.my_output_product] * 2.5)),
                                             (self.awi.current_step + 1,
        ...
        #collusion
            ...
            if self.awi.state.balance > 100 and len(my_brother_suppliers) > 0:
                ...
                self.neg_extras = {'threshold': 0.8, 'behave': 'suicidal', 'exec_time': 1, 'brother': True,
                                   'all_money': int(self.awi.state.balance * 0.8) // (len(my_brother_suppliers))}
                self._start_negotiations(self.awi.my_input_product, False, self.awi.current_step, (1, 1),
                                         (int(self.awi.state.balance * 0.8) // (len(my_brother_suppliers))- 1, int(self.awi.state.balance * 0.8) // (len(my_brother_suppliers))),
                                         (1, 1),
                                         my_brother_suppliers)
        ...
        commands[inputs:] = -1
        self.awi.set_commands(self.awi.my_input_product * np.ones(self.awi.n_lines, dtype=int))
        ...
        for contract in signed:
            ...
            if contract.agreement['time'] >= self.awi.current_step:
                if contract.annotation['buyer'] == self.id:
                    self.step_wise_inventory[contract.agreement['time']] += contract.agreement['quantity']
                    self.step_wise_buy_inventory[contract.agreement['time']] += contract.agreement['quantity']
                    self.step_wise_balance[contract.agreement['time']] -= contract.agreement['quantity'] * \
                ...
                if contract.annotation['seller'] == self.id:
                    self.step_wise_sell_inventory[contract.agreement['time']] += contract.agreement['quantity']
                    self.step_wise_inventory[contract.agreement['time']] -= contract.agreement['quantity']
                    self.step_wise_balance[contract.agreement['time']] += contract.agreement['quantity'] * \
                                                                          contract.agreement['unit_price']
                    self.did_sell = True
    ...
    def _start_negotiations(self, product, sell, step, qvalues, uvalues, tvalues, partners):
        ...
        for partner in partners:
            ...
            if sell:
                if self.did_sell:
                    self.neg_extras['threshold'] = min(0.9, self.neg_extras['threshold'] + 0.05 * 2)
                else:
                    self.neg_extras['threshold'] = max(0.4, self.neg_extras['threshold'] - 0.05 * 4)
            else:
                if self.did_buy:
                    self.neg_extras['threshold'] = min(0.9, self.neg_extras['threshold'] + 0.05 * 2)
                else:
...
class GryffindorTradingStrategy(TradingStrategy, SignAll):
    ...
    def init(self):
        ...
        self.brothers_nt += [Brother(id=self.id, in_product=self.awi.my_input_product, balance=self.awi.state.balance)]
        self.neg_extras = {'threshold': 0.8}

    def _greedy(self, contract_buy, contract_sell, production_cost, target):
        if self.awi.current_step < contract_buy['time'] < self.awi.n_steps and self.awi.current_step < contract_sell[
            'time'] < self.awi.n_steps:
            if contract_sell['time'] > contract_buy['time'] and contract_sell['unit_price'] > contract_buy[
                'unit_price'] + production_cost and target[contract_buy['time']] >= contract_buy['quantity'] and \
                    contract_buy['quantity'] > contract_sell['quantity']:
                return True
    ...
    def sign_all_contracts(self, contracts):
        ...
        buy_from_brother = [_ for _ in contracts if
                            _.annotation['buyer'] == self.id and _.annotation["seller"] in self.brothers]
        sell = [_ for _ in contracts if _.annotation['seller'] == self.id]
        sell_to_brother = [_ for _ in contracts if
                           _.annotation['seller'] == self.id and _.annotation["buyer"] in self.brothers]
        temp_b, temp_s = [], []
        buy.sort(key=lambda x: x.agreement['unit_price'])
        sell.sort(key=lambda x: x.agreement['unit_price'], reverse=True)
        max_sell_possible = (self.awi.n_steps - self.awi.current_step - 2) * 10 + self.awi.state.inventory[
            self.awi.my_output_product] - sum(self.step_wise_sell_inventory[self.awi.current_step:])


        def profit_estimate():
            basic_future_balance_change_estimate = sum(self.step_wise_balance[self.awi.current_step:])
            total_production_cost = sum(self.step_wise_buy_inventory[self.awi.current_step:]) * production_cost
            ...
            if over_sold[-1] > 0:
                spot_market_loss = over_sold[-1] * self.awi.catalog_prices[self.awi.my_input_product] * 1.2
            else:
                spot_market_loss = 0

            estimated_profit = (self.awi.state.balance + basic_future_balance_change_estimate - self.init_balance -
                                total_production_cost - spot_market_loss) / self.init_balance
        ...
        if len(self.brothers) < 2 or test_proximity() and profit_estimate() <1:
            ...
            for b in buy:
                for s in sell:
                    if self._greedy(b.agreement, s.agreement, production_cost, self.target) is True:
            ...
            for b in buy:
                if b.agreement['unit_price'] + production_cost < self.awi.catalog_prices[
                    self.awi.my_output_product] // 2:
                    sign[contracts.index(b)] = self.id

            _ = self.awi.state.inventory[self.awi.my_output_product] + self.awi.state.inventory[
                self.awi.my_input_product]

            # sell extra bought products
            if _ > sum(self.step_wise_sell_inventory[self.awi.current_step:]) and sum(
                    self.step_wise_sell_inventory[self.awi.current_step:]) < max_sell_possible:
            ...
            if sum(self.step_wise_sell_inventory[self.awi.current_step:]) == 0 and sum(
                    self.step_wise_buy_inventory[self.awi.current_step:]) == 0:
            ...
            if sum(self.step_wise_sell_inventory[self.awi.current_step + 1:]) > sum(
                    self.step_wise_buy_inventory[self.awi.current_step:] + self.awi.state.inventory[
                ...
                for specs in buy_specs:
                    for b in buy:
                        if self._over_sold_late_buy(b.agreement, specs) is True and specs[1] > 0:
