...
import numpy
from negmas import SAONegotiator, AgentWorldInterface, NonlinearHyperRectangleUtilityFunction
...
class Controller(StepController):
    def __init__(self,
        ...
        super().__init__(*args, is_seller=is_seller, negotiator_type=negotiator_type, negotiator_params=negotiator_params, **kwargs)
        if is_seller:
            self.ufun = NonlinearHyperRectangleUtilityFunction(hypervolumes=[
                {0: (1.0, 2.0), 1: (1.0, 2.0)},
                {0: (1.4, 2.0), 2: (2.0, 3.0)}],
