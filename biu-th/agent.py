...
class THBiuAgent(NegotiationManager, PredictionBasedTradingStrategy, SupplyDrivenProductionStrategy, SCML2020Agent):
    ...
    def respond_to_negotiation_request(
        ...
        if self.is_breach_in_last_steps(breached_agent_name, min(int(self.awi.n_steps*0.6), 20)):
            return None

        if self.is_there_agent_with_the_same_type(annotation["is_buy"]) and self.get_agent_type(
