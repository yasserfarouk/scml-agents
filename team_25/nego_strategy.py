...
class SyncController(SAOSyncController):
    ...
    def myUtil(self, offer):
        ...
        if (offer[TIME] < parent.awi.current_step or offer[TIME] > parent.awi.n_steps - 1):
            return -1000.0
        ...
        if (self._is_seller and offer[TIME] == parent.awi.current_step and
        offer[QUANTITY] > output_inventory):
            return -1000.0
        ...
        if offer[QUANTITY] > (needed[offer[TIME]] - secured[offer[TIME]]):
            quantity_util = (needed[offer[TIME]]-secured[offer[TIME]])/offer[QUANTITY]
        else:
            quantity_util = offer[QUANTITY]/(needed[offer[TIME]] - secured[offer[TIME]])

        time_factor = (0.9)**(offer[TIME] - parent.awi.current_step + 1)

        util = (price_util * self._price_weight + (1 - self._price_weight) * quantity_util)*time_factor
        return util
    ...
    def counter_all(
        ...
        if best_utility < 0:
            ...
            return {
                k: SAOResponse(response, best_proposals[k])
    ...
    def best_offers(self) -> Dict[str, "Outcome"]:
        ...
        return {nid: self.best_proposal_for_nid(nid)[0] for nid in self.negotiators.keys()}
...
class MyNewAgent(MyNegotiationManager, ReactiveTradingStrategy,
              SupplyDrivenProductionStrategy, SCML2020Agent):
    pass
...
if __name__ == "__main__":
    import matplotlib.pyplot as plt
    world = SCML2020World(
        **SCML2020World.generate([MyNewAgent, DecentralizingAgent], n_steps=10, n_processes=3, n_agents_per_process=3),
