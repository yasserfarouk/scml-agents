...
class avg_data_collect():
    def __init__(self,expected_inputs,expected_outputs,input_cost=0,output_cost=0,number_of_running = 10):
    ...
    def get_expected_inputs(self):
        ...
        self.expected_inputs = np.where(self.expected_inputs > 0, self.expected_inputs, 5)
        return self.expected_inputs // data_len

    def get_expected_outputs(self):
        data_len = self.len if self.len > 0 else 1
        self.expected_outputs = np.where(self.expected_outputs > 0, self.expected_outputs, 5)
...
class NewFixedTradePredictionStrategy(TradePredictionStrategy):
    ...
    def trade_prediction_init(self):
        ...
        self.datacollect = avg_data_collect(self.expected_inputs,self.expected_outputs, number_of_running=self.awi.n_steps)
    ...
    def on_contracts_finalized(
        ...
        for contract in signed:
            ...
            if is_seller:
                ...
                self.avg_o_p = (self.avg_o_p * self.avg_o_q + u * q)/(self.avg_o_q + q)
                self.avg_o_q += q
    ...
    def step(self):
        ...
        if t > 0:
            for i in range(t,len(self.inputs_secured)):
                expected_inventory = self.calc_expected_inv(self.inputs_secured, self.outputs_secured, i, t) #sum(self.inputs_secured[0:i]) - sum(self.outputs_secured[0:i])
    ...
    def sign_all_contracts(self, contracts: List[Contract]) -> List[Optional[str]]:
        ...
        for contract, indx in contracts:
            ...
            if is_seller:
                ...
                factor = 1
            if (factor*needed[t] < secured[t] + q + taken) and (not self.is_good_price(u, t, is_seller)):
                continue
...
class PredTestAgent(MyNegotiationManager, NewPredictionBasedTradingStrategy,
...
if __name__ =="__main__":
    for _ in range(10):
        world = SCML2020World(
            **SCML2020World.generate([PredTestAgent, DecentralizingAgent], n_steps=10, n_processes=3, n_agents_per_process=2, log_stats_every=1),
