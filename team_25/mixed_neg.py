...
class ProtectedSyncController(SyncController):
    ...
    def utility(self, offer: Tuple[int, int, int], max_price: int) -> float:
        ...
        """
        production_cost = np.max(self.__parent.awi.profile.costs[:, self.__parent.awi.my_input_product])
        if self._is_seller:
            _needed, _secured = (
    ...
    def good_neg_proposal(self, nid):
        ...
        production_cost = int(np.max(self.__parent.awi.profile.costs[:, self.__parent.awi.my_input_product]))
        n = len(good_offers)
        lowest_offer = good_offers[0]
        for i in range(n):
            if ((lowest_offer[0][2] < good_offers[i][0][2]) or 
            ((lowest_offer[0][0] < good_offers[i][0][0]) and (lowest_offer[0][2] == good_offers[i][0][2]))):
...
class MyAsp(AspirationNegotiator):
    ...
    def my_acceptable_unit_price(self, step: int, sell: bool):
        production_cost = np.max(self.manager.awi.profile.costs[:, self.manager.awi.my_input_product])
        expected_inventory = sum(self.manager.inputs_secured[0:step]) - sum(self.manager.outputs_secured[0:step])
    ...
    def respond(self, state, offer):
        ...
        if u is None or u < self.reserved_value:
            return ResponseType.REJECT_OFFER
        production_cost = np.max(self.manager.awi.profile.costs[:, self.manager.awi.my_input_product])
        ok_ufunc = self._utility_function((1,1,self.manager.output_price[0] - production_cost))
        asp = (
            self.aspiration(state.relative_time) * ((self.ufun_max*0.3 + ok_ufunc*0.7) - self.ufun_min)
            + self.ufun_min
        )
...
class StepBuyBestSellNegManager(StepNegotiationManager):
    ...
    def stepSell(self):
        ...
        needs = np.max(needed[self._current_start : self._current_end]
            - secured[self._current_start : self._current_end])

        # set a range of prices
        min_price = np.min([self.acceptable_unit_price(t, True) for t in
               range(self._current_start, self._current_end)])
    ...
    def respond_to_negotiation_request(
        ...
        is_seller = annotation["seller"] == self.id
        if is_seller:
            return self.sell_respond_to_negotiation_request(initiator, issues, annotation, mechanism)
        else:
            return self.buy_respond_to_negotiation_request(initiator, issues, annotation, mechanism)


class MixedNegAgent(_NegotiationCallbacks,
...
if __name__ == "__main__":
    ...
    world = SCML2020World(
        **SCML2020World.generate([MixedNegAgent, DecentralizingAgent], n_steps=30, n_processes=2, n_agents_per_process=2, log_stats_every=1),
