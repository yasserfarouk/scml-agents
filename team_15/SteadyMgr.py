...
from scml.scml2020 import SCML2020Agent, TradingStrategy, StepNegotiationManager, SupplyDrivenProductionStrategy
...
class MyTradingStrategy(TradingStrategy):
    def init(self):
        ...
        self.input_cost = (self.awi.catalog_prices[self.awi.my_output_product] - production_cost) * np.ones(self.awi.n_steps, dtype=int)
        self.output_price = (self.awi.catalog_prices[self.awi.my_input_product] + production_cost) * np.ones(self.awi.n_steps, dtype=int)

        # Maximum number of products that can be produced
        self.inputs_needed = (self.awi.n_steps * self.awi.n_lines) * np.ones(self.awi.n_steps, dtype=int)
    ...
    def on_contracts_finalized(
        ...
        for contract in signed:
            ...
            if is_seller:
                ...
                self.outputs_needed[t:] -= q    # Subtract the number of products that need to sell after t
                sold += 1

            else:
                self.inputs_secured[t] += q    # Add the number of buys at t
                self.inputs_needed[t:] -= q    # Subtract the number of units that need to buy after t
                self.outputs_needed[t + 1:] += q    # Add 'outputs_needed' as many as buys
                bought += 1
        ...
        if self.awi.my_output_product != (self.awi.n_products - 1):
            if sold > 0:    # If products are sold, the price is increased by 10% (upper limit is twice of the catalog price)
                self.output_price[step:] = min(self.output_price[step] + self.output_price[step] // 10, self.awi.catalog_prices[self.awi.my_output_product] * 2)
            else:    # If the product doesn't sell, the price is reduced by 10% (lower limit is the initial selling price)
                self.output_price[step:] = max(self.output_price[step] - self.output_price[step] // 10, self.output_price[0])
        ...
        if self.awi.my_input_product != 0:
            if bought > 0:    # If I could buy the units, reduce the buying price by 10% (lower limit is half of the catalogue price)
                self.input_cost[step:] = max(self.input_cost[step] - self.input_cost[step] // 10, self.awi.catalog_prices[self.awi.my_input_product] // 2)
            else:    # If I could not buy, increase the buying price by 10% (upper limit is the initial buying price)
                self.input_cost[step:] = min(self.input_cost[step] + self.input_cost[step] // 10, self.input_cost[0])
    ...
    def sign_all_contracts(self, contracts: List[Contract]) -> List[Optional[str]]:
        ...
        for contract, indx in contracts:
            ...
            if is_seller:
                ...
                available = est + self.internal_state["_output_inventory"] - (self.outputs_secured[s:]).sum()  # Add stock and sub contracted
                ...
                needed = self.inputs_needed[self.awi.n_steps - 1]    # Maximum number of products that can be produced
                if needed - bought > q:
                    signatures[indx] = self.id
                    bought += q
