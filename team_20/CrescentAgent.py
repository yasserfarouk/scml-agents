    ...
    def sign_all_contracts(self, contracts: List[Contract]) -> List[Optional[str]]:
        ...
        for step in range(self.awi.n_steps):
            if not sell_contracts[step]:
                continue
            sell_contracts[step].sort(key=lambda c: c.agreement["unit_price"], reverse=True)
            for sell_contract in sell_contracts[step]:
                ...
                if not bcus:
                    ...
                    continue
                #check unit price
                bcus_over_up = [c for c in bcus if c.agreement["unit_price"] < sell_contract.agreement["unit_price"]]
                if not bcus_over_up:
                ...
                for pick_times in range(1, len(bcus_over_up)):
                    contract_combinations = list(itertools.combinations(bcus_over_up, pick_times))
                    #each combination check prroduction-schedule,calculate sum(q) and sum(u*q)
                    for contract_combination in contract_combinations:
                        #check q_sum u*q_sum
                        q_sum,u_q_sum = self.quantities_cost_check(contract_combination)
                        #if not enough quantity
                        if q_sum < sell_contract.agreement["quantity"] and q_sum>=sign_q_sum and u_q_sum>=sign_u_q_sum:
                            continue
                        sign_q_sum = q_sum
                ...
                if sign_combination:
                    ...
                    for buy_contract in sign_combination:
                        ...
                        buy_contracts[buy_contract.agreement["time"]].remove(buy_contract)
