...
import numpy as np
from negmas import Contract, Issue, AgentMechanismInterface, Negotiator, MechanismState, ResponseType, SAOState

from scml.scml2020 import SCML2020Agent, TIME, ANY_LINE, QUANTITY, UNIT_PRICE, FinancialReport
...
class AgentProjectGCNego(SyncController):
    ...
    def utility(self, offer: Tuple[int, int, int], max_price: int) -> float:
        ...
        if self._is_seller:
            # seller
            ufunc = (offer[UNIT_PRICE] * q * revenue_coef) + ((1 / offer[TIME]) * time_coef)
        else:
            # buyer
            ufunc = ((0.5 * offer[UNIT_PRICE]) * q * revenue_coef) + ((1 / offer[TIME]) * time_coef)
...
class AgentProjectGC(
    SCML2020Agent):  # , IndependentNegotiationsManager, PredictionBasedTradingStrategy, DemandDrivenProductionStrategy):
    ...
    def sign_all_contracts(self, contracts: List[Contract]) -> List[Optional[str]]:
        ...
        return [heapq.heappop(possible_contracts)[1] for _ in range(len(possible_contracts))]

    # END

    # BEGIN PRODUCTION STRATEGY
    def on_contracts_finalized(self, signed: List[Contract], cancelled: List[Contract],
                               rejectors: List[List[str]]) -> None:
    ...
    def utility(self, is_seller: bool, issues: List[Issue], outcomes=None):
        ...
        if not is_seller:
            ufunc = (issues[UNIT_PRICE] * issues[QUANTITY] * -revenue_coef) + ((1 / issues[TIME]) * -time_coef)
        else:
            ufunc = (issues[UNIT_PRICE] * issues[QUANTITY] * revenue_coef) + ((1 / issues[TIME]) * time_coef)
