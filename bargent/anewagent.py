...
class MyPredictionBasedTradingStrategy(FixedTradePredictionStrategy, MeanERPStrategy, TradingStrategy):
    ...
    def on_contracts_finalized(
        ...
        self.awi.logdebug_agent(f"Exit Contracts Finalized:\n{pformat(self.internal_state)}")
    ...
    def sign_all_contracts(self, contracts: List[Contract]) -> List[Optional[str]]:
        ...
        contracts = sorted(
            ...
            key=lambda x: (
                x[0].agreement["unit_price"] if x[0].annotation["buyer"] == self.id else max_price - x[0].agreement["unit_price"],
                x[0].agreement["time"],
                0
