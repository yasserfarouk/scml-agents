...
from negmas import (AgentMechanismInterface, Breach, Contract, Issue,
                    MechanismState, Negotiator, ResponseType, LinearUtilityFunction)
...
class StdAgent(IndependentNegotiationsManager,SupplyDrivenProductionStrategy,SCML2020Agent):
    def init(self):
        self.n_shipments=0
        self.n_arrivals=0
    ...
    def step(self):
        for i in range(1,self.awi.n_steps):
            self.inventory[i]=max(self.inventory[i-1]+self.buyschedule[i-1]-self.awi.n_lines, 0)
        self.update_price()
        super().step()
    ...
    def sign_all_contracts(self,contracts):
        ...
        for d in range(5):
            for b in buy:
                q,p,t=b.agreement["quantity"],b.agreement["unit_price"],b.agreement["time"]
                ...
                if self.inventory[t]+arrivals+q<=productivity:
                    if (t==bfast and self.inventory[t]+self.buyschedule[t]<self.awi.n_lines) or p<=self.ip+d:
    ...
    def update_price(self):
        if self.n_arrivals>self.Pbuy*self.awi.n_lines:
            ...
            self.ip=min(self.ip+1,self.awi.catalog_prices[self.awi.my_input_product]-3)
        if self.n_shipments>self.Psell*self.awi.n_lines:
            self.op+=1
        elif self.awi.current_step<self.awi.n_steps*0.7:
            self.op=max(self.op-1,self.awi.catalog_prices[self.awi.my_output_product]+3)
        else:
            self.op-=1
    ...
    def on_contracts_finalized(self,signed,cancelled,rejectors):
        for c in signed:
            if self.awi.current_step<=c.agreement["time"] and c.agreement["time"]<self.awi.n_steps:
...
class CollusionAgent(StdAgent):
    ...
    def step(self):
        ...
        if self.awi.current_step == 0:
            super()._start_negotiations(
                ...
                uvalues=[1,int(np.max(self.awi.profile.costs[:,self.awi.my_input_product]))],
                tvalues=[1,1],
        ...
        if self.gifted and self.awi.state.balance+self.awi.catalog_prices[self.awi.my_output_product]*self.awi.state.inventory[self.awi.my_output_product]>self.initbal*(1+self.score):
            super()._start_negotiations(
                product=self.awi.my_input_product,
                sell = False,
                step = 1,
        ...
        if self.consumer!=[] and (self.awi.current_step>=self.awi.n_steps-self.awi.n_processes*2 or self.awi.state.balance<self.initbal//2):
    ...
    def respond_to_negotiation_request(self,
        ...
        if qmin==qmax and tmax==tmin and umax==umin and (partner in self.consumer or partner in self.supplier):
    ...
    def sign_all_contracts(self, contracts):
        ...
        for c in contracts:
            partner=c.annotation["buyer"] if c.annotation["buyer"]!=self.id else c.annotation["seller"]
