...
from utility_model import UtilityModel, load_seller_utiltiy_model, load_buyer_utility_model
...
def get_train_data(world):
    ...
    for agent in world.agents.values():
        ...
        seller_data = [negotiation for negotiation in agent.neg_history[True].values() if len(negotiation) > 0]
        seller_train_data.extend(seller_data)

        buyer_data = [negotiation for negotiation in agent.neg_history[False].values() if len(negotiation) > 0]
...
seller_train_features = [torch.from_numpy(feature).float() for feature in seller_train_features]
...
train_seller_data = all_seller_data[: -int(len(all_seller_data) * UTILITY_VALIDATION_SPLIT)]
test_seller_data = all_seller_data[-int(len(all_seller_data) * UTILITY_VALIDATION_SPLIT):]

buyer_train_features, buyer_train_tags = split_features_tags(buyer_train_data)
buyer_train_features = [torch.from_numpy(feature).float() for feature in buyer_train_features]
...
train_buyer_data = all_buyer_data[: -int(len(all_buyer_data) * UTILITY_VALIDATION_SPLIT)]
...
if UTILITY_TRAIN_SELLER:
    ...
    train_seller, valid_seller = seller_model.fit(train_seller_data, test_seller_data, save_model=UTILITY_SAVE_MODEL, path=UTILITY_SELL_PATH)
    seller_model.plot(train_seller, valid_seller)


if UTILITY_TRAIN_BUYER:
    print('buyer training...')
    train_buyer, valid_buyer = buyer_model.fit(train_buyer_data, test_buyer_data, save_model=UTILITY_SAVE_MODEL, path=UTILITY_BUY_PATH)
