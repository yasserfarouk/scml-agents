...
class MyTradePredictionStrategy(TradePredictionStrategy):
    ...
    def trade_prediction_step(self):
        ...
        expected_input = list(map(lambda x: round(x.item()), self.input_model(history)[-1]))
        expected_output = list(map(lambda x: round(x.item()), self.output_model(history)[-1]))
        ...
        if t + len(expected_input) < len(self.outputs_secured):
            ...
            self.expected_inputs[t + 1:] = expected_input[:len(self.expected_inputs[t + 1:])]
...
class MyPredictionBasedTradingStrategy(MyTradePredictionStrategy, PredictionBasedTradingStrategy):
    def __init__(self,
