...
from scml.scml2020.components.negotiation import NegotiationManager, AspirationNegotiator
from negmas import ResponseType, outcome_is_valid, SAOSyncController, Issue, AgentMechanismInterface, Negotiator
...
class MyFixedUtilityNegotiator(SAONegotiator):
    def __init__(
        ...
        super().__init__(assume_normalized=assume_normalized,
                         ufun=ufun,
                         name=name,
        ...
        if manager is None:
            ...
            self.manager = manager  # the negotiation manager (parent may be a controller)
    ...
    def build_data(self, state: MechanismState, offer: "Outcome"):
        ...
        if next_step >= self.manager.awi.n_steps - 1:
            ...
            data[AUX_NEEDED_START: AUX_NEEDED_END + 1] = self.pad(self.manager.outputs_needed[next_step: next_step + MAX_HORIZON],
                                                                  MAX_HORIZON)
            data[AUX_PRICE_START: AUX_PRICE_END + 1] = self.pad(self.manager.output_price[next_step: next_step + MAX_HORIZON],
                                                                MAX_HORIZON)
        else:
            data[AUX_NEEDED_START:AUX_NEEDED_END + 1] = self.pad(self.manager.inputs_needed[next_step: next_step + MAX_HORIZON],
                                                                 MAX_HORIZON)
            data[AUX_PRICE_START: AUX_PRICE_END + 1] = self.pad(self.manager.input_cost[next_step: next_step + MAX_HORIZON],
                                                                MAX_HORIZON)
    ...
    def fill_response_data(self, state: MechanismState, offer: "Outcome"):
        ...
        self.manager.neg_history[self._is_seller][self.id][-1][RESPONSE_RELATIVE_TIME:] = data[OFFER_RELATIVE_TIME : OFFER_UTILITY + 1]
    ...
    def predict_outcome(self, history, prediction_model):
        to_features = np.array([message[:RESPONSE_RELATIVE_TIME] for message in history])
    ...
    def propose(self, state: MechanismState) -> Optional["Outcome"]:
        ...
        # TODO: check this...
        time_range = (self.manager.awi.current_step + 1,
                      min(self.manager.awi.current_step + self.horizon, self.manager.awi.n_steps - 2) + 1)

        offer = [0] * 3

        history = self.manager.neg_history[self._is_seller][self.id]
        current_proposal = [0] * (OFFER_UTILITY + 1)
        current_proposal[AUX_NEEDED_START: AUX_NEEDED_END + 1] = self.pad(_needed[offer[TIME]: offer[TIME] + MAX_HORIZON],
                                                                          MAX_HORIZON)
        current_proposal[AUX_PRICE_START: AUX_PRICE_END + 1] = self.pad(_price[offer[TIME]: offer[TIME] + MAX_HORIZON],
                                                                        MAX_HORIZON)
        ...
        best_outcome = self.predict_outcome(history + [best_proposal], _prediction_model)[-1]
        ...
        for time in range(*time_range):
            ...
            current_proposal[AUX_NEEDED_START: AUX_NEEDED_END + 1] = self.pad(_needed[time: time + MAX_HORIZON],
                                                                              MAX_HORIZON)
            current_proposal[AUX_PRICE_START: AUX_PRICE_END + 1] = self.pad(_price[time: time + MAX_HORIZON],
                                                                            MAX_HORIZON)

            quantity_range = (1, max(_needed[time: time + MAX_HORIZON]) + 1)  # TODO: maybe allow even more?
            if self._is_seller:
            ...
            for quantity in range(*quantity_range):
                ...
                while start < end:
                    ...
                    predicted_outcome = self.predict_outcome(to_predict, _prediction_model)
...
class MySyncController(SAOSyncController):
    ...
    def create_negotiator(
        ...
        **kwargs,
        ...
        negotiator = super().create_negotiator(
            name=name,
            cntxt=cntxt,
            **kwargs
        )
    ...
    def step(self):
        ...
        for seller, needed, secured, product in [
            ...
            if seller:
                ...
                min_price = min(self.output_price[self._current_start:self._current_end])
                ...
                price_range = (0, max(self.input_cost[self._current_start:self._current_end]))
