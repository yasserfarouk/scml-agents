...
class SaveHistoryWrapper(SCML2020Agent):
    ...
    def init(self):
        ...
        inventory_input = state['_input_inventory']  # includes both production lines and the inventory
        ...
        needed_input3 = self.inputs_needed[t + 1] if t + 1 < len(self.inputs_secured) else 0
        needed_output1 = self.outputs_needed[t - 1] if t - 1 >= 0 else 0
        needed_output2 = self.outputs_needed[t]
        needed_output3 = self.outputs_needed[t + 1] if t + 1 < len(self.outputs_secured) else 0

        self.history = [[balance, inventory_input, inventory_output, productivity,
                             needed_input1, needed_input2, needed_input3,
                             needed_output1, needed_output2, needed_output3]]
    ...
    def step(self):
        ...
        needed_input1 = self.inputs_needed[t - 1] if t - 1 >= 0 else 0
        needed_input2 = self.inputs_needed[t]
        needed_input3 = self.inputs_needed[t + 1] if t + 1 < len(self.inputs_needed) else 0
        needed_output1 = self.outputs_needed[t - 1] if t - 1 >= 0 else 0
        needed_output2 = self.outputs_needed[t]
        needed_output3 = self.outputs_needed[t + 1] if t + 1 < len(self.outputs_needed) else 0

        self.history.append([balance, inventory_input, inventory_output, productivity,
                             needed_input1, needed_input2, needed_input3,
                             needed_output1, needed_output2, needed_output3])
...
class _NegotiationCallbacks:
    def average_production_cost(self):
        return sum(self.awi.profile.costs[:, self.awi.my_input_product]) / self.awi.n_lines
...
class ComponentAgent(IndependentNegotiationsManager, PredictionBasedTradingStrategy,
              SupplyDrivenProductionStrategy, SaveHistoryWrapper, SCML2020Agent):
    pass
...
class MyLearnNegotiationAgent(_NegotiationCallbacks,
...
class MyLearnUtilityAgent(_NegotiationCallbacks,
                          MySupplyDrivenProductionStrategy,
                          MyNegotiationManager,
                          MyPredictionBasedTradingStrategy,
                          SaveHistoryWrapper,
...
class UnicornAgent(_NegotiationCallbacks,
                   MySupplyDrivenProductionStrategy,
                   MyNegotiationManager,
                   MyPredictionBasedTradingStrategy,
                   SaveHistoryWrapper,
