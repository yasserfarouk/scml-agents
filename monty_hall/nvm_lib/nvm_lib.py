...
class NVMPlan:
    ...
    def __str__(self):
        return str([tuple((self.x[t], self.y[t], self.z[t])) for t in range(len(self.x))])
...
class NVMLib:
    def __init__(self,
        ...
        self.mpnvp_feasible_sols = NVMLib.read_qtty_feasible_domain(self.mpnvp_number_of_periods,
        ...
        if self.check_if_data_exists():
            ...
            self.q_inn_uncertainty_model = q_uncertainty_model['p' + str(self.input_product_index)]
            self.q_out_uncertainty_model = q_uncertainty_model['p' + str(self.output_product_index)]
            ...
            self.expectations_q_min_inn = {t: NVMLib.compute_min_expectation(self.q_inn_uncertainty_model[str(t + 1)],
                                                                             self.mpnvp_quantities_domain_size)
                                           for t in range(game_length - 1)}
            self.expectations_q_min_out = {t: NVMLib.compute_min_expectation(self.q_out_uncertainty_model[str(t + 1)],
                                                                             self.mpnvp_quantities_domain_size)
                                           for t in range(game_length - 1)}

            # Compute the expected quantities
            self.q_inn_expected = {t: sum([int(i) * p for i, p in self.q_inn_uncertainty_model[str(t + 1)].items()])
                                   for t in range(0, game_length - 1)}
            self.q_out_expected = {t: sum([int(i) * p for i, p in self.q_out_uncertainty_model[str(t + 1)].items()])
                                   for t in range(0, game_length - 1)}
    ...
    @staticmethod
    def read_qtty_feasible_domain(the_number_of_periods: int, the_quantities_domain_size: int) -> pd.DataFrame:
        """
        ...
        file_location = 'nvm_lib/qtty_domain/qtty_domain_t_' + str(
           the_number_of_periods) + '_d_' + str(the_quantities_domain_size) + '.zip'
        ...
        if not os.path.isfile(file_location):
            raise Exception(f'Could not find the file with feasible domain at {file_location}')
    ...
    def pandas_tuple_to_nvm_plan(sol: pd, number_of_periods: int) -> Optional[NVMPlan]:
        ...
        if sol is None:
            return None
        return NVMPlan([sol[t * 3] for t in range(number_of_periods)],
                       [sol[t * 3 + 1] for t in range(number_of_periods)],
                       [sol[t * 3 + 2] for t in range(number_of_periods)])

    @staticmethod
    def solve_mpnvm(the_feasible_sols: pd.DataFrame,
        ...
        for row in the_feasible_sols.itertuples(index=False):
            ...
            candidate_sol_value = sum(
                [the_prices_out[t] * the_expectations_q_min_out[t]['min_' + str(row[(t * 3) + 1])]
            ...
            if candidate_sol_value > optimal_sol_revenue:
                ...
                optimal_sol = row
                if verbose:
                    print(f'\tit took ' + format(time.time() - t0, '.4f') +
        ...
        return optimal_sol_revenue, NVMLib.pandas_tuple_to_nvm_plan(optimal_sol, T), positive_solutions
    ...
    def check_if_data_exists(self) -> bool:
        ...
        if not os.path.isfile(qttyFilePath):
            raise Exception(f'The uncertainty model for quantities could not be found at {qttyFilePath}')
        if not os.path.isfile(priceFilePath):
            raise Exception(f'The uncertainty model for prices could not be found at {priceFilePath}')
        return True

    def get_complete_plan(self, current_time: int, verbose: bool = False) -> Optional[NVMPlan]:
        ...
        slice_expectations_q_min_out = {t - current_time: self.expectations_q_min_out[t] for t in
                                        range(current_time, end)}
        slice_expectations_q_min_inn = {t - current_time: self.expectations_q_min_inn[t] for t in
                                        range(current_time, end)}
        ...
        slice_prices_inn = {t - current_time: self.prices_inn[str(t)] if str(t) in self.prices_inn else 0.0 for t in
                            range(current_time, end)}
        slice_prices_out = {t - current_time: self.prices_out[str(t)] if str(t) in self.prices_out else 0.0 for t in
                            range(current_time, end)}
        ...
        if verbose:
            ...
            t0 = time.time()
        optimal_sol_value, optimal_sol, positive_solutions = NVMLib.solve_mpnvm(self.mpnvp_feasible_sols,
