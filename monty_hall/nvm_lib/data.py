...
def get_prob_dist(raw_uncertainty_model: pd.DataFrame, time: int, product: str) -> dict:
    ...
    :return:
    """
    x = raw_uncertainty_model[(raw_uncertainty_model['time'] == time) & (raw_uncertainty_model['product'] == product)]
    total = x['count'].sum()
    return_dict = {}
...
def save_json_qytt_uncertainty_model(json_file_name: str, the_game_logs: pd.DataFrame, game_length: int):
    """
    ...
    d = {product: {t: get_prob_dist(raw_uncertainty_model, t, product) for t in range(game_length)} for product in products}
...
def save_json_price_data(json_file_name: str, the_game_logs: pd.DataFrame):
    ...
    for product in the_game_logs['product'].unique():
        ...
        product_data_grouped['avg_weighted_price'] = product_data_grouped['weighted_price'] / product_data_grouped['quantity']
