...
def generate_domain(the_production_capacity: int, number_of_periods: int, quantities_domain_size: int):
    """
...
def bulk_domain_generation(the_production_capacity: int,
    ...
    """
    # Solve the CP
    for number_of_periods in range(the_lower_bound_number_of_periods, the_upper_bound_number_of_periods + 1):
        for quantities_domain_size in range(the_lower_bound_qtty_domain_size, the_upper_bound_qtty_domain_size + 1):
            if not os.path.isfile(get_zip_location(number_of_periods, quantities_domain_size)):
                generate_domain(the_production_capacity, number_of_periods, quantities_domain_size)
            else:
