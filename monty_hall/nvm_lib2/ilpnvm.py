...
def construct_ILP():
    ...
    inn_vars = pulp.LpVariable.dicts('inn', ((t, k) for t, k in it.product(range(0, T), range(0, q_max))),
                                     lowBound=0, upBound=1, cat='Integer')
    # out_vars[t][k] == 1 iff in the business plan the agent tries to sell k inputs at time t
    out_vars = pulp.LpVariable.dicts('out', ((t, k) for t, k in it.product(range(0, T), range(0, q_max))),
                                     lowBound=0, upBound=1, cat='Integer')
...
def print_pretty_tables():
    ...
    x.add_row(['B-E'] + [str(round(inn[t][buy_plan[t]], 2)) for t in range(0, T)] + [str(round(total_exp_buy_qtty, 2))])
    total_exp_sell_qtty = sum([out[t][sell_plan[t]] for t in range(0, T)])
    x.add_row(
        ['S-E'] + [str(round(out[t][sell_plan[t]], 2)) for t in range(0, T)] + [str(round(total_exp_sell_qtty, 2))])
    print(x)
    x = PrettyTable()
