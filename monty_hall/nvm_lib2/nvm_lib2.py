...
class NVMLib2:
    ...
    def check_if_data_exists(self) -> bool:
        ...
        if not price_path.is_file():
            raise Exception(f'The uncertainty model for prices could not be found at {price_path}')
        return True
    ...
    def generate_synthetic_uncertainty_model(T: int, q_max: int):
        ...
        Q_inn = {t: {q: random_quantities_inn[t][q] / normalization_qtt_inn[t] for q in range(0, q_max)} for t in
                 range(0, T)}
        ...
        Q_out = {t: {q: random_quantities_out[t][q] / normalization_qtt_out[t] for q in range(0, q_max)} for t in
                 range(0, T)}
        # Random prices
    ...
    def construct_ILP(self, T: int, q_max: int, inn: Dict[int, Dict], out: Dict[int, Dict],
                      p_inn, p_out, current_inventory: int):
        t0 = time.time()
        ...
        inn_vars = pulp.LpVariable.dicts('inn', (
        ...
        if optimistic:
            ...
            for t in range(self.current_time, self.current_time + T):
                ...
                right_hand_size += sum(
                    [inn_vars[t, k] * inn[t][k] - out_vars[t, k] * out[t][k] for k in range(0, q_max)])
    ...
    def print_pretty_tables(self, T, q_max, current_inventory, inn, out, p_inn, p_out, buy_plan, sell_plan,
                            time_to_generate_ILP, solve_time, total_time, optimistic):
        x = PrettyTable()
        x.field_names = ['t'] + [t for t in range(self.current_time, self.current_time + T)] + ['total']
        total_buy_qtty = sum([buy_plan[t] for t in range(self.current_time, self.current_time + T)])
        x.add_row(['B-Q'] + [buy_plan[t] for t in range(self.current_time, self.current_time + T)] + [total_buy_qtty])
        total_sell_qtty = sum([sell_plan[t] for t in range(self.current_time, self.current_time + T)])
        x.add_row(['S-Q'] + [sell_plan[t] for t in range(self.current_time, self.current_time + T)] + [total_sell_qtty])
        x.add_row(['B-P'] + [str(round(p_inn[str(t)], 2)) for t in range(self.current_time, self.current_time + T)] + ['--'])
        x.add_row(['S-P'] + [str(round(p_out[str(t)], 2)) for t in range(self.current_time, self.current_time + T)] + ['--'])
        total_exp_buy_qtty = sum([inn[t][buy_plan[t]] for t in range(self.current_time, self.current_time + T)])
        x.add_row(
            ['B-E'] + [str(round(inn[t][buy_plan[t]], 2)) for t in range(self.current_time, self.current_time + T)] +
            [str(round(total_exp_buy_qtty, 2))])
        total_exp_sell_qtty = sum([out[t][sell_plan[t]] for t in range(self.current_time, self.current_time + T)])
        x.add_row(
