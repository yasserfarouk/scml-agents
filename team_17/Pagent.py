...
from scml.scml2020.agents import DecentralizingAgent, BuyCheapSellExpensiveAgent, IndDecentralizingAgent
    ...
    def on_contracts_finalized(
        ...
        for contract in signed:
            ...
            if self.awi.current_step == (self.awi.n_steps-1):
                if self.awi.profile.costs[0, self.awi.my_input_product] > (self.awi.catalog_prices[self.awi.my_input_product]/2):
                    continue
...
class AvoidOverproductionTradingStrategy(FixedTradePredictionStrategy, MeanERPStrategy, TradingStrategy):
    def init(self):
        super().init()
            ...
            if self.awi.my_suppliers == ['SELLER']:
                ...
                if is_seller:
                    ...
                    if zaiko < 1:
                        if t < s + 3:
                            continue
                    sellprice = max((self.awi.catalog_prices[self.awi.my_output_product] - self.awi.catalog_prices[self.awi.my_input_product] - self.awi.profile.costs[0, self.awi.my_input_product]) // 2 - 1, 0)
                    if u < self.awi.catalog_prices[self.awi.my_output_product] - sellprice:
                        continue
                ...
                if is_seller:
                    ...
                    if needtime == s + 1:
                        ...
                        buyprice2 = max((self.awi.catalog_prices[self.awi.my_output_product] - self.awi.catalog_prices[self.awi.my_input_product]) // 2 - 1, 0)
                        if u < self.awi.catalog_prices[self.awi.my_input_product] + buyprice2:
                            continue
                    else:
                        buyprice = max((self.awi.catalog_prices[self.awi.my_output_product] - self.awi.catalog_prices[self.awi.my_input_product] - self.awi.profile.costs[0, self.awi.my_input_product]) // 2 - 1, 0)
                        if u < self.awi.catalog_prices[self.awi.my_input_product] + buyprice:
                            continue
...
class PreNegotiationManager(IndependentNegotiationsManager):
    ...
    def _start_negotiations(
        ...
        super()._start_negotiations(product, sell, step, qvalues, uvalues, tvalues, partners)
    ...
    def target_quantity(self, step: int, sell: bool) -> int:
        if sell:
            if sum(self.outputs_secured) < self.awi.n_lines * (self.awi.n_steps - self.awi.current_step - 2):
                sellnum = min(self.awi.n_lines, self.outputs_secured[step])
            else:
                sellnum = 0
        else:
            if sum(self.inputs_secured) < self.awi.n_lines * (self.awi.n_steps - self.awi.current_step - 2):
...
class MyAgent(AvoidOverproductionTradingStrategy, PreNegotiationManager, AllmakeProductionStrategy, SCML2020Agent):
    def init(self):
        super().init()
    def step(self):
        super().step()