...
for path in get_stats(path_dir):
    ...
    stats_step_X = np.genfromtxt(path, delimiter=",", skip_header=1, usecols=[8,11])  # trading_price_1,2だけ
    stats_step_y = np.genfromtxt(path, delimiter=",", skip_header=1, usecols=[14])  # trading_price_2を予測
    # print(stats_step_y)
    if stats_X is None:
...
print('Coefficient of determination: %.2f'
      % r2_score(stats_y_test, stats_y_pred))
