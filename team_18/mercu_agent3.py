...
from scml.scml2020.components.negotiation import NegotiationManager, StepNegotiationManager
from scml.scml2020.components.trading import TradingStrategy
from scml.scml2020.components.prediction import ExecutionRatePredictionStrategy, MeanERPStrategy
...
class MyNegotiationManager(NegotiationManager):
    ...
    def respond_to_negotiation_request(
        ...
        step = max(
            0, tmin - 1) if is_seller else min(self.awi.n_steps - 1, tmax + 1)
    ...
    def all_negotiations_concluded(
        ...
        self.awi.logdebug_agent(
            f"Killing Controller {str(controllers.controller)}"
        )
    ...
    def add_controller(
        ...
        if is_seller and self.seller.controller is not None:
            if next_step != self.seller.step == next_step:
                self.seller.controller.update_param(
                    target, urange, agent_confidence)
        ...
        if not is_seller and self.buyer.controller is not None:
            if next_step != self.buyer.step:
                self.buyer.controller.update_param(
                    target, urange, agent_confidence)
...
class MyTradePredictionStrategy():
    ...
    def trade_prediction_init(self) -> None:
        ...
        self.output_price = self.awi.catalog_prices[self.awi.my_input_product] * np.ones(
            self.awi.n_steps)
    ...
    def trade_prediction_step(self) -> None:
        ...
        input_diff = self.expected_inputs[cur_step] - \
            self.inputs_secured[cur_step]
        output_diff = self.expected_outputs[cur_step] - \
            self.outputs_secured[cur_step]
        if input_diff > 0:
            input_cor_value = 0
        elif input_diff == 0:
            input_cor_value = 2
        ...
        if inp_not_zero_num == 0:
            ...
            inp_sec_ave = min((inp_sec_ave / inp_not_zero_num) + input_cor_value,
                              self.awi.n_lines)
            inp_sec_ave = max(1, inp_sec_ave)
        if oup_not_zero_num == 0:
            oup_sec_ave = self.awi.n_lines // 2
        else:
            oup_sec_ave = min((oup_sec_ave / oup_not_zero_num) + output_cor_value,
                              self.awi.n_lines)
        ...
        self.expected_inputs[self.awi.my_input_product -
                             self.awi.n_processes:] = 0
...
class MyTradingStrategy(MyTradePredictionStrategy, TradingStrategy):
    ...
    def step(self):
        ...
        self.inputs_needed[:-1] = self.expected_outputs[1:]
        self.outputs_needed[1:] = self.expected_inputs[:-1]
        if self.awi.n_steps - (self.awi.n_processes - self.awi.my_input_product) >= self.awi.current_step:
    ...
    def on_contracts_finalized(
        ...
        if seller_num:
            self.output_price[self.awi.current_step:] = total_output_price / \
                seller_num + 1
        else:
            self.output_price = self.output_price
        # predict input_cost
        if buyer_num:
            self.input_cost[self.awi.current_step:] = total_input_price / \
                buyer_num + 1
        else:
    ...
    def sign_all_contracts(self, contracts: List[Contract]) -> List[Optional[str]]:
        ...
        contracts = sorted(
            ...
            key=lambda x: (
                ...
                x[0].agreement["time"],
                0 if is_system_agent(x[0].annotation["seller"]) or is_system_agent(x[0].annotation["buyer"])
                else 1,
                x[0].agreement["unit_price"],
            ),
        )
