...
from negmas import (AgentMechanismInterface, Breach, Contract, Issue,
                    MechanismState, Negotiator, UtilityFunction, SAONegotiator, AspirationNegotiator,
                    UtilityValue, Outcome)
...
class MyAgent(SCML2020Agent):
    ...
    def init(self):
        ...
        print_log("catalog_in = " + str(self.awi.catalog_prices[self.awi.my_input_product]))
        print_log("catalog_out = " + str(self.awi.catalog_prices[self.awi.my_output_product]))
        print_log("production cost = " + str(self.awi.profile.costs[0, self.awi.my_input_product]))
    ...
    def step(self):
        ...
        for i in range(self.awi.current_step):
            self.stock_cap[i] = 0
        self.stock_cap[self.awi.current_step] = self.awi.state.inventory[self.awi.my_output_product]
        ...
        if self.awi.current_step + 1 <= self.awi.n_steps - (
            ...
            tvalues = (self.awi.current_step + 1,
                       min(self.awi.current_step + 6,
                           self.awi.n_steps - (len(self.awi.state.inventory) - self.awi.my_input_product))
                       )
        ...
        if self.awi.current_step + 3 <= self.awi.n_steps - 1:
            ...
            uvalues = (max(int(self.awi.catalog_prices[self.awi.my_output_product] * 1.7),
                           self.awi.catalog_prices[self.awi.my_input_product] + self.awi.profile.costs[
                               0, self.awi.my_input_product] * 3),
    ...
    def create_ufun(self, is_seller: bool, issues=None) -> UtilityFunction:
        ...
        return MySpecialUtilityFunction(is_seller=is_seller, issues=issues, awi=self.awi)
    ...
    def negotiator(self, is_seller: bool, issues=None) -> Optional[SAONegotiator]:
        ...
        return AspirationNegotiator(ufun=self.create_ufun(is_seller=is_seller, issues=issues),
    ...
    def sign_all_contracts(
        ...
        buy_conts = [contract for contract in contracts if contract.annotation["seller"] != self.id]
        sell_conts = [contract for contract in contracts if contract.annotation["seller"] == self.id]

        print_log("Got " + str(len(buy_conts)) + " buy contracts and " + str(
            len(sell_conts)) + " sell contracts on day " + str(self.awi.current_step))
        print_log("=== BUY ===")
        for c in buy_conts:
            print_log(
        ...
        for contract in sorted(buy_conts,
                               key=lambda x: (x.agreement["time"],
                                              x.agreement["unit_price"],
                                              -1 * x.agreement["quantity"])):
            new_line_cap, new_stock_cap = self.schedule_process(
                est_line_cap, est_stock_cap, contract.agreement["time"], contract.agreement["quantity"])
        ...
        for contract in sorted(sell_conts,
                               key=lambda x: (x.agreement["time"],
                                              -1 * x.agreement["unit_price"],
                                              -1 * x.agreement["quantity"])):
            new_stock_cap, num_of_shortage = self.pick_stock(
                est_stock_cap, contract.agreement["time"], contract.agreement["quantity"])
            if num_of_shortage is None:
                good_cont_set.append(contract.id)
                est_stock_cap = new_stock_cap

        print_log("GCS = " + str(good_cont_set))
        return ["Hello world!!" if contract.id in good_cont_set else None for contract in contracts]
    ...
    def eval_set(self, cont_set) -> float:
        ...
        # 各要素に対する評価値 [0,1]
        # 生産ライン
        l_nml = (self.awi.n_lines * self.awi.n_steps - sum(est_line_cap)) / (self.awi.n_lines * self.awi.n_steps)
        # 製品余剰
        s_nml = gauss(sum(est_stock_cap), mu=2 * self.awi.n_lines, sigma=self.awi.n_lines)
        # 利益
        p_nml = 1 / (1 + math.exp(
            -(profit - 2 * self.awi.profile.costs[0, self.awi.my_input_product]) / self.awi.catalog_prices[
    ...
    def simulate_set(self,
                     buy_conts: List[Contract],
                     sell_conts: List[Contract]
        ...
        for contract in buy_conts:
            profit -= (contract.agreement["unit_price"] + self.awi.profile.costs[0, self.awi.my_input_product]
        ...
        for contract in sell_conts:
            ...
            est_stock_cap, num_of_shortage = self.pick_stock(est_stock_cap, contract.agreement["time"],
                                                             contract.agreement["quantity"])
            if num_of_shortage:
    ...
    def on_contracts_finalized(
        ...
        buy_conts = [contract for contract in signed if contract.annotation["seller"] != self.id]
        sell_conts = [contract for contract in signed if contract.annotation["seller"] == self.id]

        print_log("Finalized " + str(len(buy_conts)) + " buy contracts and " + str(
            len(sell_conts)) + " sell contracts on day " + str(self.awi.current_step))
        print_log("=== BUY ===")
        for c in buy_conts:
            print_log(
        ...
        for contract in buy_conts:
            new_line_cap, new_stock_cap = self.schedule_process(
                self.line_cap, self.stock_cap, contract.agreement["time"], contract.agreement["quantity"])
            if new_line_cap is None:
                print_log("######################### Never Printed #########################")
        ...
        for contract in sell_conts:
            new_stock_cap, num_of_shortage = self.pick_stock(self.stock_cap, contract.agreement["time"],
                                                             contract.agreement["quantity"])
            if num_of_shortage:
    ...
    def schedule_process(self,
        ...
        return self.schedule_process(line_cap, self.cover_shortage(stock_cap), input_date, input_num)

    def pick_stock(self,
                   stock_cap: List[int],
                   output_date: int,
                   output_num: int
                   ) -> Tuple[List[int], Optional[int]]:  # returns (estimated_stock_cap, num_of_shortage)
        stock_cap = copy.copy(stock_cap)
...
class MySpecialUtilityFunction(UtilityFunction):
    ...
    def eval(self, offer: "Outcome") -> UtilityValue:
        ...
        if self.is_seller:
            price_nml = (price - self.awi.catalog_prices[self.awi.my_output_product] + self.awi.profile.costs[
                0, self.awi.my_input_product]) / (4 * self.awi.profile.costs[0, self.awi.my_input_product])
            quantity_nml = 1.0 if quantity < self.awi.n_lines else 2.0 - quantity / self.awi.n_lines
            date_nml = 1.0 - (date - self.awi.current_step) / (
                    self.awi.n_steps - self.awi.current_step) if date > self.awi.current_step else -1.0

            util = 0.5 * price_nml + 0.3 * quantity_nml + 0.2 * date_nml

        else:
            price_nml = (self.awi.catalog_prices[self.awi.my_output_product] - price + self.awi.profile.costs[
                0, self.awi.my_input_product]) / (4 * self.awi.profile.costs[0, self.awi.my_input_product])
            quantity_nml = 1.0 if quantity < self.awi.n_lines else 2.0 - quantity / self.awi.n_lines
            date_nml = (date - self.awi.current_step) / (
                    self.awi.n_steps - self.awi.current_step) if date > self.awi.current_step else -1.0
...
class GreedyFactoryManager2(MyAgent):
    pass
