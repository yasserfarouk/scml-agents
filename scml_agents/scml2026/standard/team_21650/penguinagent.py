...
MAX_EXACT_SUBSET_PARTNERS = 12


def exact_subsets(partners):
    partners = list(partners)
    for r in range(len(partners) + 1):
        yield from combinations(partners, r)


def quantity_subset_dp(current_step_offers, needs: int):
    """Find near-need subsets, using exact search for ordinary cases."""
    partners = list(current_step_offers.keys())
    if len(partners) <= MAX_EXACT_SUBSET_PARTNERS:
        best_plus: tuple[str, ...] = tuple()
        best_minus: tuple[str, ...] = tuple()
        best_plus_diff = float("inf")
        best_minus_diff = float("inf")
        for partner_ids in exact_subsets(partners):
            if not partner_ids:
                continue
            offered = sum(int(current_step_offers[p][QUANTITY]) for p in partner_ids)
            diff = abs(offered - int(needs))
            if offered >= int(needs):
                if diff < best_plus_diff and int(needs) > 0:
                    best_plus_diff = diff
                    best_plus = tuple(partner_ids)
            elif offered > 0 and diff < best_minus_diff:
                best_minus_diff = diff
                best_minus = tuple(partner_ids)
        return best_plus, best_plus_diff, best_minus, best_minus_diff

    states: dict[int, tuple[str, ...]] = {0: tuple()}

    def qty(partner: str) -> int:
        return int(current_step_offers[partner][QUANTITY])

    def price(partner: str) -> float:
        p = current_step_offers[partner][UNIT_PRICE]
        return 0.0 if p is None else float(p)

    def rank(subset: tuple[str, ...]) -> tuple[float, int]:
        revenue = sum(qty(p) * price(p) for p in subset)
        return (-revenue, len(subset))

    for partner, offer in current_step_offers.items():
        ...
        for total, subset in list(states.items()):
            ...
            incumbent = updates.get(new_total, states.get(new_total))
    ...
    for total, subset in states.items():
        if total <= 0 or not subset:
            continue
        diff = abs(total - int(needs))
        if total >= int(needs) and int(needs) > 0:
            key = (diff, rank(subset))
            if not best_plus or key < (best_plus_diff, rank(best_plus)):
                best_plus = subset
                best_plus_diff = diff
        elif total < int(needs):
            key = (diff, rank(subset))
            if not best_minus or key < (best_minus_diff, rank(best_minus)):
                best_minus = subset
                best_minus_diff = diff
    return best_plus, best_plus_diff, best_minus, best_minus_diff
...
class PenguinAgent(StdSyncAgent):
    ...
    def counter_all(self, offers, states):
        ...
        for edge_needs, all_partners, issues in [
            ...
            if best_plus_diff <= self._threshold and best_plus:
            ...
            if has_accept_offer and needs > 0:
                others = list(all_current_step_partners.difference(partner_ids))
