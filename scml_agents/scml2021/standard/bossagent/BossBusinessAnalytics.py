...
class BossBusinessAnalytics:
	...
	def update_stats(self):  # for reference
		# Nego stat history.
		# nego bid history.
		opponent_respond_history = self.__parent.nego_stats.get_opponent_respond_history()
		agent_respond_history = self.__parent.nego_stats.get_agent_respond_history()

		# AWI Dynamic Nego calls.

		current_step = self.__parent.awi_caller.get_current_step() # Returns day number (integer), e.g. 3rd day.
		relative_time = self.__parent.awi_caller.get_relative_time() # Normalized negotiation time between [0, 1].
		my_balance = self.__parent.awi_caller.get_balance() # bizim para
		current_inventory = self.__parent.awi_caller.get_current_inventory() # list -> [0, 0, 3, 5] index prod levellari gosteriyor
		inventory_input_prod_quantity = self.__parent.awi_caller.get_inventory_input_prod_quantity() #bizim
		inventory_output_prod_quantity = self.__parent.awi_caller.get_inventory_output_prod_quantity() #bizim
		input_catalog_price = self.__parent.awi_caller.get_input_catalog_price()  # this
		output_catalog_price = self.__parent.awi_caller.get_output_catalog_price()  # this

		# Agent reports

		my_first_supplier_id = self.__parent.awi_caller.get_suppliers()[0]
		all_reports = self.__parent.awi_caller.get_agent_all_reports(agentID=my_first_supplier_id) # Returns all history until current step.
		latest_report = self.__parent.awi_caller.get_latest_agent_report(agentID=my_first_supplier_id) # Returns latest possible report of the agent.
	...
	def pred_issues(self, agent_id):
		...
		for day in negotiation_bid_history[agent_role].keys():
			...
			for agent in negotiation_bid_history[agent_role][day].keys():
				...
				if agent == agent_id:
					contracts = list(negotiation_bid_history[agent_role][day][agent].values())
					...
					for contract in contracts:
						...
						if ("accept" in list(contract["Acceptance"].keys())) and (contract["Acceptance"]["accept"] == 1):
		...
		regressor_q = lm.LinearRegression(fit_intercept=True).fit(x_q, np.array(accepted_q))
		x_pred_q = np.array(len(accepted_q))
		x_pred_q = x_pred_q.reshape(1, 1)  # x_pred_q = x_pred_q.reshape(len(x_pred_q), 1)
		q_pred = regressor_q.predict(x_pred_q)

		# Predict P
		x_p = np.array(range(len(accepted_p)))
		x_p = x_q.reshape(len(x_p), 1)
		regressor_p = lm.LinearRegression(fit_intercept=True).fit(x_p, np.array(accepted_p))
		...
		regressor_t = lm.LinearRegression(fit_intercept=True).fit(x_t, np.array(accepted_t))
