import copy
from .helper import (is_schedule_available, schedule_dispatch_production, buyer_closest_available_delivery,
 					seller_closest_available_delivery, most_available_amount_in_schedule)
	...
	def run_dispatch(self, buyers, sellers, formatted_schedule):
		...
		if len(buyers) == 0: # If there are no buyer, all sellers are red.
			for seller in sellers:
				...
				dispatch_categories['Red'][seller_id] = {'Offer': seller_offer, 'Issues': {'No Buyer': 'No Buyer'}}
		...
		if len(sellers) == 0:
			for buyer in buyers:
				...
				buyer_id = buyer[0]
				buyer_offer = buyer[1]
				dispatch_categories['Red'][buyer_id] = {'Offer': buyer_offer, 'Issues': {'No Seller': 'No Seller'}}
		...
		for buyer_index, buyer in enumerate(buyers):
			...
			for seller in sellers:
				...
				if (seller_id not in dispatch_categories['Green'].keys()) or (seller_id in dispatch_categories['Yellow'].keys() and buyers[buyer_index-1][0] in dispatch_categories['Green'].keys()):
					if (seller_id in dispatch_categories['Yellow'].keys() and buyers[buyer_index-1][0] in dispatch_categories['Green'].keys()):
						is_nego_with_yellow = True
					# Calculate the remaining volume for the buyer.
					buyer_remaining_partial_volume = buyer_volume - buyer_paid_partial_volume 
					if seller_id in dispatch_categories['Yellow'].keys(): # If seller is yellow.
						seller_volume = dispatch_categories['Yellow'][seller_id]['Surplus'] # Update the available seller volume with surplus amount.
					# Check if this seller can manage to fill remaining volume.
					if buyer_remaining_partial_volume <= seller_volume: # Check if seller can supply the demand with a surplus (>= 0).
						seller_new_remaining_partial_volume = seller_volume - buyer_remaining_partial_volume
						# Calculate profit.
						profit = ((buyer_price - self.production_cost) * buyer_volume) - (buyer_paid_partial_price + (buyer_remaining_partial_volume * seller_price))
						# Check if schedule is available or not.
						is_available, _ = is_schedule_available(temp_dispatch_schedule, buyer_remaining_partial_volume, seller_delivery_time, buyer_delivery_time)
						# Check whether this buyer is profitable and schedule is ok.
						if profit > 0 and is_available: 
							if seller_new_remaining_partial_volume == 0: # If we supply the buyer's demand with profit and exact same amount.									
								temp_dispatch_categories['Yellow'].pop(seller_id, None) # If seller is yellow, we can pop seller from yellow.
								temp_dispatch_categories['Red'].pop(seller_id, None) # If seller is red, we can pop seller from red.																	
								# We add seller to the green list.
								temp_dispatch_categories['Green'][seller_id] = {'Offer': seller_offer}
							else: # If we supply the buyer's demand with profit and surplus.
								temp_dispatch_categories['Red'].pop(seller_id, None) # If seller is red, we can pop seller from red.	
								# We add (or update) the seller to the yellow group.
								temp_dispatch_categories['Yellow'][seller_id] = {'Offer': seller_offer, 'Surplus': seller_new_remaining_partial_volume, 'Target': None}
							# We will put seller to the green either way.	
							temp_dispatch_categories['Green'][buyer_id] = {'Offer': buyer_offer}
							# Update global categories with modifications.
							dispatch_categories = copy.deepcopy(temp_dispatch_categories)
							# Update the temp schedule.
							temp_dispatch_schedule, partner_schedule = schedule_dispatch_production(temp_dispatch_schedule, buyer_remaining_partial_volume, seller_delivery_time, buyer_delivery_time)
							...
							if profit <= 0: # Check if problem is price.
								buyer_issue = 'Price'
								buyer_issue_target = ((buyer_paid_partial_price + (buyer_remaining_partial_volume * seller_price)) / (buyer_volume * 1.0)) + self.production_cost + 1
							...
							if not is_available: # Otherwise check delivery time, since seller can fill this buyer to green.
								buyer_issue_target = buyer_closest_available_delivery(temp_dispatch_schedule, buyer_remaining_partial_volume, seller_delivery_time, buyer_delivery_time)
								if buyer_issue_target != -1:
									buyer_issue = 'Delivery'							
									buyer_issues[buyer_issue] = buyer_issue_target
								else:
									buyer_issue = 'Volume'
									buyer_issues[buyer_issue] = most_available_amount_in_schedule(temp_dispatch_schedule, seller_delivery_time, buyer_delivery_time)
							# Add buyer to the red list, with problematic issue and the target for offer.	
							dispatch_categories['Red'][buyer_id] = {'Offer': buyer_offer, 'Issues': buyer_issues}
							 # If seller is not yellow before, we should add it to the red.
							if seller_id not in dispatch_categories['Yellow'].keys():
								if profit <= 0:
									seller_issue = 'Price'
									seller_issue_target = (((buyer_price - self.production_cost) * buyer_volume) - buyer_paid_partial_price) / (buyer_remaining_partial_volume * 1.0)
									seller_issues[seller_issue] = seller_issue_target
								if not is_available:
									seller_issue_target = seller_closest_available_delivery(temp_dispatch_schedule, buyer_remaining_partial_volume, buyer_delivery_time)
									if seller_issue_target != -1:
										seller_issue = 'Delivery'							
										seller_issues[seller_issue] = seller_issue_target
									else:
										seller_issue = 'Volume'
										seller_issues[seller_issue] = most_available_amount_in_schedule(temp_dispatch_schedule, seller_delivery_time, buyer_delivery_time)
								dispatch_categories['Red'][seller_id] = {'Offer': seller_offer, 'Issues': seller_issues}
						...
						buyer_new_remaining_partial_volume = buyer_remaining_partial_volume - seller_volume
						buyer_new_paid_partial_volume = buyer_volume - buyer_new_remaining_partial_volume
						# Calculate profit.
						profit = ((buyer_price - self.production_cost) * buyer_new_paid_partial_volume) - (buyer_paid_partial_price + (seller_volume * seller_price))
						# Check if schedule is available or not.
						is_available, _ = is_schedule_available(temp_dispatch_schedule, seller_volume, seller_delivery_time, buyer_delivery_time)
						# Check whether this buyer is profitable and schedule is ok.
						if profit > 0 and is_available: # This buyer is still profitable.
							...
							temp_dispatch_categories['Green'][seller_id] = {'Offer': seller_offer}
							# Update the temp schedule.
							temp_dispatch_schedule, partner_schedule = schedule_dispatch_production(temp_dispatch_schedule, seller_volume, seller_delivery_time, buyer_delivery_time)	
							...
							if profit <= 0: # Check if problem is price.
								buyer_issue = 'Price'
								buyer_issue_target = ((buyer_paid_partial_price + (seller_volume * seller_price)) / (buyer_new_paid_partial_volume * 1.0)) + self.production_cost + 1
								buyer_issues[buyer_issue] = buyer_issue_target
							if not is_available: # Otherwise its volume, since seller can fill this buyer to green.
								buyer_issue_target = buyer_closest_available_delivery(temp_dispatch_schedule, seller_volume, seller_delivery_time, buyer_delivery_time)
								if buyer_issue_target != -1:
									buyer_issue = 'Delivery'
									buyer_issues[buyer_issue] = buyer_issue_target
								else:
							...
							if seller_id not in dispatch_categories['Yellow'].keys():
								if profit <= 0:
									seller_issue = 'Price'
									seller_issue_target = (((buyer_price - self.production_cost) * buyer_new_paid_partial_volume) - buyer_paid_partial_price) / (seller_volume * 1.0)
									seller_issues[seller_issue] = seller_issue_target
								if not is_available:
									seller_issue_target = seller_closest_available_delivery(temp_dispatch_schedule, seller_volume, buyer_delivery_time)
									if seller_issue_target != -1:
										seller_issue = 'Delivery'		
										seller_issues[seller_issue] = seller_issue_target
									else:
			...
			if buyer_id not in dispatch_categories['Red'] and buyer_id not in dispatch_categories['Green']:
				# We should mark buyer as red, and volume should be partial volume that is added until now.
				if buyer_paid_partial_volume != 0:
					dispatch_categories['Red'][buyer_id] = {'Offer': buyer_offer, 'Issues': {'Volume': buyer_paid_partial_volume}}	
				else:
					dispatch_categories['Red'][buyer_id] = {'Offer': buyer_offer, 'Issues': {'No Seller': 'No Seller'}}	

			if is_nego_with_yellow and buyer_id in dispatch_categories['Red']:
				if 'Issues' in dispatch_categories['Red'][buyer_id].keys():
					if 'Volume' in dispatch_categories['Red'][buyer_id]['Issues'].keys():
						yellow_id = list(dispatch_categories['Yellow'].keys())[0]		
						dispatch_categories['Yellow'][yellow_id]['Target'] = dispatch_categories['Yellow'][yellow_id]['Offer'][0] + (buyer_volume - buyer_paid_partial_volume)
		...
		for seller in sellers:	
			...
			if seller_id not in dispatch_categories['Red'] and seller_id not in dispatch_categories['Yellow'].keys() and seller_id not in dispatch_categories['Green'].keys():
				dispatch_categories['Red'][seller_id] = {'Offer': seller_offer, 'Issues': {'No Buyer': 'No Buyer'}}	
