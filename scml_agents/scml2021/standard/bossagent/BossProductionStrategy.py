...
from .helper import (get_contract_buyer_sellers, sort_negotiators_by_delivery, 
					get_unscheduled_total_pseudo_quantity, format_schedule, is_schedule_available, calculate_contract_quantity)
import copy
import numpy as np
...
class BossProductionStrategy:
	...
	def on_contracts_finalized(self, signed, cancelled, rejectors):
		...
		dispatch_categories, dispatch_partners = self.dispatch_module.run_dispatch(sorted_buyers, 
																				   sorted_sellers, 
																				   formatted_awi_schedule)		
		...
		for buyer_contract_id, seller_contract_details in dispatch_partners.items():
			for seller_contract_id, schedule in seller_contract_details.items():
				if seller_contract_id != 'KEEP': # If seller is keep, we dont need to schedule anything.
				...
				if seller_contract_id not in self.scheduled_buyer_contracts[buyer_contract_id].keys(): 	
					self.scheduled_buyer_contracts[buyer_contract_id][seller_contract_id] = (seller_offer, schedule)
					self.scheduled_seller_contracts[seller_contract_id][buyer_contract_id] = (buyer_offer, schedule)
				else:
					current_offer = self.scheduled_buyer_contracts[buyer_contract_id][seller_contract_id][0]
					current_schedule = copy.deepcopy(self.scheduled_buyer_contracts[buyer_contract_id][seller_contract_id][1])
					...
					self.scheduled_buyer_contracts[buyer_contract_id][seller_contract_id] = (seller_offer, updated_schedule)
					self.scheduled_seller_contracts[seller_contract_id][buyer_contract_id] = (buyer_offer, updated_schedule)	
		...
		for yellow_contract_id, offer_content in dispatch_categories['Yellow'].items():
			...
			surplus_offer = (offer_content['Surplus'], offer_content['Offer'][1], offer_content['Offer'][2])
		...
		for red_contract_id, offer_content in dispatch_categories['Red'].items():
			if red_contract_id in list(to_sort_buyers.keys()):
				self.pseudo_buyers[red_contract_id] = copy.deepcopy(offer_content['Offer'])
			elif red_contract_id in list(to_sort_sellers.keys()):
				offer_delivery = max(self.current_step + 1, offer_content['Offer'][1])
				offer = (offer_content['Offer'][0], offer_delivery, offer_content['Offer'][2])
		...
		for signed_contract in signed:
			...
			if signed_contract.mechanism_id is not None:
				self.nego_stats.set_negotiation_bid_sign(self.current_step, agent_id, signed_contract.mechanism_id, agent_sign, opp_sign)
		...
		for contract_index, cancelled_contract in enumerate(cancelled):
			...
			if cancelled_contract.mechanism_id is not None:
				self.nego_stats.set_negotiation_bid_sign(self.current_step, agent_id, cancelled_contract.mechanism_id, agent_sign, opp_sign)	

			if agent_id in rejectors[contract_index] and agent_id in self.reject_acceptance_rate.keys() and agent_sign == 1:
				if self.current_step in self.reject_acceptance_rate[agent_id]['Reject'].keys():
					self.reject_acceptance_rate[agent_id]['Reject'][self.current_step] += cancelled_contract.agreement['quantity']	
				else:
					self.reject_acceptance_rate[agent_id]['Reject'][self.current_step] = cancelled_contract.agreement['quantity']
