...
# Custom imports.
from .helper import sort_buyers_by_price, sort_sellers_by_price, sort_negotiators_by_delivery
...
class SyncController(SAOSyncController):
	...
	def set_limitations(self, current_step, max_number_of_steps, formatted_schedule, inventory_output_product,
						seller_price_upper_bound, buyer_price_lower_bound, consumer_agent_negotiable_count, supplier_agent_negotiable_count):
		'''
		...
		self.accepted_buyer_list = defaultdict()
		self.accepted_seller_list = defaultdict()
		...
		self.consumer_agent_negotiable_count = consumer_agent_negotiable_count
		self.supplier_agent_negotiable_count = supplier_agent_negotiable_count
	...
	def first_offer(self, negotiator_id: str) -> "Outcome":
		...
		if partner_id in self.my_suppliers: # If current proposer agent is seller.
			self.nego_history[negotiator.ami.id][0] = {'agentID': self.__parent.id, 'q': int(issues[QUANTITY].max_value), 't': int(issues[TIME].min_value), 'p': int(issues[UNIT_PRICE].min_value + 1)}
			return (issues[QUANTITY].max_value, issues[TIME].min_value, issues[UNIT_PRICE].min_value + 1)
		elif partner_id in self.my_consumers: # If current proposer agent is buyer.	
			self.nego_history[negotiator.ami.id][0] = {'agentID': self.__parent.id, 'q': int(issues[QUANTITY].max_value), 't': int(issues[TIME].min_value), 'p': int(issues[UNIT_PRICE].max_value - 1)}
			return (issues[QUANTITY].max_value, issues[TIME].max_value, issues[UNIT_PRICE].max_value - 1)
		else:
			return None	
	...
	def counter_all(
		...
		for negotiator_id, state in states.items():
			partner_id = self.partner_agent_ids(negotiator_id)[0]
			negotiator, _ = self.negotiators.get(negotiator_id, (None, None))
			self.nego_history[negotiator.ami.id][2 * (states[negotiator_id].step) - 1] = {'agentID': partner_id, 'q': int(offers[negotiator_id][QUANTITY]), 't': int(offers[negotiator_id][TIME]), 'p': int(offers[negotiator_id][UNIT_PRICE])}
			if partner_id in self.my_suppliers: # If current proposer agent is seller.
		...
		sorted_buyer_list = sort_buyers_by_price(buyers) # E.g. [(('agent1', (10, 20, 5), ('agent2', (5, 10, 3))]
		sorted_seller_list = sort_sellers_by_price(sellers) # E.g. [(('agent1', (10, 20, 5), ('agent2', (5, 10, 3))]
		# Sort pseudo buyer & sellers by delivery time.
		sorted_pseudo_buyers = sort_negotiators_by_delivery({**self.__parent.pseudo_buyers, **self.accepted_buyer_list})
		sorted_pseudo_sellers = sort_negotiators_by_delivery({**self.__parent.pseudo_sellers, **self.accepted_seller_list})
		# Add & give priority to the accepted buyer & sellers, since we want to resolve them first.
		complete_buyer_list = sorted_pseudo_buyers + sorted_buyer_list 
		complete_seller_list = sorted_pseudo_sellers + sorted_seller_list
		# Get dispatch result from the module.
		dispatch_result_categories, _ = self.__parent.dispatch_module.run_dispatch(complete_buyer_list, complete_seller_list, self.formatted_schedule)						   
		...
		responses = self.set_green_category_responses(responses, states, green_negotiators, buyer_ids, seller_ids)
		responses = self.set_yellow_category_responses(responses, states, yellow_negotiators, buyer_ids, seller_ids)
		responses = self.set_red_category_responses(responses, states, red_negotiators, buyer_ids, seller_ids)
	...
	def on_negotiation_end(self, negotiator_id: str, state: "MechanismState") -> None:
		...
		negotiator, _ = self.negotiators.get(negotiator_id, (None, None))
		if state.agreement:
			self.nego_history[negotiator.ami.id]['Acceptance'] = {'q': int(state.current_offer[QUANTITY]), 't': int(state.current_offer[TIME]), 'p': int(state.current_offer[UNIT_PRICE]), 'accept': 1, 'agent_sign': 0, 'opponent_sign': 0}
			if partner_id in self.my_consumers:
				self.accepted_buyer_list[negotiator_id] = state.current_offer
			...
			self.__parent.reject_acceptance_rate[partner_id]['Acceptance'] += state.current_offer[QUANTITY]							
		else:
		...
		self.__parent.nego_stats.set_negotiation_bid_history(self.current_step, partner_id, negotiator.ami.id, mechanism_nego_history)			

	# =========================================
	#       Dispatch Category Responses
	# =========================================	

	def set_green_category_responses(self, before_responses, states, green_negotiators, buyer_ids, seller_ids):
		'''
			Set responses for GREEN negotiation categories.
			NOT UPDATED WITH MULTIPLE RED ISSUES.
		'''	
		pseudo_ids = list(self.__parent.pseudo_buyers.keys()) + list(self.__parent.pseudo_sellers.keys())
		...
		for negotiator_id, offer_content in green_negotiators.items():
			...
			if negotiator_id in buyer_ids and negotiator_id not in (pseudo_ids + self.accepted_negotiator_ids) and\
				(states[negotiator_id].step >= 19 or self.consumer_agent_negotiable_count <= 1):
				responses[negotiator_id] = SAOResponse(ResponseType.ACCEPT_OFFER, None)
			elif negotiator_id in seller_ids and negotiator_id not in (pseudo_ids + self.accepted_negotiator_ids) and\
				(states[negotiator_id].step >= 19 or self.supplier_agent_negotiable_count <= 1):
				responses[negotiator_id] = SAOResponse(ResponseType.ACCEPT_OFFER, None)			
				...
				if negotiator_id not in (pseudo_ids + self.accepted_negotiator_ids):
					...
					if negotiator_id in buyer_ids:
						...
						predicted_p, _, _ = self.__parent.business_analytics.pred_issues(self.partner_agent_ids(negotiator_id)[0])
						if self.__parent.current_step > 15 and predicted_p != -1:
							predicted_p = max(issues[UNIT_PRICE].min_value, predicted_p)
							offer_price = self.boulware_proportion_tactic((issues[UNIT_PRICE].max_value - 1), predicted_p, relative_time)	
						else:
							offer_price = self.boulware_proportion_tactic((issues[UNIT_PRICE].max_value - 1), max(self.buyer_prices[negotiator_id]), relative_time)
					# Check if negotiator is seller.	
						...
						predicted_p, _, _ = self.__parent.business_analytics.pred_issues(self.partner_agent_ids(negotiator_id)[0])
						if self.__parent.current_step > 15 and predicted_p != -1:
							predicted_p = min(issues[UNIT_PRICE].max_value, predicted_p)
							offer_price = self.boulware_inverse_proportion_tactic(predicted_p, (issues[UNIT_PRICE].min_value + 1), relative_time)
						else:	
							offer_price = self.boulware_inverse_proportion_tactic(min(self.seller_prices[negotiator_id]), (issues[UNIT_PRICE].min_value + 1), relative_time)
					# Negotiator should be either buyer or seller, end negotiation otherwise.		
					else: 
						# End the negotiation.
						responses[negotiator_id] = SAOResponse(ResponseType.END_NEGOTIATION, None)			
					# Check whether generated offer is valid or not.
					if not self.is_valid(negotiator_id, (offer[QUANTITY], offer[TIME], offer_price)):
						# End the negotiation.								
						responses[negotiator_id] = SAOResponse(ResponseType.END_NEGOTIATION, None)
					else:	
						if offer_price == offer[UNIT_PRICE]:
							...
							negotiator, _ = self.negotiators.get(negotiator_id, (None, None))
							self.nego_history[negotiator.ami.id][2* states[negotiator_id].step] = {'agentID': self.__parent.id, 'q': int(offer[QUANTITY]), 't': int(offer[TIME]), 'p': int(offer_price)}
							# Add our offer to responses with correspoding negotiator id.									
							responses[negotiator_id] = SAOResponse(ResponseType.REJECT_OFFER, (offer[QUANTITY], offer[TIME], offer_price))
		return responses
						

	def set_yellow_category_responses(self, before_responses, states, yellow_negotiators, buyer_ids, seller_ids):
		'''
		...
		for negotiator_id, offer_content in yellow_negotiators.items():
			# Check whether this negotiator's negotiation is accepted or not, we do not send request to finished nego.
			if negotiator_id not in (pseudo_ids + self.accepted_negotiator_ids) and offer_content["Surplus"] <= 20 and states[negotiator_id].step >= 19:
				responses[negotiator_id] = SAOResponse(ResponseType.ACCEPT_OFFER, None)	
			elif negotiator_id not in (pseudo_ids + self.accepted_negotiator_ids):	
				# Check whether this negotiator's negotiation is accepted or not, we do not send request to finished nego.
				if negotiator_id not in (pseudo_ids + self.accepted_negotiator_ids):
					...
					if negotiator_id in seller_ids:
						...
						if offer_content['Target']:
							offer_q = min(offer_content['Target'], issues[QUANTITY].max_value)
						else:
							offer_q = max(offer[QUANTITY] - offer_content['Surplus'], issues[QUANTITY].min_value)	
						...
						if self.__parent.current_step > 15 and predicted_p != -1:
							...
							offer_price = self.boulware_inverse_proportion_tactic(min(self.seller_prices[negotiator_id]), (issues[UNIT_PRICE].min_value + 1), relative_time)							
						# Check whether generated offer is valid or not.
						if not self.is_valid(negotiator_id, (offer_q, offer[TIME], offer_price)):
							# End the negotiation.					
							responses[negotiator_id] = SAOResponse(ResponseType.END_NEGOTIATION, None)	
						else:	
							negotiator, _ = self.negotiators.get(negotiator_id, (None, None))
							self.nego_history[negotiator.ami.id][2 * states[negotiator_id].step] = {'agentID': self.__parent.id, 'q': int(offer_q), 't': int(offer[TIME]), 'p': int(offer_price)}							
							# Add our offer to responses with correspoding negotiator id.	
							responses[negotiator_id] = SAOResponse(ResponseType.REJECT_OFFER, (offer_q, offer[TIME], offer_price))
					else:
						# End negotiation for now.
						responses[negotiator_id] = SAOResponse(ResponseType.END_NEGOTIATION, None)
		return responses	

	def set_red_category_responses(self, before_responses, states, red_negotiators, buyer_ids, seller_ids):
		'''
			...
			if negotiator_id not in (pseudo_ids + self.accepted_negotiator_ids):
				...
				if negotiator_id in buyer_ids:				
					...
					if self.__parent.current_step > 15 and predicted_p != -1:
						...
						offer_price = self.boulware_proportion_tactic((issues[UNIT_PRICE].max_value - 1), self.buyer_price_lower_bound, relative_time)
					...
					for red_issue, red_issue_target in red_issues.items():
						...
						if 'No Seller' in red_issues.keys():
							responses[negotiator_id] = SAOResponse(ResponseType.REJECT_OFFER, (issues[QUANTITY].min_value, issues[TIME].max_value, issues[UNIT_PRICE].max_value))
							negotiator, _ = self.negotiators.get(negotiator_id, (None, None))
							self.nego_history[negotiator.ami.id][2 * states[negotiator_id].step] = {'agentID': self.__parent.id, 'q': int(issues[QUANTITY].min_value), 't': int(issues[TIME].max_value), 'p': int(issues[UNIT_PRICE].max_value)}
							break
						...
						if red_issue == 'Price':
							...
							# 	buyer_lower_price = max(red_issue_target, self.buyer_price_lower_bound)
							# else:
							# 	buyer_lower_price = self.buyer_price_lower_bound	
							# offer_price = self.boulware_proportion_tactic(issues[UNIT_PRICE].max_value, buyer_lower_price, relative_time)
						elif red_issue == 'Volume':
							buyer_upper_quantity = min(red_issue_target, issues[QUANTITY].max_value)
							offer_quantity = self.boulware_proportion_tactic(buyer_upper_quantity, offer[QUANTITY], relative_time)
						elif red_issue == 'Delivery':
							if red_issue_target <= issues[TIME].max_value and red_issue_target >= issues[TIME].min_value:
								buyer_lower_time = red_issue_target
								offer_time = self.boulware_proportion_tactic(issues[TIME].max_value, buyer_lower_time, relative_time)
							else:
				...
				# Check if the red negotiator is a seller.
					...
					# Get initial offer values from the negotiation tactic.
					offer_quantity = self.boulware_proportion_tactic(issues[QUANTITY].max_value, offer[QUANTITY], relative_time)
					offer_time = self.boulware_inverse_proportion_tactic(offer[TIME], issues[TIME].min_value, relative_time)
					predicted_p, _, _ = self.__parent.business_analytics.pred_issues(self.partner_agent_ids(negotiator_id)[0])
					if self.__parent.current_step > 15 and predicted_p != -1:
						predicted_p = min(issues[UNIT_PRICE].max_value, predicted_p)
						offer_price = self.boulware_inverse_proportion_tactic(predicted_p, (issues[UNIT_PRICE].min_value + 1), relative_time)
					else:	
						offer_price = self.boulware_inverse_proportion_tactic(self.seller_price_upper_bound, max(issues[UNIT_PRICE].min_value + 1, 1), relative_time)
					# Iterate over red issues, and fix them with target values.
					for red_issue, red_issue_target in red_issues.items():
						# If there is no buyer.
						if 'No Buyer' in red_issues.items():
							responses[negotiator_id] = SAOResponse(ResponseType.REJECT_OFFER, (issues[QUANTITY].min_value, issues[TIME].min_value, issues[UNIT_PRICE].min_value))
							negotiator, _ = self.negotiators.get(negotiator_id, (None, None))
							self.nego_history[negotiator.ami.id][2 * states[negotiator_id].step] = {'agentID': self.__parent.id, 'q': int(issues[QUANTITY].min_value), 't': int(issues[TIME].min_value), 'p': int(issues[UNIT_PRICE].min_value)}
							break	
						# Check the red issue and update the target value and check negotiation boundries.
						if red_issue == 'Price':
							continue
							# if red_issue_target >= issues[UNIT_PRICE].min_value:
							# 	seller_upper_price = min(red_issue_target, self.seller_price_upper_bound)
							# else:
							# 	seller_upper_price = self.seller_price_upper_bound	
							# offer_price = self.boulware_proportion_tactic(seller_upper_price, issues[UNIT_PRICE].min_value, relative_time)
						elif red_issue == 'Volume':
							seller_upper_quantity = min(red_issue_target, issues[QUANTITY].max_value)
							offer_quantity = self.boulware_proportion_tactic(seller_upper_quantity, offer[QUANTITY], relative_time)
						elif red_issue == 'Delivery':
							if red_issue_target <= issues[TIME].max_value and red_issue_target >= issues[TIME].min_value:
								seller_upper_time = red_issue_target
								offer_time = self.boulware_inverse_proportion_tactic(seller_upper_time, issues[TIME].min_value, relative_time)
							else:
								offer_time = red_issue_target
						else:
							responses[negotiator_id] = SAOResponse(ResponseType.REJECT_OFFER, (issues[QUANTITY].min_value, issues[TIME].min_value, issues[UNIT_PRICE].min_value))
				...
				if negotiator_id not in responses.keys():
					...
					if not self.is_valid(negotiator_id, (offer_quantity, offer_time, offer_price)):
						# We are ending negotiation, since it is not valid (it should be somehow).								
						responses[negotiator_id] = SAOResponse(ResponseType.END_NEGOTIATION, None)	
					else:					
						negotiator, _ = self.negotiators.get(negotiator_id, (None, None))
						self.nego_history[negotiator.ami.id][2 * states[negotiator_id].step] = {'agentID': self.__parent.id, 'q': int(offer_quantity), 't': int(offer_time), 'p': int(offer_price)}									
						# Add our offer to responses with correspoding negotiator id.														
						responses[negotiator_id] = SAOResponse(ResponseType.REJECT_OFFER, (offer_quantity, offer_time, offer_price))	
	...
	def boulware_proportion_tactic(self, upper_bound, lower_bound, current_time):
		...
		normalized_target = ((1 - current_time) ** 2) * P0 + 2 * (1 - current_time) * current_time * P1 + (current_time ** 2) * P2
		target = math.floor(normalized_target * upper_bound)

		return int(target)

	def boulware_inverse_proportion_tactic(self, upper_bound, lower_bound, current_time):
		'''
