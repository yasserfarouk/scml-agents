...
def buyer_closest_available_delivery(schedule, amount, start_date, end_date):
	...
	for i in range(start_date, end_date):
		...
		if possible_production_amount > 0:
			if amount - possible_production_amount <= 0:
				return (i + 1) # Since production completes at i. We can deliver it closest to (i + 1).
			else:
...
def is_balance_available(balance, production_cost, seller_proposed_min_quantity, seller_proposed_min_price):
		''' Check whether our balance is enough in best case scenario or not. If not, dont bother to negotiate with this agent. '''
		
		if balance >= ((seller_proposed_min_quantity + production_cost) * seller_proposed_min_price):
			return True
		else:
			return False	
...
def sort_buyers_by_price(buyers):
	...
	if len(list(buyers.keys())) > 0:
		buyers_sorted = list(buyers.items()) # Convert to tuple. [(('agent', (1, 3, 5)) ('agent2', (3, 5, 8)))]
		buyers_sorted.sort(key=lambda x:x[1][2], reverse=True) # Sort tuple by prices. (desceding)
...
def sort_sellers_by_price(sellers):
	...
	if len(list(sellers.keys())) > 0:
		sellers_sorted = list(sellers.items()) # Convert to tuple. [(('agent', (1, 3, 5)) ('agent2', (3, 5, 8)))]
		sellers_sorted.sort(key=lambda x:x[1][2], reverse=False) # Sort tuple by prices. (ascending)
...
def sort_negotiators_by_delivery(negotiators):
	...
	if len(list(negotiators.keys())) > 0:
		negotiators_sorted = list(negotiators.items()) # Convert to tuple. [(('agent', (1, 3, 5)) ('agent2', (3, 5, 8)))]
		negotiators_sorted.sort(key=lambda x:x[1][1], reverse=False) # Sort tuple by delivery time. (ascending)
...
def sort_negotiators_by_descending_delivery(negotiators):
	...
	if len(list(negotiators.keys())) > 0:
		negotiators_sorted = list(negotiators.items()) # Convert to tuple. [(('agent', (1, 3, 5)) ('agent2', (3, 5, 8)))]
		negotiators_sorted.sort(key=lambda x:x[1][1], reverse=True) # Sort tuple by delivery time. (descending)
		return negotiators_sorted
	else:
		return []    
...
def get_contract_buyer_sellers(our_agent_id, contracts):
	...
	for contract in contracts:
		...
		tuple_agreed_offer = (agreed_offer["quantity"], agreed_offer["time"], agreed_offer["unit_price"])
...
def calculate_current_keep_amount(current_step, scheduled_buyer_contracts, current_output_in_inventory):
	'''
	...
	for buyer_contract_id, seller_contract_details in scheduled_buyer_contracts.items():
		...
		if buyer_contract_id != 'KEEP':
			for seller_contract_id, (seller_contract, schedule) in seller_contract_details.items():  
...
def calculate_total_keep_amount(current_step, scheduled_buyer_contracts, current_output_in_inventory):
	'''
	...
	'''
	current_keep_amount = calculate_current_keep_amount(current_step, scheduled_buyer_contracts, current_output_in_inventory)
...
def split(sorted_contracts, nego_quota):
	...
	return (copy_sorted_c[i*k+min(i, m):(i+1)*k+min(i+1, m)] for i in range(nego_quota))	

def get_pseudo_seller_contracts_with_quota(formatted_schedule, seller_pseudo_contracts, nego_quota, current_step, max_buyer_delivery_day):
	'''
	...
	for contract_id, offer in seller_pseudo_contracts.items():
		if buyer_closest_available_delivery(formatted_schedule, offer[0], offer[1], max_buyer_delivery_day) > current_step + 1:
			filtered_pseudo_contracts[contract_id] = offer


	sorted_contracts = [c[1] for c in sort_negotiators_by_delivery(filtered_pseudo_contracts)]
...
def get_pseudo_buyer_contracts_with_quota(formatted_schedule, buyer_pseudo_contracts, nego_quota):
	'''
		Splits pseudo contracts into nego_quota pieces, gets the lowest delivery day for that chunk (seller) or highest d.t. for that chunk (buyer).
		Returns list of tuples with (t, q) 
		e.g. [(3, 10), (4, 8)]
	...
	for contract_id, offer in buyer_pseudo_contracts.items():
		seller_delivery_upper = seller_closest_available_delivery(formatted_schedule, offer[0], offer[1])
...
def get_agent_reject_rate(agent_id, reject_acceptance_rate, current_step):
	...
	if (reject_acceptance_rate[agent_id]['Acceptance'] == 0) or current_step == 0:
		...
		for reject_c, (reject_day, reject_quantity) in enumerate(reject_acceptance_rate[agent_id]['Reject'].items()):
			# Get rejectance rate of buyer id.
			agent_reject_rate = reject_quantity / (reject_acceptance_rate[agent_id]['Acceptance'])
			reject_coefficient = (current_step - reject_day) / (current_step)
			total_agent_reject_rate += (reject_c + 1) * agent_reject_rate * (1 - reject_coefficient)
...
def get_negotiable_agent_rate(agents, reject_acceptance_rate, current_step, bankrupt_agents):
	negotiable_agent_count = 0
	if len(agents) != 0:
		for agent in agents:
			if (0 <= get_agent_reject_rate(agent, reject_acceptance_rate, current_step) <= 0.75) and (agent not in bankrupt_agents):
...
if __name__ == '__main__':
	pseudo_contracts = {'a': (10, 20, 15), 'b': (5, 7, 9), 'c': (8, 15, 21), 'd': (7, 6, 3), 'e': (4, 8, 9)}
