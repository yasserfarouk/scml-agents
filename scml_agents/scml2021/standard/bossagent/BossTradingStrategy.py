...
from .helper import (calculate_current_keep_amount, sort_buyers_by_price, 
					sort_sellers_by_price, 
					get_contract_buyer_sellers, 
					sort_negotiators_by_delivery,
					get_unscheduled_total_pseudo_quantity,
					calculate_total_keep_amount, schedule_dispatch_production,
					format_schedule, is_schedule_available, sort_negotiators_by_delivery, sort_negotiators_by_descending_delivery)
...
class BossTradingStrategy:
	...
	def sign_all_contracts(self, contracts: List[Contract]) -> List[Optional[str]]:
		...
		dispatch_categories, dispatch_partners = self.dispatch_module.run_dispatch(to_dispatch_buyers, to_dispatch_sellers, self.formatted_schedule)
		...
		if self.awi_caller.is_first_level():
			...
			awi_schedule = self.awi.available_for_production(repeats=1, step=(self.current_step + 1, self.max_number_of_steps - 1), line=-1, override=False)[0]
			formatted_temp_schedule = format_schedule(awi_schedule, self.max_number_of_steps)	
			# Get green agent ids.
				...
				for seller_contract_id, schedule in seller_contract_details.items():
					...
					if seller_contract_id != 'KEEP':
						for schedule_step, quantity_to_produce in schedule.items():
							formatted_temp_schedule, _ = schedule_dispatch_production(formatted_temp_schedule, quantity_to_produce, schedule_step, schedule_step + 1)
			...
			for (contract_id, offer) in delivery_sorted_sellers:
				...
				is_available, _ = is_schedule_available(formatted_temp_schedule, offer[0], offer[1], self.max_buyer_delivery_day)
				# Get contract index.
				...
				if is_available and (contract_id in red_contract_ids) and (agent_id == 'SELLER'):		
					# Check if this contract is feasible.
					if (contract.agreement['quantity'] + current_signed_amount + get_unscheduled_total_pseudo_quantity(self.pseudo_sellers)) + self.current_keep_amount <= self.max_seller_quantity:
						formatted_temp_schedule, _ = schedule_dispatch_production(formatted_temp_schedule, offer[0], offer[1], self.max_buyer_delivery_day)
			...
			for (contract_id, offer) in delivery_sorted_buyers:
				# Check if schedule is available or not.
				is_available, _ = is_schedule_available(formatted_temp_schedule, offer[0], self.min_seller_delivery_day, offer[1])
				# Get contract index.
				contract_index = contract_ids.index(contract_id)
				contract = contracts[contract_index]
				contract_partners = copy.deepcopy(contract.partners)
				...
				agent_id = contract_partners[0]
				# Check availability and sign feasible red buyer contracts.
				if is_available and (contract_id in red_contract_ids) and (agent_id == 'BUYER'):		
					# Check if this contract is feasible.
					if (contract.agreement['quantity'] + get_unscheduled_total_pseudo_quantity(self.pseudo_buyers) + current_signed_amount) <= self.max_buyer_quantity and\
						contract.agreement['time'] >= self.min_buyer_delivery_day:
						formatted_temp_schedule, _ = schedule_dispatch_production(formatted_temp_schedule, offer[0], self.min_seller_delivery_day, offer[1])
						current_signed_amount += contract.agreement['quantity']
						signs[contract_index] = self.id 												
		else:
	...
	def on_contract_executed(self, contract: Contract) -> None:
		...
		if contract.id in self.pseudo_sellers.keys():
			# Get the schedule.
			awi_schedule = self.awi.available_for_production(repeats=1, step=(self.current_step, self.max_number_of_steps), line=-1, override=False)[0]
			formatted_awi_schedule = format_schedule(awi_schedule, self.max_number_of_steps)
			formatted_awi_schedule[self.current_step] = list(self.awi.state.commands[self.current_step]).count(-1)

			offer = self.pseudo_sellers[contract.id]
			# Check if schedule is available or not.
			is_available, schedule = is_schedule_available(formatted_awi_schedule, offer[0], offer[1], self.max_buyer_delivery_day)
			...
			if is_available:
				for schedule_step, quantity_to_produce in schedule.items():
					if schedule_step != self.current_step:
						...
						temp_commands = copy.deepcopy(self.awi.state.commands[self.current_step])		
				...
				if contract.id not in self.scheduled_buyer_contracts['KEEP'].keys(): 
					# FIX THIS PLEASE.	
					self.scheduled_buyer_contracts['KEEP'][contract.id] = (offer, schedule)
					self.scheduled_seller_contracts[contract.id]['KEEP'] = ((offer[0], self.max_buyer_delivery_day, self.awi_caller.get_output_catalog_price()), schedule)
				else:
					current_offer = self.scheduled_buyer_contracts['KEEP'][contract.id][0]
					updated_offer = (current_offer[0] + offer[0], current_offer[1], current_offer[2])
					current_schedule = copy.deepcopy(self.scheduled_buyer_contracts['KEEP'][contract.id][1])
					combined_steps = current_schedule.keys() | schedule.keys()
					updated_schedule = {step: current_schedule.get(step, 0) + schedule.get(step, 0) for step in combined_steps}
					self.scheduled_buyer_contracts['KEEP'][contract.id] = (updated_offer, updated_schedule)
					self.scheduled_seller_contracts[contract.id]['KEEP'] = ((offer[0], self.max_buyer_delivery_day, self.awi_caller.get_output_catalog_price()), updated_schedule)
		...
		if executed_contract_id in self.scheduled_buyer_contracts.keys():
			...
			for seller_contract_id, (seller_contract, schedule) in self.scheduled_buyer_contracts[executed_contract_id].items():
				# We pop executed buyer contract from the sellers.
				self.scheduled_seller_contracts[seller_contract_id].pop(executed_contract_id)
	...
	def on_agent_bankrupt(self,
		...
		keep_amount = calculate_current_keep_amount(self.current_step, self.scheduled_buyer_contracts, self.awi_caller.get_inventory_output_prod_quantity())
			...
			if agent in self.my_suppliers:
				...
				for bankrupt_contract in contracts:
					...
					if bankrupt_contract.id in self.scheduled_seller_contracts.keys():
						...
						for buyer_contract_id, (buyer_contract, schedule) in self.scheduled_seller_contracts[bankrupt_contract.id].items():
							...
							self.scheduled_buyer_contracts[buyer_contract_id].pop(bankrupt_contract.id)	
							...
							if keep_amount >= cancelled_quantity:
								...
								if 'KEEP' not in self.scheduled_buyer_contracts[buyer_contract_id].keys():
									self.scheduled_buyer_contracts[buyer_contract_id]['KEEP'] = ((cancelled_quantity, -1, self.seller_price_upper_bound), schedule)
									self.scheduled_seller_contracts['KEEP'][buyer_contract_id] = (buyer_contract, schedule)
								else:
									current_offer = self.scheduled_buyer_contracts[buyer_contract_id]['KEEP'][0]
									updated_offer = (current_offer[0] + cancelled_quantity, current_offer[1], current_offer[2])
									schedule = {-1: current_offer[0] + cancelled_quantity}
									self.scheduled_buyer_contracts[buyer_contract_id]['KEEP'] = (updated_offer, schedule)
									self.scheduled_seller_contracts['KEEP'][buyer_contract_id] = (buyer_contract, schedule)
							# If keep is not enough.		
								...
								if keep_amount > 0:
									...
									# Check whether we have keep in buyer schedule before.
									if 'KEEP' not in self.scheduled_buyer_contracts[buyer_contract_id].keys():
										self.scheduled_buyer_contracts[buyer_contract_id]['KEEP'] = ((keep_amount, -1, self.seller_price_upper_bound), schedule)
										self.scheduled_seller_contracts['KEEP'][buyer_contract_id] = (buyer_contract, schedule)
									else:
										current_offer = self.scheduled_buyer_contracts[buyer_contract_id]['KEEP'][0]
										updated_offer = (current_offer[0] + keep_amount, current_offer[1], current_offer[2])
										updated_schedule = {-1: current_offer[0] + keep_amount}
										self.scheduled_buyer_contracts[buyer_contract_id]['KEEP'] = (updated_offer, updated_schedule)
										self.scheduled_seller_contracts['KEEP'][buyer_contract_id] = (buyer_contract, updated_schedule)
									remaining_amount = cancelled_quantity - keep_amount
								else:
									remaining_amount = cancelled_quantity
								# Add this buyer contract as pseudo.
								self.pseudo_buyers[buyer_contract_id] = (remaining_amount, buyer_contract[1], buyer_contract[2])
				...
				for bankrupt_contract in contracts:
					...
					if bankrupt_contract.id in self.scheduled_buyer_contracts.keys():
						# Check bankrupted agent's contracts.
						for seller_contract_id, (seller_contract, schedule) in self.scheduled_buyer_contracts[bankrupt_contract.id].items():
							...
							if seller_contract_id != 'KEEP':
								...
								if seller_contract_id not in self.pseudo_sellers and total_cancelled_q > 0:
									self.pseudo_sellers[seller_contract_id] = (total_cancelled_q, seller_contract[1], seller_contract[2])
								elif seller_contract_id in self.pseudo_sellers and total_cancelled_q > 0:
									current_pseudo_offer = self.pseudo_sellers[seller_contract_id]
									updated_pseudo_offer = (current_pseudo_offer[0] + total_cancelled_q, current_pseudo_offer[1], current_pseudo_offer[2])	
									self.pseudo_sellers[seller_contract_id] = updated_pseudo_offer

								self.scheduled_seller_contracts[seller_contract_id].pop(bankrupt_contract.id)
