...
class PerryTheAgent(
    ...
    def step(self):
        super().step()
        self.input_cost[self.awi.current_step] = self.awi.trading_prices[self.awi.my_input_product]
        self.output_price[self.awi.current_step] = self.awi.trading_prices[self.awi.my_output_product]
    ...
    def _urange(self, step, is_seller, time_range):
        ...
        if is_seller:
            ...
            if time_frame < self.awi.n_steps / 10:
                return int(price * self._min_margin + 0.2), int(self._max_margin * price + 0.5)
            else:
                return int(price * self._min_margin), int(self._max_margin * price + 0.5)
        else:
