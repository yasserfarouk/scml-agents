...
class StdAgent(
    ...
    def sign_all_contracts(self, contracts):
        # future inventory
        self.inputs[self.awi.current_step] = self.awi.current_inventory[self.awi.my_input_product]
        self.estimated_outputs[self.awi.current_step] = self.awi.current_inventory[self.awi.my_output_product]
        self.breachflg=[0,0]
        for i in range(self.awi.current_step+1, self.awi.n_steps):
            self.inputs[i] = max(self.inputs[i-1]+self.buyschedule[i-1]-self.awi.n_lines, 0)
            tmp = self.estimated_outputs[i-1] + min(self.inputs[i-1]+self.buyschedule[i-1], self.awi.n_lines) - self.sellschedule[i-1]
            if tmp<0:
                self.breachflg = [i, -tmp]
            self.estimated_outputs[i] = max(tmp, 0)

        self.outputs[self.awi.current_step] = self.awi.current_inventory[self.awi.my_input_product]
        inputs = self.awi.current_inventory[self.awi.my_input_product]
        for i in range(self.awi.current_step+1, self.awi.n_steps):
            self.outputs[i] = self.estimated_outputs[i-1] + min(inputs, self.awi.n_lines) - self.sellschedule[i-1]
        ...
        for b in buy:
            ...
            if (self.inputs[t] + arrivals + sum(qb) + q <= productivity
                    and (self.balance-dbal-self.initbal)/self.initbal > -0.2 + 0.5*self.awi.current_step/self.awi.n_steps
                    and t < self.awi.n_steps*0.8
                    and self.awi.catalog_prices[self.awi.my_input_product]+self.cost<self.awi.catalog_prices[self.awi.my_output_product]
                ) or (
                    t <= self.breachflg[0]
                    and sum(qb[:t])*0.8 < self.breachflg[1]
                ):
        ...
        for s in sell:
            ...
            if (self.outputs[t]>=sum(self.sellschedule[t:])+sum(qs)+q
                or (self.awi.my_input_product==0 and self.estimated_outputs[t]>=sum(self.sellschedule[t:])+sum(qs)+q)):
    ...
    def update_price(self):
        ...
        if self.n_arrivals > 0:
            ...
            self.ip = min(self.ip*1.05, self.awi.catalog_prices[self.awi.my_input_product])
        # acceptable output price
        if self.n_shipments > 0:
            self.op *= 1.1
        elif self.awi.current_step < self.awi.n_steps * 0.5:
            self.op = max(self.op*0.95, self.awi.catalog_prices[self.awi.my_output_product])
        elif self.awi.current_step < self.awi.n_steps * 0.8:
            self.op = max(self.op*0.95, self.awi.catalog_prices[self.awi.my_input_product]+self.cost)
        else:
            self.op *=0.9
    ...
    def create_ufun(self, is_seller, issues, outcomes):
        if is_seller:
            return LinearUtilityFunction((1, 1, 10))#q,t,p?
        ...
        for c in signed:
            if self.awi.current_step <= c.agreement["time"] < self.awi.n_steps:
                if c.annotation["seller"] == self.id:
                    ...
                    self.balance -= (c.agreement["unit_price"]+self.cost)*c.agreement["quantity"]
...
class M4(StdAgent):
    ...
    def init(self):
        ...
        self.my_friends[self.id] = dict(
            level=self.awi.my_input_product, cost=self.cost
        )
