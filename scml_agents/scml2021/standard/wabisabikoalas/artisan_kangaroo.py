...
class MyTradingStrategy:
    ...
    def on_contracts_finalized(
        ...
        self.input_contracts_stack = sorted(self.input_contracts_stack, key=lambda contract: contract["time"])
        self.output_contracts_stack = sorted(self.output_contracts_stack, key=lambda contract: contract["time"])
            ...
            for output_contract in self.output_contracts_stack:
                if input_contract["quantity"] > 0:
                    if output_contract["quantity"] > 0:
                        ...
                        use_quantity = min(input_contract["quantity"], output_contract["quantity"])
                        ...
                        if self.use_available_capacity(input_contract["time"], output_contract["time"], use_quantity, False):
                            input_contract["quantity"] = input_contract["quantity"] - use_quantity
                            input_contract["correspond"].append(output_contract["id"])
                            output_contract["quantity"] = output_contract["quantity"] - use_quantity
    ...
    def get_required_quantity(self, time):
        target_contracts = list(filter(lambda contract: contract["time"] > time, self.output_contracts_stack))
        total_quantity = 0
    ...
    def get_surplus_quantity(self, time):
        ...
        target_contracts = list(filter(lambda contract: contract["time"] + 2 < time, self.input_contracts_stack))
        total_quantity = 0
        for i in range(len(target_contracts)):
            total_quantity = total_quantity + target_contracts[i]["quantity"]
    ...
    def is_available_capacity(self, input_arrival_time, output_ship_time, required_quantity, is_provisional) -> bool:
        ...
        available_capacity = list(filter(lambda capacity: (capacity["time"] > input_arrival_time+1) and (capacity["time"] < output_ship_time) and (capacity[key] > 0), self.produce_capacity))
        available_capacity = sorted(available_capacity, key=lambda capacity: capacity["time"])
    ...
    # is_provisional should be set True in sign_all_contracts()
    def use_available_capacity(self, input_arrival_time, output_ship_time, required_quantity, is_provisional) -> bool:
        if input_arrival_time >= output_ship_time:
            return False
...
class MyNegotiator(SAONegotiator):
    ...
    def respond(self, state: MechanismState, offer: "Outcome") -> "ResponseType":
        ...
        if self.is_seller:
            if self.profit_width > 0:
                if offer[2] < self.catalog_prices[self.my_output_product] - min(self.profit_width // 5, 2):
                    return ResponseType.REJECT_OFFER
            else:
                if offer[2] <= self.catalog_prices[self.my_output_product] - self.profit_width + 1:
                    return ResponseType.REJECT_OFFER
        else:
            if self.profit_width > 0:
                if offer[2] > self.catalog_prices[self.my_input_product] + min(self.profit_width // 5, 2):
                        return ResponseType.REJECT_OFFER
            else:
                if offer[2] >= self.catalog_prices[self.my_input_product] + self.profit_width - 1:
                    return ResponseType.REJECT_OFFER
    ...
    def propose(self, state: MechanismState) -> Optional["Outcome"]:
        ...
        if self.last_offer == None:
            self.last_offer = (self.target_quantity, (self.target_time[0] + self.target_time[1]) // 2, self.target_unitprice)
        ...
        if self.collude:
            if self.is_seller:
                return (self.target_quantity, (self.target_time[0] + self.target_time[1]) // 2, self.catalog_prices[self.my_output_product])
            else:
                return (self.target_quantity, (self.target_time[0] + self.target_time[1]) // 2, self.catalog_prices[self.my_input_product])
...
class ArtisanKangaroo(
    ...
    def request(self, is_sell, qvalues, uvalues, tvalues, target_quantity, target_time, target_unitprice):
    ...
    def init(self):
        ...
        self.profit_width = self.awi.catalog_prices[self.awi.my_output_product] - (self.awi.catalog_prices[self.awi.my_input_product] + self.awi.profile.costs[0][self.awi.my_input_product])
        ...
        self.input_unitprices = [
            {
                ...
                "unitprice": int(self.awi.catalog_prices[self.awi.my_input_product] * 0.8),
                "prev_success": 0
            },
            {
            ...
            },
            {
                ...
                "unitprice": int(self.awi.catalog_prices[self.awi.my_input_product] * 0.7),
                "prev_success": 0
            },
            {
            ...
            },
            {
                ...
                "unitprice": int(self.awi.catalog_prices[self.awi.my_input_product] * 0.6),
                "prev_success": 0
            },
        ]
        ...
        self.output_unitprices = [
            {
                "start_time": 0,
                "end_time": self.awi.n_steps // 5,
                "unitprice": int(self.awi.catalog_prices[self.awi.my_output_product] * 1.4),
                "prev_success": 0
            },
            {
                "start_time": (self.awi.n_steps // 5) + 1,
                "end_time": (self.awi.n_steps // 5) * 2,
                "unitprice": int(self.awi.catalog_prices[self.awi.my_output_product] * 1.3),
                "prev_success": 0
            },
            {
                "start_time": ((self.awi.n_steps // 5) * 2) + 1,
                "end_time": (self.awi.n_steps // 5) * 3,
                "unitprice": int(self.awi.catalog_prices[self.awi.my_output_product] * 1.2),
                "prev_success": 0
            },
            {
                "start_time": ((self.awi.n_steps // 5) * 3) + 1,
                "end_time": (self.awi.n_steps // 5) * 4,
                "unitprice": int(self.awi.catalog_prices[self.awi.my_output_product] * 1.1),
                "prev_success": 0
            },
            {
                "start_time": ((self.awi.n_steps // 5) * 4) + 1,
                "end_time": (self.awi.n_steps // 5) * 5,
                "unitprice": int(self.awi.catalog_prices[self.awi.my_output_product] * 1.0),
                "prev_success": 0
            },
        ]
    ...
    def step(self):
        ...
        for unitprice_obj in self.input_unitprices:
            change = max(int(self.awi.catalog_prices[self.awi.my_input_product] * 0.1), 3)
            if unitprice_obj["prev_success"] <= 0:
                unitprice_obj["unitprice"] = min(unitprice_obj["unitprice"] + (change // 3), self.awi.catalog_prices[self.awi.my_input_product] + min(self.profit_width // 5, 2))
            else:
        ...
        for unitprice_obj in self.output_unitprices:
            change = max(int(self.awi.catalog_prices[self.awi.my_output_product] * 0.1), 3)
            if unitprice_obj["prev_success"] <= 0:
                unitprice_obj["unitprice"] = max(unitprice_obj["unitprice"] - (change // 3), self.awi.catalog_prices[self.awi.my_output_product] - min(self.profit_width // 5, 2))
            else:
                unitprice_obj["unitprice"] = unitprice_obj["unitprice"] + change
            unitprice_obj["prev_success"] = 0
        ...
        if self.is_first_level:
            if self.awi.current_step + 5 < self.awi.n_steps - 1:
                ...
                for target_time in range(start_time, end_time, interval):
                    target_quantity = min(self.default_target_quantity, self.get_surplus_quantity(target_time))

                    output_unitprice = list(filter(lambda unitprice_obj: ((unitprice_obj["start_time"] <= target_time) and (target_time <= unitprice_obj["end_time"])), self.output_unitprices))
                    if len(output_unitprice) == 0:
                        target_unitprice = self.awi.catalog_prices[self.awi.my_output_product]
                    else:
                    ...
                    sell_uvalues = (int(self.awi.catalog_prices[self.awi.my_output_product] - self.profit_width), target_unitprice+1)

                    self.request(True, sell_qvalues, sell_uvalues, sell_tvalues, target_quantity, sell_tvalues, target_unitprice)

        elif self.is_last_level:
            if self.awi.current_step + 2 < self.awi.n_steps - 1:
                start_time = self.awi.current_step + 2
                end_time = self.awi.n_steps // 2 if (self.awi.current_step < 0.3 * self.awi.n_steps) else self.awi.n_steps - 1
                interval = ((end_time - start_time) // 5) + 1
                ...
                for target_time in range(start_time, end_time, interval):
                    ...
                    input_unitprice = list(filter(lambda unitprice_obj: ((unitprice_obj["start_time"] <= target_time) and (target_time <= unitprice_obj["end_time"])), self.input_unitprices))
                    if len(input_unitprice) == 0:
                        target_unitprice = self.awi.catalog_prices[self.awi.my_input_product]
                    else:
                    ...
                    buy_uvalues = (target_unitprice-1, self.awi.catalog_prices[self.awi.my_input_product]+self.profit_width)

                    self.request(False, buy_qvalues, buy_uvalues, buy_tvalues, target_quantity, buy_tvalues, target_unitprice)
    ...
    def respond_to_negotiation_request(
        ...
        try:
            ...
            if get_agent_name(self.name) in initiator:
                ...
                if is_seller:
                    output_unitprice = list(filter(lambda unitprice_obj: ((unitprice_obj["start_time"] <= middle_time) and (middle_time <= unitprice_obj["end_time"])), self.output_unitprices))[0]
                    if len(output_unitprice) == 0:
                        target_unitprice = self.awi.catalog_prices[self.awi.my_output_product]
                    else:
                        target_unitprice = iniput_unitprice[0]["unitprice"]
                else:
                    input_unitprice = list(filter(lambda unitprice_obj: ((unitprice_obj["start_time"] <= middle_time) and (middle_time <= unitprice_obj["end_time"])), self.input_unitprices))[0]
                    if len(input_unitprice) == 0:
                        target_unitprice = self.awi.catalog_prices[self.awi.my_input_product]
                    else:
                        target_unitprice = input_unitprice[0]["unitprice"]
                ...
                if self.is_first_level:
                    if is_seller:
                        target_quantity = min(self.default_target_quantity, self.get_surplus_quantity(target_time[0]))
                    else:
                        return None
                elif self.is_last_level:
                    if is_seller:
    ...
    def sign_all_contracts(self, contracts: List[Contract]) -> List[Optional[str]]:
        ...
        all_input_contracts_stack = sorted(all_input_contracts_stack, key=lambda contract: (contract["tmp"], contract["time"]))
        all_output_contracts_stack = sorted(all_output_contracts_stack, key=lambda contract: (contract["tmp"], contract["time"]))
            ...
            for all_output_contract in all_output_contracts_stack:
                ...
                if all_input_contract["quantity"] > 0:
                    if all_output_contract["quantity"] > 0:
                        use_quantity = min(all_input_contract["quantity"], all_output_contract["quantity"])
                        if self.is_available_capacity(all_input_contract["time"], all_output_contract["time"], use_quantity, True):
                            # decide how many input products in this input contract can be used to this output contract
                            all_output_contract_id = all_output_contract["id"]
                            all_input_contract["quantity"] = all_input_contract["quantity"] - use_quantity
                            all_input_contract[all_output_contract_id] = use_quantity
                            all_input_contract["prov_correspond"].append(all_output_contract["id"])
                            all_output_contract["quantity"] = all_output_contract["quantity"] - use_quantity
                            all_output_contract["prov_correspond"].append(all_input_contract)
        ...
        used_output_contracts_stack = list(filter(lambda contract: contract["quantity"] <= 0, all_output_contracts_stack))
        ...
        for output_contract in used_output_contracts_stack:
            ...
            for prov_correspond_input_contract in output_contract["prov_correspond"]:
                if prov_correspond_input_contract["index"] != None:
                    if not self.is_available_capacity(prov_correspond_input_contract["time"], output_contract["time"], prov_correspond_input_contract[output_contract_id], True):
                        is_possible = False
            ...
            if is_possible:
                for prov_correspond_input_contract in output_contract["prov_correspond"]:
                    if prov_correspond_input_contract["index"] != None:
                        if self.use_available_capacity(prov_correspond_input_contract["time"], output_contract["time"], prov_correspond_input_contract[output_contract_id], True):
                            results[prov_correspond_input_contract["index"]] = self.id
                            all_input_contracts_stack = list(filter(lambda contract: contract["id"] != prov_correspond_input_contract["id"], all_input_contracts_stack))
                            used_input_contracts_stack.append(prov_correspond_input_contract)

                if output_contract["index"] != None:
                    results[output_contract["index"]] = self.id

                all_output_contracts_stack = list(filter(lambda contract: contract["id"] != output_contract["id"], all_output_contracts_stack))

        # filter and sort all_ stacks
        all_input_contracts_stack = list(filter(lambda contract: contract["quantity"] > 0, all_input_contracts_stack))
        all_output_contracts_stack = list(filter(lambda contract: contract["quantity"] > 0, all_output_contracts_stack))
        all_input_contracts_stack = list(filter(lambda contract: contract["index"] != None, all_input_contracts_stack))
        all_output_contracts_stack = list(filter(lambda contract: contract["index"] != None, all_output_contracts_stack))
        all_input_contracts_stack = sorted(all_input_contracts_stack, key=lambda contract: (contract["unit_price"], contract["time"]))
        all_output_contracts_stack = sorted(all_output_contracts_stack, key=lambda contract: (-contract["time"], contract["unit_price"]))
        ...
        for output_contract in all_output_contracts_stack:
            ...
            if quantity <= 0:
                continue
            if step > self.awi.n_steps - 1 - self.awi.n_lines or step < self.awi.current_step:
                continue
            ...
            if self.is_first_level:
                ...
                neighbor_output_contracts = list(filter(lambda contract: ((contract["time"] >= step-self.awi.n_lines) and (contract["time"] < step+self.awi.n_lines)), self.output_contracts_stack))
                neighbor_stock_output_contracts = list(filter(lambda contract: ((contract["time"] >= step-self.awi.n_lines) and (contract["time"] < step+self.awi.n_lines)), stock_output_contracts))

                total_quantity = 0
                for neighbor_output_contract in neighbor_output_contracts:
                    total_quantity = total_quantity + neighbor_output_contract["quantity"]
                for neighbor_stock_output_contract in neighbor_stock_output_contracts:
                    total_quantity = total_quantity + neighbor_stock_output_contract["quantity"]
                ...
                if step < 0.15 * self.awi.n_steps:
                    continue
                if self.get_required_quantity(self.awi.n_steps - 1) > 3 * self.awi.n_lines:
                    continue
            else:
            ...
            if self.is_available_capacity(output_contract["time"] - self.awi.n_lines, output_contract["time"], output_contract["quantity"], True):
        ...
        for input_contract in all_input_contracts_stack:
            ...
            max_stock_count = int(0.2 * self.awi.n_steps) * self.awi.n_lines if self.profit_width > 0 else (self.awi.n_lines * 2) // 3
            if self.get_surplus_quantity(self.awi.n_steps-1) + bought_count + quantity > max_stock_count:
                continue


            stock_input_contracts.append(input_contract)

            if self.is_available_capacity(input_contract["time"], input_contract["time"] + self.awi.n_lines, input_contract["quantity"], True):
    ...
    def on_negotiation_success(
        ...
        if is_seller:
            for output_unitprice in self.output_unitprices:
                if output_unitprice["start_time"] <= contract.agreement["time"] and contract.agreement["time"] <= output_unitprice["end_time"]:
                    output_unitprice["prev_success"] = output_unitprice["prev_success"] + 1
        else:
            for input_unitprice in self.input_unitprices:
                if input_unitprice["start_time"] <= contract.agreement["time"] and contract.agreement["time"] <= input_unitprice["end_time"]:
                    input_unitprice["prev_success"] = input_unitprice["prev_success"] + 1
    ...
    def on_agent_bankrupt(
        ...
        for contract in contracts:
            ...
            if is_seller:
                affected_input_contracts = list(filter(lambda affected_contract: (contract.id in affected_contract["correspond"]), self.input_contracts_stack))
                for affected_input_contract in affected_input_contracts:
                    affected_input_contract["quantity"] += contract.agreement["quantity"]
            else:
                affected_output_contracts = list(filter(lambda affected_contract: (contract.id in affected_contract["correspond"]), self.output_contracts_stack))
                for affected_output_contract in affected_output_contracts:
                    affected_output_contract["quantity"] += contract.agreement["quantity"]
