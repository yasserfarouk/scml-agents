...
from .Bidding import SellBidding, BuyBidding
...
class Mediocre(SCML2020Agent, ProductionStrategy, TradingStrategy, NegotiationManager):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.buy_pmin = None #minumum price level defined at current step for buy trade
    ...
    def init(self):
        ...
        self.production_cost = np.max(self.awi.profile.costs[:, self.awi.my_input_product])
        ...
        self.trade_stats = pd.DataFrame(-1,
                                       columns=['sign_rate', 'agree_rate', 'hist_quantity', 'hist_utility', 'last_quantity', 'last_utility'],
                                       index=[p for p in self.partners if p not in ['SELLER', 'BUYER']])


        self.past_negotiations = pd.DataFrame(columns=['is_sell', 'step', 'nego_no', 'round', 'delivery_t', 'quantity', 'price', 'utility', 'norm_utility'])
        self.past_contracts = pd.DataFrame(columns=['cid', 'partner', 'sign_step', 'is_sell', 'q', 't', 'p', 'utility']).set_index(['partner', 'cid'])

        #agent identifier for log data
        self.identifier = "Med_"+"_".join([str(self.awi.my_input_product), str(self.prod_cap), str(self.production_cost), str(self.input_catalog_price), str(self.output_catalog_price)])

        self.STOP_PRODUCTION_LAST_N_STEP = max(1, int(self.awi.n_steps * 0.1)) if not self.is_last_level else 1
    ...
    def before_step(self):
        ...
        if self.current_step > 0:
            lst = [negotiator.received_bids for negotiator_lst in self.negotiators_engaged[self.current_step-1].values() for negotiator in negotiator_lst]
            if lst:
                all_received_bids = pd.concat(lst)
                all_received_bids['step'] = self.current_step-1
                self.past_negotiations = pd.concat([self.past_negotiations, all_received_bids])

            self.update_trade_stats()

        self.supplier_states = {partner: self.get_state(partner) for partner in self.my_suppliers if partner != "SELLER"}
        self.consumer_states = {partner: self.get_state(partner) for partner in self.my_consumers if partner != "BUYER"}

        if not self.is_first_level:
            self.eagerness_for_suppliers = self.calculate_eagerness_rates(self.supplier_states)

        if not self.is_last_level:
            self.eagerness_for_consumers = self.calculate_eagerness_rates(self.consumer_states)
    ...
    def step(self):
        if self.awi.current_step < self.awi.n_steps - self.STOP_PRODUCTION_LAST_N_STEP and len(self.my_suppliers):
    ...
    def update_trade_stats(self):
        ...
        for partner in self.partners:
            if partner in ['SELLER', 'BUYER']: #no negotiation with global players, so, trade_stats is not needed for them
                continue
            ...
            if partner in self.past_contracts.index.get_level_values(0):
                ...
                last_utility = contracts[contracts['sign_step'] == last_contract_step]['utility'].mean()
                last_quantity = contracts[contracts['sign_step'] == last_contract_step]['q'].mean()
            ...
            self.trade_stats.loc[partner, 'sign_rate'] = n_signs/n_agree if n_agree else 0
            self.trade_stats.loc[partner, 'agree_rate'] = n_agree/n_negotiations if n_negotiations else 0
    ...
    def get_state(self, partner):
        ...
        if reports:
            ...
            breach_score = 1.1**(self.current_step-report.step) * report.breach_level * report.breach_prob
            cash = report.cash
        ...
        state['cash'] = cash
        state['wealth']  = state['wealth'] = balance_trend / y.mean() if balance_trend != None else "unknown"
    ...
    def calculate_neediness_level(self):
        ...
        signed_future_inputs = self.past_contracts[(self.past_contracts.t >= self.awi.current_step) & (self.past_contracts.is_sell==False)]['q'].sum()
        signed_future_outputs = self.past_contracts[(self.past_contracts.t >= self.awi.current_step) & (self.past_contracts.is_sell)]['q'].sum()

        future_outputs = current_inputs + signed_future_inputs + current_outputs
        neediness = (future_outputs - signed_future_outputs) / future_outputs if future_outputs else 0

        self.current_neediness = round(max(-1, min(1, neediness)), 3) #between -1 and 1
    ...
    def calculate_eagerness_rates(self, partner_states):
        ...
        if self.current_step > 0:
            ...
            eagerness["nego_parameters"] = [(self.trade_stats.loc[partner, 'hist_utility'],
                                             self.trade_stats.loc[partner, 'last_utility'],
                                             self.trade_stats.loc[partner, 'sign_rate'],
                                             self.trade_stats.loc[partner, 'agree_rate']) for partner in eagerness.index]

            eagerness["nego_score"] =  eagerness.nego_parameters.apply(lambda row: round((row[0]*row[1])**0.5 * row[2] * row[3], 3) if -1 not in row else 0)

            average_score = eagerness.nego_score.mean()
            average_score = average_score if average_score > 0 else 1

            eagerness.nego_score = eagerness.nego_score.apply(lambda score: average_score if score == 0 else score)
            ...
            for partner, state in partner_states.items():
                ...
                eagerness.loc[partner, "score"] = partner_score * eagerness.loc[partner, "nego_score"]
    ...
    def initialize_negotiators(self):
        ...
        for partner in self.my_suppliers:
            ...
            negotiator = MediocreNegotiator(self.current_step, self.id, self.identifier, partner, params, to_sell=False) #buyer negotiator
        ...
        for partner in self.my_consumers:
            ...
            negotiator = MediocreNegotiator(self.current_step, self.id, self.identifier, partner, params, to_sell=True) #seller negotiator
            self.negotiators[self.current_step][partner] = negotiator
            self.negotiators_engaged[self.current_step][partner] = []
    ...
    def request_negotiations(self):
        ...
        for partner, negotiator_ in self.negotiators[self.awi.current_step].items():
            ...
            product = self.awi.my_output_product if is_sell else self.awi.my_input_product
            ...
            if negotiator.q_bounds[1] > 0:
                ...
                if offer_availability:
                    ...
                    acceptance = self.awi.request_negotiation(is_buy=not is_sell,
                                                              product=product,
                    ...
                    if acceptance:
                        ...
                        self.negotiators_engaged[self.awi.current_step][partner].append(negotiator)
    ...
    def respond_to_negotiation_request(
        ...
        if negotiator.q_bounds[1] > 0:
            ...
            negotiator.params["alpha"] = -1 # counter agenda might not be suitable for us, therefore all desirable bids are allowed to propose
            ...
            if offer_availability:
                negotiator_.nego_no += 1
                self.negotiators_engaged[self.awi.current_step][initiator].append(negotiator)
                return negotiator
    ...
    def do_allocations(self):
        ...
        n_steps_to_produce = self.awi.n_steps - self.awi.current_step - self.STOP_PRODUCTION_LAST_N_STEP
        targetQ, remained_production_capacity = 0, 0
        if n_steps_to_produce > 0:
            remained_production_capacity = max(0, n_steps_to_produce * self.awi.n_lines - input_inventory - signed_future_inputs)
            targetQ = remained_production_capacity // n_steps_to_produce

        if len(self.eagerness_for_suppliers) > 0:
            suppliers_allocations = pd.DataFrame({"eagerness":self.eagerness_for_suppliers, #used for logging
                                                  "qmin": targetQ // len(self.eagerness_for_suppliers),
                                                  "qmax": (targetQ * 4 * self.eagerness_for_suppliers).astype(int)},
                                                 index= self.eagerness_for_suppliers.index).T.to_dict()
        else: #all suppliers were bankrupted!
            suppliers_allocations = {}

        outputs_to_sell = max(0, output_inventory - signed_future_outputs)

        customers_allocations = pd.DataFrame({"eagerness": self.eagerness_for_consumers, #used for logging
                                              "quantity": outputs_to_sell,
                                              "output_inventory":output_inventory, #used for logging
                                              "signed_future_outputs":signed_future_outputs }, #used for logging
                                             index = self.eagerness_for_consumers.index).T.to_dict()
    ...
    def observed_trade_price(self, is_sell, partner=None):
        ...
        """
        if partner == None or partner not in self.past_contracts.index.get_level_values(0):
            pcs = self.past_contracts
        else:
            pcs = self.past_contracts.loc[partner]

        catalog_price = self.output_catalog_price if is_sell else self.input_catalog_price
        ...
        if pcs[pcs.is_sell==is_sell].shape[0] > 0:
            ...
            try:
                ...
                historical_weighted_p = (historical_q_p['q'] * historical_q_p['p']).sum() / (historical_q_p['q']).sum()

                last_q_p = pcs.loc[(pcs.is_sell==is_sell) & (pcs.sign_step == last_sign_step), ['q','p']]
                last_weighted_p = (last_q_p['q'] * last_q_p['p']).sum() / (last_q_p['q']).sum()
    ...
    def get_nego_params_for(self, partner, partner_state, allocation, to_sell):
        ...
        params = {}
        params['eagerness'] = self.eagerness_for_consumers[partner] if to_sell else self.eagerness_for_suppliers[partner]
        ...
        if to_sell:
            if self.is_first_level:
                 pmin = (self.production_cost + self.input_catalog_price + self.output_catalog_price) // 2
            else:
                r = 1 if self.current_neediness > 0.5 else 1.1
                pmin =  r * (self.production_cost + traded_input_price + self.output_catalog_price + self.awi.trading_prices[self.awi.my_output_product]) // 3

            pmax = max((self.output_catalog_price + pmin + traded_output_price)//2, int(1.5*pmin))
            ...
            if self.current_neediness < -0.05: # needs to buy
                ...
                d = 3.5
            pmin = (traded_input_price + self.input_catalog_price + self.awi.trading_prices[self.awi.my_input_product]) // d
            pmax =  min(int(traded_output_price - self.production_cost), min(int(self.input_catalog_price*1.25), self.input_catalog_price+10)) #min(int(self.input_catalog_price*1.25), self.input_catalog_price+10)
            if pmax <= pmin:
                pmax = int(pmin*1.5)

        impoverishment_indicator = (traded_input_price + self.production_cost) / traded_output_price - 1
        ...
        if to_sell:
            ...
            qmax = max(allocation['qmin'], allocation["qmax"]) #using max() to avoid any bug

        signed_buying_inputs = self.past_contracts[self.past_contracts.is_sell==False].q.sum() if self.past_contracts.shape[0] > 0 else 0
        signed_selling_outputs = self.past_contracts[self.past_contracts.is_sell].q.sum() if self.past_contracts.shape[0] > 0 else 0

        if self.is_last_level:
             if signed_buying_inputs > signed_selling_outputs and self.current_step / self.awi.n_steps > 0.5:
                 qmin, qmax = 0, 0

        if to_sell:
            tmin, tmax = int(self.awi.current_step+1), min(int(self.awi.current_step+3), self.awi.n_steps-1)
        else:
            tmin, tmax = int(self.awi.current_step+1), min(int(self.awi.current_step+6), self.awi.n_steps-self.STOP_PRODUCTION_LAST_N_STEP)
            if tmax < tmin:
                qmin, qmax = 0, 0

            if self.is_last_level:
                tmax = min(int(self.awi.current_step+10), self.awi.n_steps-self.STOP_PRODUCTION_LAST_N_STEP)
        ...
        if self.trade_stats.loc[partner, 'agree_rate'] > 0:
            ...
            contract_rate = (self.trade_stats.loc[partner, 'sign_rate'] * self.trade_stats.loc[partner, 'agree_rate'])**0.5
        ...
        params['reasonable_q'] = (hist_quantity * last_quantity)**0.5 if last_quantity != -1 else (qmin + qmax) // 2
    ...
    def sell_utility_func(self, p, q, t, breach_score=0):
        ...
        return q * min(p**1.1, 1.5) / (1 + delivery_delay**0.1) / (1 + breach_score**0.5)

    def buy_utility_func(self, p, q, t, breach_score=0):
        delivery_delay = t-self.awi.current_step

        return q / (max(0, p - self.buy_pmin) + 1) / (1 + delivery_delay**0.1)/ (1 + breach_score**0.5)
    ...
    def sign_all_contracts(self, contracts: List[Contract]) -> List[Optional[str]]:
        ...
        buying_contracts =  self.past_contracts[(self.past_contracts.t >= self.awi.current_step) & ( self.past_contracts.is_sell == False)]
        signed_future_inputs = buying_contracts['q'].sum()

        to_be_produced = signed_future_inputs + input_inventory
        total_production_capacity = (self.awi.n_steps - self.awi.current_step - self.STOP_PRODUCTION_LAST_N_STEP) * self.awi.n_lines - to_be_produced
        total_production_capacity = max(0, total_production_capacity)

        suppliers = pd.DataFrame([{ "sign_indx": sign_indx,
                                    "price": contract.agreement["unit_price"],
                                    "quantity":contract.agreement["quantity"],
                                    "time": contract.agreement["time"],
                                    "breach_score": self.supplier_states[contract.annotation["seller"]]['breach_score'] if contract.annotation["seller"] != "SELLER" else 0,
                                    "partner": contract.annotation["seller"]}
                                  for sign_indx, contract in enumerate(contracts) if contract.annotation["seller"] != self.id])

        consumers = pd.DataFrame([{ "sign_indx": sign_indx,
                                    "price": contract.agreement["unit_price"],
                                    "quantity":contract.agreement["quantity"],
                                    "time": contract.agreement["time"],
                                    "breach_score": self.consumer_states[contract.annotation["buyer"]]['breach_score'] if contract.annotation["buyer"] != "BUYER" else 0,
                                    "partner": contract.annotation["buyer"]}
                                  for sign_indx, contract in enumerate(contracts) if contract.annotation["seller"] == self.id])


        total_sell = self.past_contracts.loc[self.past_contracts.is_sell, 'q'].sum()
        total_buy = self.past_contracts.loc[self.past_contracts.is_sell==False, 'q'].sum()
        flow_ratio = total_sell / total_buy if total_buy else 0

        if self.awi.n_steps - self.awi.current_step  <= self.STOP_PRODUCTION_LAST_N_STEP or len(self.my_consumers)==0 or total_buy >= total_production_capacity:
            volume_to_buy = 0
        elif self.awi.current_step / self.awi.n_steps > 0.5 and flow_ratio < 0.5 and total_buy > 0:
        ...
        if suppliers.shape[0] > 0:
            ...
            selected_sup_contracts = pd.DataFrame({"sign_indx": suppliers.sign_indx,
                                   "cost": [p*q + q* self.production_cost for p, q in zip(suppliers.price, suppliers.quantity)],
                                   "quantity": suppliers.quantity,
                                   "delivery_t" : suppliers.time,
                                   "utility": [self.buy_utility_func(p, q, t, bs) for p, q, t, bs in zip(suppliers.price, suppliers.quantity, suppliers.time, suppliers.breach_score)]
                                   })

            selected_sup_contracts = selected_sup_contracts[selected_sup_contracts.delivery_t < self.awi.n_steps - self.STOP_PRODUCTION_LAST_N_STEP]
            selected_sup_contracts = selected_sup_contracts.sort_values("utility", ascending = False).T.to_dict()
        ...
        if consumers.shape[0] > 0:
            selected_cons_contracts = pd.DataFrame({"sign_indx": consumers.sign_indx,
                                                    "quantity": consumers.quantity,
                                                    "delivery_t" : consumers.time,
                                                    "utility": [self.sell_utility_func(p, q, t, bs) for p, q, t, bs in zip(consumers.price, consumers.quantity, consumers.time, consumers.breach_score)]})

            selected_cons_contracts = selected_cons_contracts.sort_values("utility", ascending = False)

            signed_sell_quantities = self.past_contracts[(self.past_contracts.t >= self.awi.current_step) & (self.past_contracts.is_sell)].groupby('t')['q'].sum()
            stepwise_future_input_inventory = np.ones(self.awi.n_steps) * input_inventory
            ...
            if buying_contracts.shape[0] > 0:
                if not self.is_first_level:
                    for (partner, cid), contract in buying_contracts.iterrows():
                        ...
                        elif self.supplier_states[partner]["cash"] > contract.q * self.awi.trading_prices[self.awi.my_input_product] * 2:
                            continue

                        else:
                            buying_contracts.loc[(partner, cid), 'q'] = int(buying_contracts.loc[(partner, cid), 'q'] * (1 - self.supplier_states[partner]["breach_score"]))

                signed_future_inputs = buying_contracts.set_index('t').groupby('t', axis=0)['q'].sum()

                for t, q in signed_future_inputs.iteritems():
                    stepwise_future_input_inventory[t:] += q

                stepwise_future_output_inventory = np.ones(self.awi.n_steps) * output_inventory

                for t in range(self.awi.current_step, self.awi.n_steps):
                    if t <= self.awi.n_steps - self.STOP_PRODUCTION_LAST_N_STEP:
                        production_amount = max(0, min(stepwise_future_input_inventory[t], self.prod_cap))
                ...
                for _, contract in selected_cons_contracts.iterrows():
                    ...
                    if int(contract['quantity']) <= stepwise_future_output_inventory[int(delivery_t)]:

                        signatures[int(contract['sign_indx'])] = self.id
                        stepwise_future_output_inventory[int(delivery_t):] -= int(contract['quantity'])
                        signeds.append((delivery_t, contract['quantity']) )

                    elif self.is_last_level and len(self.my_suppliers) and delivery_t - self.awi.current_step > 5 and int(contract['quantity']) <= self.prod_cap *  (delivery_t - self.awi.current_step) - extra_q:
                        signatures[int(contract['sign_indx'])] = self.id
    ...
    def on_contracts_finalized(
        ...
        for contract in signed:
            ...
            partner = contract.annotation["buyer"] if is_sell else contract.annotation["seller"]
            cid = contract.id
            ...
            if partner not in ['SELLER', 'BUYER']:
                ...
                if contract.signed_at != 0:
                    for negotiator in self.negotiators_engaged[int(contract.signed_at)-1][partner]:
                        if agreement in negotiator.agreement_utility:
                            utility = negotiator.agreement_utility[agreement]

                if not utility:
                    for negotiator in self.negotiators_engaged[int(contract.signed_at)][partner]:
                        if agreement in negotiator.agreement_utility:
                            utility = negotiator.agreement_utility[agreement]
        ...
        new_contracts = pd.DataFrame(contract_list, columns=['cid', 'partner', 'sign_step', 'is_sell', 'q', 't', 'p', 'utility'])
    ...
    def contract_utility(self, contract):
        ...
        utility = self.sell_utility_func(p,q,t) if contract.annotation['seller'] == self.id else self.buy_utility_func(p,q,t)
    ...
    def on_agent_bankrupt(
        ...
        try:
            if agent in self.partners:
                ...
                if contracts:
                    self.past_contracts.drop([c.id for c in contracts], level='cid', inplace=True)
        except: #bug
            pass
...
class MediocreNegotiator(SAONegotiator):
    ...
    def __init__(self, current_step, agent_id, agent_id_long, opponent, params, to_sell, **kwargs):
        ...
        self.next_bid = None
        self.received_bids = pd.DataFrame(columns=['round', 'bid', 'quantity', 'delivery_t', 'price', 'utility', 'norm_utility'])
    ...
    def init(self):
        ...
        self.valid_bounds = [self.valid_q_bounds, self.valid_t_bounds, self.valid_p_bounds]

        bidding = SellBidding if self.to_sell else BuyBidding

        self.bidding = bidding(self.current_step, self.initiator, self.eagerness, self.neediness,
                               self.params, self.valid_bounds, utility_func=self.utility_func, use_bid_func=True)
        ...
        if self.offer_availability:
            ...
            self.max_utility = self.utility_func( (self.valid_q_bounds[1], self.valid_t_bounds[0], p))

        self.preffered_delivery_t = self.valid_t_bounds[0] if self.to_sell else int(np.mean(self.valid_t_bounds))
    ...
    def respond(self, state: MechanismState, offer):
        ...
        self.received_bids.loc[len(self.received_bids)] = [state.step, offer, offer[0], offer[1], offer[2], offer_utility, norm_offer_utility]

        opponent_behavior_params = self.get_opponent_params()
        acceptance, my_bid = self.bidding.evaluate(offer, state.relative_time, opponent_behavior_params)

        if acceptance:
            if my_bid:
                my_bid_utility = self.utility_func(my_bid)
                norm_my_bid_utility = round(my_bid_utility / self.max_utility, 3)
                self.offered_bids.loc[len(self.offered_bids)] = [state.step, my_bid, my_bid[0], my_bid[1], my_bid[2], my_bid_utility, norm_my_bid_utility]
    ...
    def propose(self, state: MechanismState):
        ...
        my_bid = self.next_bid if self.next_bid else self.bidding.bid_func(state.relative_time, self.valid_t_bounds[0])
        my_bid_utility = self.utility_func(my_bid)
        norm_my_bid_utility = round(my_bid_utility / self.max_utility, 3)
        self.offered_bids.loc[len(self.offered_bids)] = [state.step, my_bid, my_bid[0], my_bid[1], my_bid[2], my_bid_utility, norm_my_bid_utility]
        self.last_decision = 'reject'
    ...
    def on_negotiation_end(self, state: MechanismState) -> None:
        ...
        if state.agreement:
            self.agreement_utility[state.agreement] = round(self.utility_func(state.agreement) / self.max_utility, 3)
        self.negotiator_id += "-agree="+str(state.agreement)
        self.received_bids[['opponent', 'is_sell', 'step', 'nego_no', 'agree']] = self.opponent, self.to_sell, self.current_step, self.nego_no, bool(state.agreement)
        self.offered_bids[['opponent', 'is_sell', 'step', 'nego_no', 'agree']] = self.opponent, self.to_sell, self.current_step, self.nego_no, bool(state.agreement)
    ...
    def get_opponent_params(self):
        behavior_params = {}
        data = self.received_bids[['round', 'quantity', 'delivery_t', 'price', 'norm_utility']]
        ...
        if data.shape[0] > 1:
            ...
            behavior_params['t_bounds'] = [max(0, int(mean_t - std_t)), int(mean_t + std_t)]
            behavior_params['q_bounds'] = [max(0, int(mean_q - std_q)), int(mean_q + std_q)]
            behavior_params['p_bounds'] = [max(0, int(mean_p - std_p)), int(mean_q + std_p)]
