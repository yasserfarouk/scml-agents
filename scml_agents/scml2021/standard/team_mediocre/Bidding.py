...
class SellBidding():
    ...
    def __init__(self, current_step, initiator, eagerness, neediness, params, valid_bounds, utility_func, use_bid_func=True):
        ...
        if len(self.q_range) > 100:
            self.q_range = np.unique(np.linspace(self.q_bounds[0], self.q_bounds[1]+1, 100).round())
    ...
    def generate_offerable_set(self):
        ...
        bid_df = pd.DataFrame(np.array([bids, list(map(self.find_bid_zone, bids))], dtype='object').T, columns=['bid', 'zone'])
        bid_df['utility'] = bid_df['bid'].apply(self.utility_func)
        self.bid_dict = {zone: bids.values for zone, bids in bid_df.groupby('zone')['bid']}
        ...
        self.offerable_bids = self.bid_df[(self.bid_df['zone'].isin(self.offerable_zones)) &
    ...
    def find_bid_zone(self, bid): #finds the zone where bid places
        ...
        d1 = (q - self.M_alpha[0][0]) * (self.M_alpha[1][1]-self.M_alpha[0][1]) - (p - self.M_alpha[0][1]) * (self.M_alpha[1][0]-self.M_alpha[0][0])
        d2 = (q - self.M_alpha[1][0]) * (self.M_alpha[2][1]-self.M_alpha[1][1]) - (p - self.M_alpha[1][1]) * (self.M_alpha[2][0]-self.M_alpha[1][0])
        d3 = (q - self.M_zero[0][0]) * (self.M_zero[1][1]-self.M_zero[0][1]) - (p - self.M_zero[0][1]) * (self.M_zero[1][0]-self.M_zero[0][0])
        d4 = (q - self.M_beta[0][0]) * (self.M_beta[1][1]-self.M_beta[0][1]) - (p - self.M_beta[0][1]) * (self.M_beta[1][0]-self.M_beta[0][0])
        d5 = (q - self.M_beta[1][0]) * (self.M_beta[2][1]-self.M_beta[1][1]) - (p - self.M_beta[1][1]) * (self.M_beta[2][0]-self.M_beta[1][0])
    ...
    def evaluate(self, opponent_bid, nego_relative_time, opponent_behaviour_params):
        ...
        o_delivery_time = o_delivery_time if o_delivery_delay >= 1 else o_delivery_time+1

        preferred_delivery_time = self.valid_t_range[0] if nego_relative_time <= 0.5 else o_delivery_time
        ...
        if self.q_bounds[0] <= q and q <= self.q_bounds[1] and self.p_bounds[0] <= p:
            ...
            if opponent_bid_zone == 'above_M_alpha' and self.t_bounds[0] <= t:
                ...
                my_bid = self.generate_responsive_bid(opponent_bid, nego_relative_time, o_delivery_time, opponent_behaviour_params)
        ...
        if p >= self.p_bounds[0] and q <= self.q_bounds[1]:
            ...
            if nego_relative_time > 0.75 and (my_bid_utility - oppo_bid_utility) / my_bid_utility <= max(0, self.neediness)/10:
    ...
    def generate_responsive_bid(self, opponent_bid, nego_relative_time, delivery_time, opponent_behaviour_params):

        opponent_concession = opponent_behaviour_params['concession_rate'] if nego_relative_time >= 0.25 else 0
        time_concession = 1 if nego_relative_time < 0.75 else self.concess_rates [int(len(self.concess_rates)*nego_relative_time)]
        restricted_q = self.q_bounds if nego_relative_time <= 0.25 else opponent_behaviour_params['q_bounds']
        upper_p = self.traded_price if nego_relative_time >= 0.9 and self.traded_price in self.valid_p_range else self.valid_p_range[1]

        oppo_bid_utility = self.utility_func(opponent_bid)

        delta = (self.max_utility - oppo_bid_utility) * time_concession * (1 + max(0, opponent_concession))
        ...
        offer_set = self.offerable_bids.loc[
            ...
            &(restricted_q[0] <= self.offerable_bids.q) & (restricted_q[1]*1.5 >= self.offerable_bids.q)
    ...
    def get_good_bid(self, nego_relative_time, delivery_time, offer_set=None):
        ...
        if not isinstance(good_bids, pd.Series):
            ...
            if "utility" not in good_bids.columns or good_bids.utility.sum()<=0:
                subset_good_bids = good_bids.loc[random.choices(good_bids.bid.values, k=min(len(good_bids), 5))]
            else:
                subset_good_bids = good_bids.loc[random.choices(good_bids.bid.values, weights=good_bids.utility, k=min(len(good_bids), 5))]
            subset_good_bids.sort_values('utility', ascending = False, inplace=True)
            bid = subset_good_bids.iloc[max(0, int(len(subset_good_bids) * nego_relative_time)-1)]['bid']
        else:
            bid = good_bids['bid']
    ...
    def generate_random_bid(self, nego_relative_time, delivery_time, offer_set=None):  #NOT USED

        if type(offer_set) == type(None):
            offer_set = self.offerable_bids

        bid = offer_set.loc[random.choices(offer_set.bid.values, weights=offer_set.utility, k=1)[0], 'bid']
        bid = (int(bid[0]), int(delivery_time), int(bid[1]))
        return bid
...
class BuyBidding():
    ...
    def evaluate(self, opponent_bid, nego_relative_time, opponent_behaviour_params):
        ...
        preferred_delivery_time = o_delivery_time if nego_relative_time >= 0.5 and o_delivery_time <= self.t_bounds[1] else np.mean(self.valid_t_range)
        ...
        if self.neediness < 0:
            more_q = min(int(my_bid[0] * (1 + abs(self.neediness))), self.valid_q_range[-1])
            my_bid = (more_q, my_bid[1], my_bid[2])

        my_bid_utility  = self.utility_func(my_bid)

        if p <= self.p_bounds[1] and q <= self.q_bounds[1]:
            if nego_relative_time > 0.75 and (my_bid_utility - oppo_bid_utility) / my_bid_utility < abs(min(0, self.neediness))/10 \
    ...
    def generate_responsive_bid(self, opponent_bid, nego_relative_time, delivery_time, opponent_behaviour_params):
        ...
        time_concession = 1 if nego_relative_time < 0.75 else self.concess_rates[int(len(self.concess_rates)*nego_relative_time)]
        restricted_q = self.q_bounds if nego_relative_time <= 0.25 else opponent_behaviour_params['q_bounds']
        lower_p = self.traded_price if nego_relative_time >= 0.9 and self.traded_price in self.valid_p_range else self.valid_p_range[0]
        ...
        offer_set = self.offerable_bids.loc[
            (oppo_bid_utility <= self.offerable_bids.utility)
            &(self.offerable_bids.utility <= oppo_bid_utility + delta)
            &(self.reasonable_q * 0.75 <= self.offerable_bids.q) & (self.reasonable_q * 1.25 >= self.offerable_bids.q)
