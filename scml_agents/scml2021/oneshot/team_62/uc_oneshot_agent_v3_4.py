...
class UcOneshotAgent3_4(OneShotAgent):
    ...
    def init(self) -> None:
        ...
        self.p_c_ratio = len(self.awi.all_consumers[0])/len(self.awi.all_consumers[1]) if self.awi.level == 1 else \
            len(self.awi.all_consumers[1])/len(self.awi.all_consumers[0])
        ...
        self.default_th_value = [((19 - a) / 19) ** (self.default_concession_rate) * (1.0 - self.default_lower_limits - self.default_upper_limits) + self.default_lower_limits for a in range(20)]
        ...
        self.success_step_factor = [0.59290947, 0.59290947, 0.49881277, 0.43398573, 0.38296614,
            0.34024604, 0.30315470, 0.27017218, 0.24034221, 0.21301989,
            0.18774760, 0.16418761, 0.14208280, 0.12123246, 0.10147656,
            0.08268525, 0.06475155, 0.04758620, 0.03111384, 0.01527020]
    ...
    def step(self) -> None:
        ...
        self.exo_quantity_history[self.awi.current_step] = self.awi.current_exogenous_input_quantity + self.awi.current_exogenous_output_quantity
        ...
        for partner in self.neg_results.keys():
            if partner not in self.partners:
                ...
                step_price = []
                for t in th:
                    p = round(self.min_value + t * (self.max_value - self.min_value)) if self.awi.level == 0 \
                        else round(self.max_value - t * (self.max_value - self.min_value))
                    step_price.append(p)
        ...
        for partner in self.neg_results.keys():
            ...
            if self.neg_results[partner][0]:
                ...
                self._th_coefficient_adjust_success(partner_index, self.neg_results[partner][1])
            else:
                #failed negotiations
                self._th_coefficient_adjust_failure(partner_index, self.neg_results[partner][1], self.neg_results[partner][2])
        ...
        for i in range(len(self.partners)):
            self.concession_rates[i] = max(self.CONCESSION_RATE["min_value"], self.concession_rates[i])
            self.concession_rates[i] = min(self.CONCESSION_RATE["max_value"], self.concession_rates[i])
            self.upper_limits[i] = max(self.UPPER_LIMIT["min_value"], self.upper_limits[i])
            self.upper_limits[i] = min(self.UPPER_LIMIT["max_value"], self.upper_limits[i])
            self.lower_limits[i] = max(self.LOWER_LIMIT["min_value"], self.lower_limits[i])
            self.lower_limits[i] = min(self.LOWER_LIMIT["max_value"], self.lower_limits[i])

        #calculate basic th value for each partner
        for i in range(len(self.partners)):
            self.th_matrix[i] = [((19 - a) / 19) ** (self.concession_rates[i]) for a in range(20)]
            for j in range(20):
                self.th_matrix[i][j] = min(self.th_matrix[i][j], (1 - self.upper_limits[i]))
        ...
        for i in range(len(self.partners)):
            for j in range(20):
                ...
                th = max(0.1, th)
                th = min(0.9, th)
                p = self.min_value + th * (self.max_value - self.min_value) if self.awi.level == 0 else self.max_value - th * (self.max_value - self.min_value)
                self.actual_price[i][j] = p
                self.actual_price[i][j] = self._adjust_with_catalog(catalog_price, self.actual_price[i][j], 10)
    ...
    def propose(self, negotiator_id: str, state: MechanismState):
        ...
        partner = ami.annotation['buyer'] if self.awi.level == 0 else ami.annotation['seller']
        ...
        if self._required_quantity() <= 0:
            ...
            if partner in self.partners:
                ...
                target_price = round(ami.issues[UNIT_PRICE].min_value + th * (ami.issues[UNIT_PRICE].max_value - ami.issues[UNIT_PRICE].min_value)) if self.awi.level == 0 \
                    else round(ami.issues[UNIT_PRICE].max_value - th * (ami.issues[UNIT_PRICE].max_value - ami.issues[UNIT_PRICE].min_value))

            return (max(self._required_quantity(), 0), self.awi.current_step, target_price)
    ...
    def respond(self, negotiator_id: str, state: MechanismState, offer) -> ResponseType:
        ...
        if partner in self.partners:
            ...
            th = self.default_th_value[state.step]
            target_price = round(min_price + th * (max_price - min_price)) if self.awi.level == 0 \
        ...
        while True:
            ...
            if self.awi.level == 0:
                if offer_quantity <= required_quantity and offer_unit_price >= target_price:
                    ...
                    over_quantity_ratio = (offer_quantity - required_quantity) / required_quantity
    ...
    def on_negotiation_success(self, contract: Contract, mechanism: AgentMechanismInterface) -> None:
        ...
        #record data
        partner = mechanism.annotation['buyer'] if self.awi.level == 0 else mechanism.annotation['seller']
        no_of_step = len(mechanism["_mechanism"]._history)
        if partner == mechanism["_mechanism"]._history[no_of_step - 1].current_proposer_agent:
            accepted_by = 1
        else:
            accepted_by = 0
    ...
    def on_negotiation_failure(self, partners, annotation, mechanism: AgentMechanismInterface, state: MechanismState) -> None:
    ...
    def _th_coefficient_adjust_failure(self, partner_index, negotiation_step, rejected_by) -> None:
        adjust_coefficient = self.success_step_factor[negotiation_step - 1]
        if rejected_by == 0:
    ...
    def _required_quantity(self) -> int:
        return self.awi.current_exogenous_input_quantity + self.awi.current_exogenous_output_quantity - self.quantity_secured

    def _past_quantity_adjust_coefficient(self) -> float:
        try:
            if self.awi.current_step == 0:
                neg_exo_ratio = self.neg_quantity_history[0]/self.exo_quantity_history[0]
            elif self.awi.current_step == 1:
                neg_exo_ratio = 0.6 * self.neg_quantity_history[1]/self.exo_quantity_history[1] \
                    + 0.4 * self.neg_quantity_history[0]/self.exo_quantity_history[0]
            else:
                neg_exo_ratio = 0.5 * self.neg_quantity_history[self.awi.current_step]/self.exo_quantity_history[self.awi.current_step] \
                    + 0.3 * self.neg_quantity_history[self.awi.current_step - 1]/self.exo_quantity_history[self.awi.current_step - 1] \
                    + 0.2 * self.neg_quantity_history[self.awi.current_step - 2]/self.exo_quantity_history[self.awi.current_step - 2]
            return 1.0 + (2 * (1 / (1 + 2.71828 ** - (8 * neg_exo_ratio - 8))) - 1) ** 5 * 0.20     #modification of sigmoid function
        except:     #divide by 0
            return 1.0
    ...
    def _adjust_with_catalog(catalog_price: int, price: float, abs_limit: int) -> int:
        ...
        if abs(diff) >= abs_limit:
            return round(catalog_price + abs_limit) if diff >= 0 else round(catalog_price - abs_limit)
        else:
