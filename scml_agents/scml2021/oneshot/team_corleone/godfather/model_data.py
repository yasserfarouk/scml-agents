...
def calc_world_cols(day_idx, agent):
    ...
    comp = agent.awi.exogenous_contract_summary[0][0] - agent.awi.exogenous_contract_summary[-1][0]

    headers = ['day', 'n_level', 'n_opp_level', 'my_layer_size', 'opp_layer_size', 'competitiveness']
...
def calc_trace_cols(hist: BilateralHistory, len_trace: int):
    ...
    for i in range(41):
        headers.append('trace {} price'.format(i))
        headers.append('trace {} quant'.format(i))
...
def calc_empirical_cols(prev_negs: List[BilateralHistory], offer_space, prior_bias=0):
    dummy_negotiation = BilateralHistory("",
        OutcomeSpace(offer_space),
        WorldInfo(-1, -1, -1, -1))

    model_empirical = ModelEmpirical("", strategy_self=StrategyGoldfishParetoAspiration, prior_bias=prior_bias)
    ...
    headers = ['empirical_distr_q_{}'.format(i) for i in range(len(q_probs))] + ['empirical_distr_p']
    row = q_probs + [est_p]
    return row, headers
...
def calc_ufun_cols(ufun_params):
    row = ufun_params
    headers = ['ufun_param_{}'.format(i) for i in range(len(row))]
    return row, headers

def insert_rows_for_agent_pairing(rows_by_trace_idx, headers_by_trace_idx, hists, opp_type, offer_space, world_data, world_headers):
    hist = hists[-1]
    for prediction_point in hist.prediction_points:   # -2 because the last one is always empty
...
def construct_test_cols(day_idx, agent, histories, opp_type, offer_space, len_trace, ufun_params):
    ...
    empirical_cols, empirical_headers = calc_empirical_cols(histories, offer_space, prior_bias=10) # TODO remove later
