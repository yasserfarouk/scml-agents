...
class BilatUFun():
    ...
    def poly_fit(self) -> List[float]:
        ...
        def poly_2(xy, a, b, c, d, e, f, g, h, i):
            x = xy[:,0]
            y = xy[:,1]
            return a*x**2*y**2 + b*x**2*y + c*x**2 + d*x*y**2 + e*x*y + f*x + g*y**2 + h*y + i
...
class BilatUFunAvg(BilatUFun):
    def __init__(self, offer_space: OfferSpace, opp_history: BilateralHistory, expected_exog_quant=None):
        ...
        input_product = None
        self.scml_ufun = OneShotUFun(ex_pin, ex_qin, ex_pout, ex_qout, input_product, input_agent, output_agent, self.expected_prod_cost, self.expected_disp_cost, self.expected_shortfall_cost, None, None, None, None, 1)
