...
class OutcomeDistr(object):
    ...
    def distance(self, other: "OutcomeDistr") -> float:
        ...
        if not self.outcome_space == other.outcome_space:
            raise ValueError("Can only find distance between distributions over same outcome space")
    ...
    def __eq__(self, other) -> bool:
        return all(abs(self(o) - other(o)) < 0.01 for o in self.outcome_space.outcome_set())
    ...
    def __call__(self, o: Outcome) -> float:
        if o in self.distr:
            ...
            if o in self.outcome_space.outcome_set():
                raise KeyError("Outcome {0} not in distribution domain".format(o))
            else:
                raise RuntimeError("OutcomeDistrTable error: distribution dict does not cover outcome space (missing {0})".format(o))
