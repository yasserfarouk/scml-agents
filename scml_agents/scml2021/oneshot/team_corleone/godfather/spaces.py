...
class OfferSpace():
    ...
    def offer_set(self) -> Set[Offer]:
        return set(Offer(p, q)
            for p in range(self._min_p, self._max_p + 1)
            for q in range(self._min_q, self._max_q + 1))

    def __eq__(self, other):
        return (self._min_p == other._min_p and
                self._max_p == other._max_p and
                self._min_q == other._min_q and
                self._max_q == other._max_q and
                self._reserve == other._reserve)
