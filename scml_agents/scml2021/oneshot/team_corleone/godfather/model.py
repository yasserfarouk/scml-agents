...
def realistic(model_call):
    def call(model,
        ...
        enable_realistic_checks=True) -> OutcomeDistr:
        if enable_realistic_checks:
            ...
            if next_move == Moves.ACCEPT:
                return OutcomeDistrPoint(ufun.outcome_space, histories[-1].standing_offer())
...
class ModelCheating(Model):
    ...
    def __call__(self, self_ufun: BilatUFun, histories: List[BilateralHistory]) -> OutcomeDistr:
        ...
        current_history = copy.deepcopy(histories[-1]) # very important! can't mess with this history
