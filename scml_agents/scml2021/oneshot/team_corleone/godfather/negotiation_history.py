...
from .spaces import Outcome, OutcomeSpace, Move, Moves
...
class BilateralHistory():
    ...
    def register_prediction_point(self, ufun: "BilatUFun") -> None:
        self.prediction_points.append({
            'trace_idx': len(self.moves),
            'ufun_params': ufun.poly_fit_3rd_correct()
        })
    ...
    def add_move(self, m: Move, is_me: bool) -> None:
        ...
        if self.moves and self.moves[-1][0] == mover_idx:
            if self.moves[-1] == m:
                warnings.warn("Negotiation history: ignoring second identical move from same mover")
            else:
                warnings.warn("Negotiation history {0} invalid: two moves in a row from mover {1}: {2}, adding {3}, is_me={4}"
                    .format(self.opp_id, mover_idx, self, m, is_me))
            return
        elif not self.moves and mover_idx == 0:
            warnings.warn("Negotiation history {0} invalid: first move from opponent".format(self.opp_id))
            self.moves.append((1, self.outcome_space.offer_space().offer_set().pop()))
        if not self.moves and m is Moves.ACCEPT:
            warnings.warn("Negotiation history {0} invalid: acceptance on first move"
                .format(self.opp_id))
        if self.is_ended():
            warnings.warn("Negotiation history {0} invalid: additional move {1} after negotiation ended due to {2}"
                .format(self.opp_id, m, "failure" if self.is_failed else "terminal move {0}".format(self.moves[-1])))
            return
        self.moves.append((mover_idx, m))
    
    def est_frac_complete(self):
        f = (len(self.moves) + self.n_moves_adjustment) / (2 * self.world_info.n_negotiation_rounds)
        return max(0, f)
    
    def invert(self) -> "BilateralHistory":
        """Returns a new BilateralHistory with self and opponent swapped.
        Note: NOT a deep copy."""
        bh = BilateralHistory(opp_id=self.opp_id,
    ...
    def standing_offer(self) -> Optional[Offer]:
        ...
        i, m = self.moves[-1] if self.moves else (None, None) # beware: do not remove parens
    ...
    def fail(self) -> None:
        ...
        if not self.moves:
            warnings.warn("Negotiation failed with no moves... uh-oh. Probably means we timed out on first_proposals()")
        self.is_failed = True
    ...
    def outcome(self) -> Outcome:
        ...
        _, last_move = self.moves[-1] if self.moves else (None, None) # beware: do not remove parens
    ...
    def __repr__(self) -> str:
        return "<bilat_hist: {0}, failed: {1}>".format([str(m) for m in self.moves], self.is_failed)
...
class SCMLHistory():
    ...
    def register_agent(self, opp_id: str, outcome_space: OutcomeSpace) -> None:
        if not opp_id in self.negotiations:
            if self.round != 0:
                warnings.warn("Cannot add negotiator {0} at round {1}".format(opp_id, self.round))
    ...
    def fail(self, opp_id: str) -> None:
        if opp_id not in self.negotiations:
            warnings.warn("failing unknown negotiation {}".format(opp_id))
            return
        current_negotiation = self.negotiations[opp_id][-1]
    ...
    def move(self, opp_id: str, move: Move, is_me: bool) -> None:
        if opp_id not in self.negotiations:
            warnings.warn("moving in unknown negotiation {}, {}, is_me={}"
                .format(opp_id, move, is_me))
            return
        current_negotiation = self.negotiations[opp_id][-1]
    ...
    def negotiator_histories(self, opp_id: str) -> List[BilateralHistory]:
        if not opp_id in self.negotiations:
            raise ValueError("Invalid negotiator id {0} is not a registered agent ({1})"
                .format(opp_id, self.negotiations.keys()))
    ...
    def __repr__(self) -> str:
        return "<SCMLHistory {0}>".format(self.negotiations)
