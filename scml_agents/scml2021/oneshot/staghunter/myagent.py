...
__all__ = [ "StagHunter", "StagHunterTough", ]
...
class EstimatedUtility(UtilityFunction):
    def __init__(self, id, owner, awi, beta, memory_size):
        ...
        self.beta = beta
        self.owner = owner
        if(memory_size < 0): memory_size = self.awi.n_steps
    ...
    def check_updated(self):
        if(type(self.historical_parameter[self.awi.current_step]) == type(None)):
            ...
            cur_market_parameters['current_exogenous_input_quantity'] = self.awi.current_exogenous_input_quantity
            cur_market_parameters['current_exogenous_input_price'] = self.awi.current_exogenous_input_price
            cur_market_parameters['current_exogenous_output_quantity'] = self.awi.current_exogenous_output_quantity
            cur_market_parameters['current_exogenous_output_price'] = self.awi.current_exogenous_output_price
            ...
            cur_market_parameters['current_shortfall_penalty'] = self.awi.current_shortfall_penalty
            cur_market_parameters['current_disposal_cost'] = self.awi.current_disposal_cost
    ...
    def loss_function(self, sampled_time):
        ...
        for t in sampled_time:
            features, ground_truth_offer, ground_truth_output = self.dataset[t][0], self.dataset[t][1], self.dataset[t][2]
            loss_t = 0
            for j in range(len(self.historical_offers[t])):
                ...
                if(code not in self.buffer):
                    self.buffer[code] = self.from_offers(offers, outputs, self.historical_parameter[t])
                ...
                while(True):
                    ...
                    for i in range(self.awi.current_step, 0, -1):
                        ...
                        cof = np.exp(-np.sum(self.weights * np.square(cur_features-features)))
                        self.weights_bound = np.maximum(self.weights_bound, np.square(cur_features-features))
                        code = hash(str(cur_offer) + '*' + str(i))
                        if(code not in self.buffer):
                            self.buffer[code] = self.from_offers(offers, outputs, self.historical_parameter[t])
                        cur_payoff = self.buffer[code]
                        trace.append((self.weights, cur_features, features, np.square(cur_features-features)))
    ...
    def eval(self, offer):
        ...
        if(self.awi.current_step == 0):
            return self.from_offers([offer], [is_output], self.historical_parameter[self.awi.current_step])
        ...
        while(True):
            ...
            res = 0
            counter = 0
            for i in range(self.awi.current_step-1, max(self.awi.current_step-5, -1), -1):
                #print(self.dataset)
                cur_features = self.dataset[i][0]
                other_offers = self.dataset[i][1]
                other_is_output = self.dataset[i][2]
                ...
                res += cof * self.from_offers(offers, outputs, self.historical_parameter[self.awi.current_step])
                counter += cof
            if(counter <= 1e-2):
                self.weights = 1/self.weights_bound
            else:
    ...
    def from_offers(
        ...
        offers += [
            (current_exogenous_input_quantity, current_step, current_exogenous_input_price / current_exogenous_input_quantity if current_exogenous_input_quantity else 0),
            (current_exogenous_output_quantity, 0, current_exogenous_output_price / current_exogenous_output_quantity if current_exogenous_output_quantity else 0),
        ]
        outputs += [False, True]
        ...
        for offer, is_output, is_exogenous in sorted_offers:
            ...
            if not going_bankrupt and (
                pin + topay_this_time + offer[QUANTITY] * cost
                > current_balance
            ):
        ...
        u = self.from_aggregates(
            qin, qout, producible, pin, pout_bar, input_penalty, output_penalty, cost, n_lines
        )
...
class StagHunter(OneShotAgent):
    ...
    def init(self, beta=5, memory_size=50):
        ...
        self.dataset = dict([(id_, [[[], [], []] for _ in range(self.awi.n_steps+1)]) for id_ in self.oppo_id_list])

        self._ufuns__ = dict([(id_, EstimatedUtility(id=id_, owner=self, awi=self.awi, beta=beta, memory_size=memory_size)) for id_ in self.oppo_id_list])

        self.__negotiatiors = dict([(id_, AspirationNegotiator(ufun=self._ufuns__[id_])) for id_ in self.oppo_id_list])
    ...
    def propose(self, negotiator_id, state):
        ...
        if(negotiator_id in self.oppo_id_list):
            ...
            if(type(res) != type(None)):
                self._ufuns__[negotiator_id].add_offer(offer=res, step=self.awi.current_step)
        return res
    ...
    def response(self, negotiator_id, state, offer):
        ...
        if(negotiator_id in self.oppo_id_list):
            ...
            if(type(offer) != type(None)):
                self._ufuns__[negotiator_id].add_offer(offer=offer, step=self.awi.current_step)
        return res
    ...
    def step(self):
        ...
        for id_ in self.oppo_id_list:
            self._ufuns__[id_].update(self.dataset[id_][self.awi.current_step], self.awi.current_step)
    ...
    def on_negotiation_failure(
        ...
        #     pickle.dump([], f)
        partner_id = annotation["buyer"] if annotation["seller"] == self._owner.id else annotation["seller"]
    ...
    def get_features(self, negotiator_id):
        ...
        for id_ in self.oppo_id_list:
            try:
                ...
                oppo_features += [self.awi.current_step-oppo_report.step, oppo_report.breach_prob, oppo_report.breach_level]
        ...
        market_features = [self.awi.exogenous_contract_summary[0][0], self.awi.exogenous_contract_summary[0][1]/(self.awi.exogenous_contract_summary[0][0] * self.awi.trading_prices[0]), self.awi.exogenous_contract_summary[-1][0], self.awi.exogenous_contract_summary[-1][1]/(self.awi.exogenous_contract_summary[-1][0] * self.awi.trading_prices[-1])]


        #my_features = [self.awi.current_disposal_cost, self.awi.current_shortfall_penalty, self.awi.current_balance]

        my_features = [self.awi.current_disposal_cost/self.awi.trading_prices[self.awi.my_input_product], self.awi.current_shortfall_penalty/self.awi.trading_prices[self.awi.my_output_product]]

        if(self.awi.is_first_level):
            my_features += [self.awi.current_exogenous_input_quantity, self.awi.current_exogenous_input_price/self.awi.trading_prices[self.awi.my_input_product]]
        else:
            my_features += [self.awi.current_exogenous_output_quantity, self.awi.current_exogenous_output_price/self.awi.trading_prices[self.awi.my_output_product]]
...
class StagHunterTough(StagHunter):
    def init(self, beta=5, memory_size=50):
        ...
        self.__negotiatiors = dict([(id_, ToughNegotiator(ufun=self._ufuns__[id_])) for id_ in self.oppo_id_list])
...
def run(
    ...
    **Not needed for submission.** You can use this function to test your agent.
    ...
    if competition == "oneshot":
        ...
        competitors = [StagHunter, StagHunterTough, RandomOneShotAgent, SyncRandomOneShotAgent, GreedyOneShotAgent, GreedySyncAgent, GreedySingleAgreementAgent]
    else:
