...
class GreedyTestAgent(GreedyOneShotAgent):
    ...
    def step(self):
        ...
        for oppo_id in self.oppo_list:
            ...
            if(len(sort_hist)):
                self._max_util_dist[oppo_id][sort_hist[0][0]] += 1.
        ...
        if(self.awi.current_step == self.awi.n_steps - 1):
            for oppo_id in self.oppo_list:
                self._max_util_dist[oppo_id] = self._max_util_dist[oppo_id] / np.sum(self._max_util_dist[oppo_id])


            with open("./dis{}.pkl".format(time.time()), "wb") as f:
...
def run(
    ...
    competitors = [GreedyTestAgent, GreedyOneShotAgent, GreedyOneShotAgent, GreedySyncAgent, GreedySingleAgreementAgent, RandomOneShotAgent]
