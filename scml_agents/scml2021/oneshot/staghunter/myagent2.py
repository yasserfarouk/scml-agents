...
from negmas.sao import AspirationNegotiator, ToughNegotiator, OnlyBestNegotiator,  NaiveTitForTatNegotiator, SimpleTitForTatNegotiator, NiceNegotiator
import warnings
...
__all__ = [ "StagHunter", "StagHunterV5", "StagHunterV6", "StagHunterV7", ]
...
class StagHunter(OneShotAgent):
    ...
    def init(self):
        ...
        self._best_opp_acc_prices = defaultdict(lambda: [0.0]) if self.awi.is_first_level else defaultdict(lambda: [float("inf")])
    ...
    def step(self, lr=0.1):
        ...
        for oppo_id in self.oppo_list:
            if(oppo_id in self.cur_offer_list):
                ...
                u_p  = self.ufun.from_offers(cur_offer, [self.awi.is_first_level] * (len(cur_offer)))
        ...
        if(self.awi.current_step > 0):
            ...
            revs = [(oppo_id, np.mean(self._prev_mag_profit[oppo_id][1:])) for oppo_id in self.oppo_list]
        ...
        for oppo_id in self.oppo_list:
            ...
            if(self.awi.is_first_level):
                self._best_opp_acc_selling[oppo_id] = max(self._best_opp_acc_prices[oppo_id][-self._num_of_consider_day_price:])
            else:
                self._best_opp_acc_buying[oppo_id] = min(self._best_opp_acc_prices[oppo_id][-self._num_of_consider_day_price:])




        self._best_acc_selling = max(self._best_acc_prices[-self._num_of_consider_day_price:])
        self._best_acc_buying = min(self._best_acc_prices[-self._num_of_consider_day_price:])
    ...
    def on_negotiation_success(self, contract, mechanism):
        ...
        if(self.awi.is_first_level):
            self._quantity_slack = [(quantity_slack * 3 + 1)/4 for quantity_slack in self._quantity_slack]
    ...
    def propose(self, negotiator_id: str, state):
        ...
        offer[QUANTITY] = min(offer[QUANTITY], max(int(self._quantity_slack[self._cur_rank[negotiator_id]] * self._needed()), 1))
        ...
        if(len(self._prev_opp_quantity[negotiator_id])):
            ...
            offer[QUANTITY] = max(1, min(np.min(self._prev_opp_quantity[negotiator_id][-self._num_of_consider_day_quantity:]), offer[QUANTITY]))



        u1 = self.ufun.from_offers(list(self.cur_offer_list.values()), [self.awi.is_first_level] * len(self.cur_offer_list))
        ...
        offer[UNIT_PRICE] = min(max(self._find_good_price(self.get_ami(negotiator_id), state, offer), unit_price_issue.min_value), unit_price_issue.max_value)
        ...
        test_price_interval = np.linspace(raw_price, ami.issues[UNIT_PRICE].max_value, 50) if self.awi.is_first_level else np.linspace(raw_price, ami.issues[UNIT_PRICE].min_value, 50)
        ...
        for p in test_price_interval:
            ...
            cur_offer[UNIT_PRICE] = max(min(int(p), ami.issues[UNIT_PRICE].max_value), ami.issues[UNIT_PRICE].min_value)

            u1 = self.ufun.from_offers(list(self.cur_offer_list.values()), [self.awi.is_first_level] * len(self.cur_offer_list))


            u2 = self.ufun.from_offers(list(self.cur_offer_list.values()) + [cur_offer], [self.awi.is_first_level] * (len(self.cur_offer_list) + 1))
    ...
    def respond(self, negotiator_id, state, offer):
        ...
        if(offer[QUANTITY] < ami.issues[QUANTITY].min_value or offer[QUANTITY] > ami.issues[QUANTITY].max_value):
            return ResponseType.REJECT_OFFER

        if(offer[UNIT_PRICE] < ami.issues[UNIT_PRICE].min_value or offer[UNIT_PRICE] > ami.issues[UNIT_PRICE].max_value):
            return ResponseType.REJECT_OFFER

        u1 = self.ufun.from_offers(list(self.cur_offer_list.values()), [is_selling] * len(self.cur_offer_list))

        offers = [offer] + list(self.cur_offer_list.values())
        u2 = self.ufun.from_offers(offers, [is_selling] * (len(self.cur_offer_list) + 1))
    ...
    def _needed(self):
        ...
        demand = self.awi.current_exogenous_input_quantity if self.awi.is_first_level else self.awi.current_exogenous_output_quantity
                ...
                max(
                    ...
                    + [
                        ...
                        for p, slack in (
                            ...
                            (self.awi.trading_prices[self.awi.my_output_product], self._trading_price_slack),
                        )
                    ]
                ),
            )
                ...
                min(
                    ...
                    + [
                        ...
                        for p, slack in (
                            ...
                            (self._best_opp_acc_buying[partner], self._opp_acc_price_slack2),
                            (self._cur_best_price, self._step_agg_price_slack2),
                            (2 * self.awi.catalog_prices[self.awi.my_input_product], self._trading_price_slack2),
                        )
                    ]
                ),
            )
...
class StagHunterV5(OneShotAgent):
    ...
    def step(self, lr=0.1):
        ...
        if(self.awi.current_step > 0):
            ...
            revs = [(oppo_id, np.mean(self._best_opp_acc_prices[oppo_id][-3:])) for oppo_id in self.oppo_list] if self.awi.is_first_level else [(oppo_id, -np.mean(self._best_opp_acc_prices[oppo_id][-3:])) for oppo_id in self.oppo_list]
            revs = sorted(revs, key=lambda x: x[1], reverse=True)
        ...
        for oppo_id in self.oppo_list:
            ...
            if(self.awi.is_first_level):
                self._best_opp_acc_selling[oppo_id] = max(self._best_opp_acc_prices[oppo_id][-3:])
            else:
                self._best_opp_acc_buying[oppo_id] = min(self._best_opp_acc_prices[oppo_id][-3:])
    ...
    def _needed(self):
        ...
        #summary = self.awi.exogenous_contract_summary
        #demand = min(summary[0][0], summary[-1][0]) / (self.awi.n_competitors + 1)
        demand = self.awi.current_exogenous_input_quantity if self.awi.current_exogenous_input_quantity else self.awi.current_exogenous_output_quantity
        return demand - self._secured
    ...
    def propose(self, negotiator_id: str, state):
        ...
        # over-write the unit price in the best offer with a good-enough price
        offer[UNIT_PRICE] = self._find_good_price(self.get_ami(negotiator_id), state, offer)
        ...
        test_price_interval = np.linspace(raw_price, unit_price_issue.max_value, 25) if self.awi.is_first_level else np.linspace(raw_price, unit_price_issue.min_value, 25)
    ...
    def respond(self, negotiator_id, state, offer):
        ...
        u2 = self.ufun.from_offers([offer] + list(self.cur_offer_list.values()), [is_selling] * (len(self.cur_offer_list) + 1))
...
class StagHunterV6(StagHunterV5):
    ...
    def _is_good_price(self, ami, state, offer):
        ...
        u_mn = self.ufun.from_offers([mn_offer] + list(self.cur_offer_list.values()), [is_selling] * (len(self.cur_offer_list) + 1))

        mx_offer = deepcopy(offer)
        mx_offer[UNIT_PRICE] = mx


        u_mx = self.ufun.from_offers([mx_offer] + list(self.cur_offer_list.values()), [is_selling] * (len(self.cur_offer_list) + 1))
    ...
    def _find_good_price(self, ami, state, offer):
        ...
        test_price_interval = np.linspace(mn, mx, 50) if self.awi.is_first_level else np.linspace(mx, mn, 50)
        ...
        for p in test_price_interval:
            ...
            u2 = self.ufun.from_offers([offer_p] + list(self.cur_offer_list.values()), [is_selling] * (len(self.cur_offer_list) + 1))
...
class StagHunterV7(StagHunter):
    def propose(self, negotiator_id: str, state):
        ...
        if(self._needed() > 0):
            #mx = min(max(int(self._quantity_slack[self._cur_rank[negotiator_id]] * self._needed()), quantity_issue.min_value), quantity_issue.max_value)
            mx = min(max(int(self._needed()), quantity_issue.min_value), quantity_issue.max_value)
        ...
        # over-write the unit price in the best offer with a good-enough price
        offer[UNIT_PRICE] = int(self._find_good_price(self.get_ami(negotiator_id), state, offer))

        if(len(self._prev_opp_price[negotiator_id]) <= 3):
            self._prev_self_price.append(offer[UNIT_PRICE])
            return offer

        offer[UNIT_PRICE] = int(self._prev_opp_price[negotiator_id][-3]/self._prev_opp_price[negotiator_id][-2] * self._prev_self_price[-1])
...
def run(
    ...
    **Not needed for submission.** You can use this function to test your agent.
    ...
    """
    competitors = [StagHunterV5, StagHunterV6, StagHunter, GreedyOneShotAgent, GreedySyncAgent, StagHunterV7, RandomOneShotAgent]
