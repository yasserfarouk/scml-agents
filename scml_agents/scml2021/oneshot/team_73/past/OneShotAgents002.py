...
class LearningSyncAgent(OneShotSyncAgent, ABC):
    ...
    def init(self):
        ...
        if self.awi.is_first_level:
            self._threshold = 0.5
            self.strong_degree = 0.0
            self.min_max_delta = 0.1
        else:
            self._threshold = 0.5
            self.strong_degree = 4.0
            self.min_max_delta = 0.3
    ...
    def step(self):
        ...
        elif self.awi.current_step in offer_simulation_steps:
            self.strong_degree = max(self.strong_degree + self.min_max_delta * 0.5, 0)
        else:
            self._threshold = max(self._threshold - self._delta, 0)
            self.strong_degree = max(self.strong_degree - self.min_max_delta, 0)

        # deltaの減衰
        self._delta *= 0.99
        self.min_max_delta *= 0.99
    ...
    def counter_all(self, offers, states):
        ...
        threshold = std + rng * self.util_th(state.step, self.get_ami(n).n_steps)
        ...
        # for k, v in chosen.items():
        #     up = v[UNIT_PRICE]
        #     if self._is_good_price(self.get_ami(n), state.step, up):
        #         responses[str(k)] = SAOResponse(ResponseType.ACCEPT_OFFER, None)
