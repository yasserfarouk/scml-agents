...
class SupplyDemandAllocator:
    ...
    def _distribute_equally_limited(
        self, quantity: int, partners: list[str], max_per_partner: int
    ) -> list[int]:
        """
    ...
    def distribute_todays_needs(
        self, productivity: float = 1.0, ptoday: float = 1.0, partners=None
    ):
        if partners is None:
            return None
        ...
        for is_partner, edge_needs in (
            ...
            needs = int(
                min(
                    max(base_needs - inventory, 0), self.awi.n_lines * self.productivity
                )
            )
            # special rules: step > 2 & 自分が購入する & (lv.1 or lv.2) -> 原材料を確保したいのでneedsを無理やり変更
            if (
                self.awi.current_step < 2
                and is_partner == au.is_seller
                ...
                is_partner == au.is_seller
                and self.awi.current_inventory_input <= self.awi.n_lines
                and self.awi.total_supplies_from(self.awi.current_step)
                < self.awi.n_lines * 2
            ):
                needs = min(
                    self.awi.n_lines * 2 - self.awi.current_inventory_input,
                    len(self.awi.my_suppliers) * 4,
                )
            if (
                is_partner == au.is_seller
                and self.awi.total_supplies_until(self.awi.current_step + 2)
                + self.awi.current_inventory_input
                >= self.awi.n_lines * 1.8
            ):
                needs = 0
            ...
            active_partners = active_partners[
                : max(1, int(self.ptoday * len(active_partners)))
            ]
            ...
            if (
                ...
                allocation = self._distribute_equally_limited(
                    needs, active_partners, max_per_partner
                )
            else:
    ...
    def has_supplier_distribution(self, distribution: dict[str, int]) -> bool:
        ...
        """
        total_supply = sum(
            qty for pid, qty in distribution.items() if au.is_seller(self.awi, pid)
        )
