...
class ProposalStrategy:
    ...
    def __init__(
        self, awi: StdAWI, partners, record: RecordManager, productivity, ptoday
    ):
        """提案戦略の初期化"""
        self.awi = awi
    ...
    def before_step(self, active_negotiators):
        ...
        self.todays_distribution = self.sda.distribute_todays_needs(
            self.productivity, self.ptoday, active_negotiators
        )
        ...
        self.offer_generator = OfferGenerator(
            self.awi, self.todays_distribution, self.ue
        )
    ...
    def should_accept_offer(self, partner_id: str, offer, threshold: float) -> bool:
        ...
        if (
            ...
            and au.is_seller(self.awi, partner_id)
            and self.awi.level > 0
        ):
            # 原材料は確保したいが，あまりにも提案量が多いofferや納期が遅いofferはREJECT
            if offer[QUANTITY] < self.awi.n_lines * 0.7:
                return (
                    self.ue.time_utility(partner_id, offer[TIME])
                    + self.ue.price_utility(partner_id, offer[UNIT_PRICE])
                ) / 2 > threshold
            else:
                is_producible = False
    ...
    def should_end_negotiation_offer(self, partner_id):
        ...
        #     return True
        if self.sda.has_supplier_distribution(
            self.todays_distribution
        ) and au.is_seller(self.awi, partner_id):
            return True
        elif (
            self.record.get_contracted_supplies_current_step() > self.awi.n_lines * 1.4
        ):
            return True
        return False
    ...
    def accept_if_few_today_offers(
        self, responses: dict[str, SAOResponse], offers: dict[str, tuple[int, int, int]]
        ...
        best_combo = self.best_offer_combination(
            today_offers, max_quantity - current_quantity
        )
    ...
    def final_round_accept_today_offers(
        self, responses: dict[str, SAOResponse], offers: dict[str, tuple[int, int, int]]
    ) -> dict[str, SAOResponse]:
        """
        ...
        for partner_id, offer in offers.items():
            if (
                au.is_buyer(self.awi, partner_id)
                and offer[TIME] == today
                and self.ue.ufun(partner_id, offer) > 0.1  # 念のため効用0以下は避ける
            ):
        ...
        for partner_id, offer in today_offers:
            ...
            if (
                partner_id in responses
                and responses[partner_id].response == ResponseType.ACCEPT_OFFER
            ):
                continue
    ...
    def generate_initial_offer(self, partners):
        ...
        for partner_id in partners:
            ...
            quantity = self.offer_generator.get_best_quantity(
                partner_id, pid_distribution
            )
    ...
    def generate_counter_offer(self, offers, threshold: float):
        ...
        for partner_id, offer in offers.items():
            ...
            if self.should_end_negotiation_offer(partner_id):
                responses[partner_id] = SAOResponse(ResponseType.END_NEGOTIATION)
                continue
            elif self.should_accept_offer(
                partner_id, offer, threshold * step_discount * round_discount
            ):
                responses[partner_id] = SAOResponse(ResponseType.ACCEPT_OFFER, offer)
                continue
            base_offer = self.offer_generator.generate_base_offer(
                partner_id, threshold * step_discount * round_discount
            )
            concession_offer = self.offer_generator.find_least_concession_offer(
                partner_id, base_offer
            )
