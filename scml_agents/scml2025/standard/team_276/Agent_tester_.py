...
parser = argparse.ArgumentParser(description="Run SCML2025 simulation with specified agents.")
parser.add_argument("--agents", nargs="+", default=["LitaAgentY",  "SyncRandomStdAgent"],
    ...
    for contract in world.saved_contracts:
        ...
        if negotiation:
            try:
                ...
                trace = None
            if trace:
                negotiation_traces.append((contract["buyer_name"], contract["seller_name"], trace))
...
for (buyer, seller, trace) in negotiation_traces:
    ...
    for idx, (step, negotiator, offer) in enumerate(trace):
        ...
        party = "Buyer" if (isinstance(negotiator, str) and buyer in negotiator) else "Seller"
        offers_data.append((idx, price, quantity, party))
    df_offers = pd.DataFrame(offers_data, columns=["round", "price", "quantity", "party"])
    plt.figure()
    for party, grp in df_offers.groupby("party"):
        plt.scatter(grp["round"], grp["price"], s=(grp["quantity"] if grp["quantity"].notnull().all() else 20)*5,
                    label=f"{party} offer", alpha=0.7)
...
for _, ct in contracts_df.iterrows():
    b = ct["buyer_name"]; s = ct["seller_name"]
    p = ct["unit_price"]; q = ct["quantity"]
...
for ct in lita_agent_y_contracts:
    ...
    # 计算买方利润 (使用环境价格，如果有的话)
    env_price = world_params.get("unit_price", 1.5 * price)  # 如果没有环境价格，假设为合约价格的1.5倍
...
agents = sorted({a for a in (contracts_df["buyer_name"].tolist() + contracts_df["seller_name"].tolist())
                 if a not in ["BUYER", "SELLER"]})
profit = {a: 0 for a in agents}
contract_count = {a: 0 for a in agents}
for _, ct in contracts_df.iterrows():
    b = ct["buyer_name"]; s = ct["seller_name"]
    qty = ct["quantity"]; price = ct["unit_price"]
    if b in profit:
        profit[b] -= price * qty
        contract_count[b] += 1
    if s in profit:
...
ax2.plot(agents, [contract_count[a] for a in agents], color="green", marker="o", linestyle="--")
