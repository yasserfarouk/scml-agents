...
def test_inventory_manager_10_days():
    ...
    for day in range(1, 11):
        ...
        # 打印未来几天的生产计划
        planned_days = sorted([d for d in production_plan.keys() if d >= day])[
            :5
        ]  # 只显示未来5天
        ", ".join([f"第{d}天: {production_plan[d]}" for d in planned_days])
        ...
        daily_reports["contracts_added"].append(
            num_supply_contracts + num_demand_contracts
        )
        ...
        future_production = sum(
            [v for k, v in production_plan.items() if k >= day and k <= day + 5]
        )
    ...
    # 计算未交付订单
    [c for c in im._pending_supply if c.delivery_time > final_day]
    [c for c in im._pending_demand if c.delivery_time > final_day]
...
def visualize_results(reports):
    ...
    plt.bar(
        df["day"] + 0.2,
        df["materials_received"] + df["products_delivered"],
        width=0.4,
        label="完成交付",
    )
    plt.title("合同和交付情况")
...
if __name__ == "__main__":
    ...
    if raw_batches:
        for batch in raw_batches:
            print(
                f"批次ID: {batch['batch_id']}, 数量: {batch['quantity']}, "
