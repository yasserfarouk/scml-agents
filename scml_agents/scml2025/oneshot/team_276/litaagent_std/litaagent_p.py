...
from negmas import (
    Outcome,  # Offers are Outcomes (dictionaries) / 报价是 Outcome 类型 (字典)
    ResponseType,
    SAOResponse,
)
...
from scml.std import (  # Standard issue names / 标准问题名称
    QUANTITY,
    TIME,
    UNIT_PRICE,
    StdSyncAgent,
...
def distribute(q: int, n: int) -> list[int]:
    ...
    r = Counter(
        choice(range(n), q - n)
    )  # choice needs a range for the first arg / choice 的第一个参数需要一个范围
...
class LitaAgentP(StdSyncAgent):
    ...
    def __init__(
        self, *args, threshold_factor=0.1, ptoday=0.70, productivity=0.7, **kwargs
    ):
        """
        Args:
        ...
        self._productivity = (
            productivity  # Agent's production efficiency / 代理的生产效率
        )
    ...
    def step(self):
        ...
        if (
            self.awi
        ):  # awi (Agent World Interface) provides simulation state / awi 提供模拟状态
            self._threshold = self.awi.n_lines * self._threshold_factor

    def _create_offer(
        self, quantity: int, time: int, unit_price: int | float
    ) -> Outcome | None:
        """
    ...
    def first_proposals(self) -> dict[str, Outcome | None]:
        ...
        future_supply_partners: list[
            str
        ] = []  # Partners I might buy from in the future / 我未来可能从其购买的伙伴
        future_consume_partners: list[
            str
        ] = []  # Partners I might sell to in the future / 我未来可能向其销售的伙伴

        for partner_id, quantity_needed in todays_distribution.items():
            if (
                quantity_needed > 0
            ):  # If a quantity is assigned for today / 如果今天分配了数量
                price = self.best_price(
                    partner_id
                )  # Get my ideal price / 获取我的理想价格
                if price is not None:
                    proposals[partner_id] = self._create_offer(
                        quantity_needed, current_step, price
                    )
            # If no quantity for today, consider for future offers
            # 如果今天没有数量，则考虑未来报价
            elif self.is_supplier(
                partner_id
            ):  # Partner is a supplier to me / 伙伴是我的供应商
                future_supply_partners.append(partner_id)
            elif self.is_consumer(
                partner_id
            ):  # Partner is a consumer of my goods / 伙伴是我的客户
    ...
    def counter_all(
        self, offers: dict[str, Outcome | None], states: dict[str, MechanismState]
        ...
        for is_selling_mode in [
            False,
            True,
        ]:  # False for buying, True for selling / False 代表购买, True 代表销售
            all_partners_on_this_side = (
                self.awi.my_suppliers if not is_selling_mode else self.awi.my_consumers
            )
                ...
                if self.awi.total_sales_at(self.awi.current_step) <= self.awi.n_lines:
                    needs_today = int(
                        max(
                            ...
                            min(
                                self.awi.n_lines,
                                daily_production + self.awi.current_inventory_input,
                            )
            ...
            active_partners_this_side = {
                p
                for p in all_partners_on_this_side
                if p in offers.keys() and p not in responses.keys()
            }

            current_step_offers_this_side: dict[
                str, Outcome
            ] = {}  # Offers for today / 今日报价
            future_step_offers_this_side: dict[
                str, Outcome
            ] = {}  # Offers for future days / 未来日期报价

            for p_id in active_partners_this_side:
                offer = offers[p_id]
                if (
                    offer is None
                ):  # No offer from this partner, or negotiation ended by them / 该伙伴无报价，或对方结束谈判
                ...
                if not (
                    ...
                    and all(k in offer for k in (TIME, UNIT_PRICE, QUANTITY))
                ):
                    responses[p_id] = SAOResponse(
                        ResponseType.REJECT_OFFER, None
                    )  # Invalid offer structure / 无效报价结构
                    continue
                ...
                if self.is_valid_price(offer[UNIT_PRICE], p_id):
                    if offer[TIME] == self.awi.current_step:
                        ...
                        and offer[TIME] < self.awi.n_steps
                    ):  # Valid future offer / 有效的未来报价
                    ...
                    counter_offer = (
                        self._create_offer(offer[QUANTITY], offer[TIME], my_price)
                        if my_price is not None
                        else None
                    )
                    responses[p_id] = SAOResponse(
                        ResponseType.REJECT_OFFER, counter_offer
                    )
            ...
            for p_id, offer_data in future_step_offers_this_side.items():
                if (
                    p_id in responses
                ):  # Already responded to this partner (e.g., due to invalid price earlier) / 已响应过此伙伴 (例如，由于之前的无效价格)
                    continue
                ...
                is_future_offer_for_my_purchase = self.is_supplier(
                    p_id
                )  # True if I'm buying from p_id / 如果我从 p_id 购买则为 True
                ...
                remaining_need_for_this_offer = (
                    current_total_need_at_future - already_accepted_this_round
                )

                if (
                    quantity_offered > 0
                    and quantity_offered <= remaining_need_for_this_offer
                ):
                    responses[p_id] = SAOResponse(ResponseType.ACCEPT_OFFER, offer_data)
                    accepted_qty_future_step_this_round[
                        (step, is_future_offer_for_my_purchase)
                    ] += quantity_offered
                else:
                    ...
                    if my_concessional_price is not None:
                        proposable_q = max(
                            0, remaining_need_for_this_offer
                        )  # How much I can still take / 我还能接受多少
                        if (
                            proposable_q > 0
                        ):  # If I still need some, counter for that amount / 如果我还需要一些，则以该数量还价
                            counter = self._create_offer(
                                proposable_q, step, my_concessional_price
                            )
                        elif (
                            quantity_offered > 0
                        ):  # I have no need, but they offered. Counter their Q at my P. / 我没有需求，但他们报价了。以他们的数量和我的价格还价。
                            counter = self._create_offer(
                                quantity_offered, step, my_concessional_price
                            )
            ...
            partners_for_current_combinatorics = {
                p_id
                for p_id, offer in current_step_offers_this_side.items()
                if p_id not in responses
            }

            chosen_combination_idx = (
                -1
            )  # Index in all_combinations list / 在 all_combinations 列表中的索引
            ...
            if needs_today > 0 and partners_for_current_combinatorics:
                ...
                all_combinations = list(
                    powerset(partner_list_powerset)
                )  # All subsets of partners / 所有伙伴子集

                best_plus_diff, best_idx_plus = (
                    float("inf"),
                    -1,
                )  # Best combination if sum_quantity >= needs_today / 如果总数量 >= 今日需求 的最佳组合
                best_minus_diff, best_idx_minus = (
                    float("inf"),
                    -1,
                )  # Best combination if sum_quantity < needs_today / 如果总数量 < 今日需求 的最佳组合

                for i, partner_id_tuple in enumerate(all_combinations):
                    if not partner_id_tuple:
                        continue  # Skip empty set / 跳过空集

                    offered_sum = sum(
                        current_step_offers_this_side[p_id][QUANTITY]
                        for p_id in partner_id_tuple
                    )
                    diff = abs(offered_sum - needs_today)

                    if offered_sum >= needs_today:
                        if diff < best_plus_diff:
                            best_plus_diff, best_idx_plus = diff, i
                    else:  # offered_sum < needs_today
                        if diff < best_minus_diff:
                            best_minus_diff, best_idx_minus = diff, i
                ...
                if best_idx_plus != -1 and best_plus_diff <= self._threshold:
                    ...
                    best_idx_minus != -1
                ):  # Otherwise, take the best one that is less than needs_today / 否则，选择小于今日需求的最佳组合
                    chosen_combination_idx = best_idx_minus

                if chosen_combination_idx != -1:
                    accept_partner_ids_current_step = all_combinations[
                        chosen_combination_idx
                    ]
                    for p_id in partner_list_powerset:
                        if p_id in responses:
                            continue  # Already handled (e.g. future offer accepted/rejected) / 已处理 (例如，未来报价已接受/拒绝)
                        if p_id in accept_partner_ids_current_step:
                            ...
                            if self.is_supplier(
                                p_id
                            ):  # I buy from p_id / 我从 p_id 购买
                                temp_offers = self.future_supply_offer([p_id])
                                if p_id in temp_offers:
                                    future_prop = temp_offers[p_id]
                            elif self.is_consumer(
                                p_id
                            ):  # I sell to p_id / 我向 p_id 销售
                                temp_offers = self.future_consume_offer([p_id])
                                if p_id in temp_offers:
                                    future_prop = temp_offers[p_id]
                            responses[p_id] = SAOResponse(
                                ResponseType.REJECT_OFFER, future_prop
                            )
            ...
            current_step_combo_was_accepted = (
                chosen_combination_idx != -1
                and needs_today > 0
                and accept_partner_ids_current_step
            )

            if not current_step_combo_was_accepted and needs_today > 0:
                partners_for_fallback = {
                    p_id
                    for p_id in all_partners_on_this_side
                    if p_id in self.negotiators
                    and p_id in offers
                    and p_id not in responses
                }
                if partners_for_fallback:
                    dist_fallback = self.distribute_todays_needs(
                        list(partners_for_fallback)
                    )
                    fb_future_supply, fb_future_consume = [], []

                    for p_id, q_assigned in dist_fallback.items():
                        if p_id not in partners_for_fallback or p_id in responses:
                            continue
                        if q_assigned > 0:
                            price = self.price(
                                p_id
                            )  # Use concessional price / 使用让步价格
                            counter = (
                                self._create_offer(
                                    q_assigned, self.awi.current_step, price
                                )
                                if price
                                else None
                            )
                    ...
                    for p_id, offer_d in self.future_supply_offer(
                        fb_future_supply
                    ).items():
                        if p_id in partners_for_fallback and p_id not in responses:
                            responses[p_id] = SAOResponse(
                                ResponseType.REJECT_OFFER, offer_d
                            )
                    for p_id, offer_d in self.future_consume_offer(
                        fb_future_consume
                    ).items():
                        if p_id in partners_for_fallback and p_id not in responses:
                            responses[p_id] = SAOResponse(
    ...
    def is_valid_price(self, price: float | int, partner_id: str) -> bool:
        ...
        if self.is_consumer(
            partner_id
        ):  # Partner is my consumer (I sell to them). `price` is what they offer to pay me.
            # 伙伴是我的客户 (我卖给他们)。`price` 是他们愿意支付给我的价格。
            return (
                price >= min_p
            )  # Penguin: accept if their buy price is not less than absolute min. / Penguin: 如果他们的购买价格不低于绝对最小值，则接受。
        elif self.is_supplier(
            partner_id
        ):  # Partner is my supplier (I buy from them). `price` is what they offer to sell at.
            # 伙伴是我的供应商 (我从他们那里购买)。`price` 是他们出售的价格。
            return (
                price <= max_p
            )  # Penguin: accept if their sell price is not more than absolute max. / Penguin: 如果他们的销售价格不高于绝对最大值，则接受。
        return False
    ...
    def needs_at(self, step: int, partner_id: str) -> int:
        ...
        这用于评估未来报价。
        """
        if not self.awi or step < 0 or step >= self.awi.n_steps:
            return 0
        ...
        if self.is_supplier(
            partner_id
        ):  # Partner is my supplier, I need to buy from them (input materials)
            ...
            needed_quantity = int(
                ...
                - self.awi.total_supplies_at(
                    step
                )  # Already contracted supplies for that future step / 该未来步骤已签约的供应量
            )
        elif self.is_consumer(
            partner_id
        ):  # Partner is my consumer, I need to sell to them (output products)
            # 伙伴是我的客户，我需要卖给他们 (输出产品)
            available_to_sell = (
                daily_production + self.awi.current_inventory_input
            )  # Penguin's logic for available amount / Penguin 计算可用量的逻辑
            potential_sales_capacity = min(
                self.awi.n_lines, available_to_sell
            )  # Capped by number of lines / 受生产线数量限制
            needed_quantity = int(
                potential_sales_capacity
                - self.awi.total_sales_at(
                    step
                )  # Already contracted sales for that step / 该步骤已签约的销售量
            )
    ...
    def distribute_todays_needs(
        ...
        target_partners = (
            list(partners) if partners is not None else list(self.negotiators.keys())
        )
        if not target_partners:
            return {}

        response: dict[str, int] = dict(
            zip(target_partners, repeat(0))
        )  # Initialize all to 0 / 全部初始化为 0

        supply_partners_dist: list[str] = [
            p for p in target_partners if self.is_supplier(p)
        ]  # I buy from these / 我从这些伙伴购买
        consume_partners_dist: list[str] = [
            p for p in target_partners if self.is_consumer(p)
        ]  # I sell to these / 我向这些伙伴销售
        ...
        # 计算今天所需的净供应量
        supplies_needed_today = max(
            0,
            int(
                daily_production
                - self.awi.current_inventory_input
                - self.awi.total_supplies_at(current_step)
            ),
        )
        # Calculate net sales capacity for today
        # 计算今天的净销售能力
        sales_capacity_today = max(
            0,
            int(
                min(
                    self.awi.n_lines,
                ...
                - self.awi.total_sales_at(current_step)
            ),
        )

        if supply_partners_dist and supplies_needed_today > 0:
            dist = self._distribute_among_subset(
                supply_partners_dist, supplies_needed_today
            )
        ...
        if consume_partners_dist and sales_capacity_today > 0:
            ...
            dist = self._distribute_among_subset(
                consume_partners_dist, sales_capacity_today
            )
            response.update(dist)
        return response

    def _distribute_among_subset(
        self, Wpartners: list[str], total_quantity: int
    ) -> dict[str, int]:
        """
        ...
        """
        if not Wpartners or total_quantity <= 0:
            return {}

        shuffled_partners = random.sample(
            Wpartners, len(Wpartners)
        )  # Shuffle for randomness / 打乱以实现随机性
        # 使用 self._ptoday 来决定选择多少伙伴
        num_to_select = max(
            1, int(self._ptoday * len(shuffled_partners))
        )  # Number of partners to select / 选择的伙伴数量
        ...
        if (
            total_quantity < n_selected and total_quantity > 0
        ):  # total_quantity > 0 确保不会因 total_quantity=0 而进入此逻辑
            num_final = random.randint(
                1, total_quantity
            )  # Select between 1 and total_quantity partners / 选择 1 到 total_quantity 个伙伴
            selected_partners = random.sample(
                selected_partners, num_final
            )  # Randomly pick from the already selected subset / 从已选子集中随机挑选
            n_selected = len(selected_partners)

        if (
            n_selected == 0 and total_quantity > 0 and Wpartners
        ):  # 如果没有选出伙伴但仍有需求且有可选伙伴，则至少选一个
            selected_partners = random.sample(Wpartners, 1)
            n_selected = 1

        if n_selected > 0:
            quantities_list = distribute(
                total_quantity, n_selected
            )  # Use the utility to distribute / 使用实用函数进行分配
            return dict(zip(selected_partners, quantities_list))
        return {}

    def _generate_future_offers_for_group(
        self,
        partners_group: list[str],
        future_step: int,
        total_need_at_future_step: int,
        is_buying: bool,
        ...
        for partner_id, q in zip(partners_group, quantities):
            if q > 0:
                price = self.best_price(
                    partner_id
                )  # Use my best price for initial future offers / 对初始未来报价使用我的最优价格
                if price is not None:
                    proposals[partner_id] = self._create_offer(q, future_step, price)
        return proposals

    def future_supply_offer(
        self, partners_list: list[str]
    ) -> dict[str, Outcome | None]:
        """
        ...
        """
        proposals: dict[str, Outcome | None] = {}
        if not self.awi or not partners_list:
            return proposals
        ...
        # 将伙伴大致分为三组用于未来步骤
        p_groups = [
            partners_list[: int(len_p * 0.5)],
            partners_list[int(len_p * 0.5) : int(len_p * 0.8)],
        ...
        base_production_need = (
            self.awi.n_lines * self._productivity - self.awi.current_inventory_input
        )

        for i, step_offset in enumerate(
            [1, 2, 3]
        ):  # For s+1, s+2, s+3 / 对于 s+1, s+2, s+3
            future_s = s + step_offset
            if (
                future_s < n_steps and p_groups[i]
            ):  # If valid future step and partners exist for this group / 如果是有效的未来步骤且该组存在伙伴
                # Total need for supplies at future_s, then take 1/3rd (Penguin's heuristic)
                # future_s 的总供应需求，然后取1/3 (Penguin 的启发式方法)
                need_at_fut_s_total = base_production_need - self.awi.total_supplies_at(
                    future_s
                )
                need_for_this_group = max(
                    0, int(need_at_fut_s_total / 3.0)
                )  # Distribute 1/3 of the need / 分配1/3的需求
                proposals.update(
                    self._generate_future_offers_for_group(
                        p_groups[i], future_s, need_for_this_group, True
                    )
                )
        return proposals

    def future_consume_offer(
        self, partners_list: list[str]
    ) -> dict[str, Outcome | None]:
        """
        ...
        for i, step_offset in enumerate([1, 2, 3]):
            ...
            if future_s < n_steps and p_groups[i]:
                ...
                sales_cap_at_fut_s_total = min(
                    self.awi.n_lines, base_available_to_sell
                ) - self.awi.total_sales_at(future_s)
                ...
                if (
                    self.awi.total_sales_at(future_s) <= self.awi.n_lines
                ):  # Keep explicit check if vital / 如果至关重要则保留显式检查
                    proposals.update(
                        self._generate_future_offers_for_group(
                            p_groups[i], future_s, cap_for_this_group, False
                        )
                    )
        return proposals
    ...
    def best_price(self, partner_id: str) -> float | int | None:
        ...
        if not nmi or not nmi.issues:
            return None
        try:
            if nmi.issues[UNIT_PRICE] is None:
                return None
        ...
        if self.is_supplier(
            partner_id
        ):  # Partner supplies to me (I buy) / 伙伴向我供应 (我购买)
            return min_p
        elif self.is_consumer(
            partner_id
        ):  # Partner consumes from me (I sell) / 伙伴从我处消费 (我销售)
            return max_p
        return None
    ...
    def price(self, partner_id: str) -> float | int | None:
        ...
        if self.is_supplier(
            partner_id
        ):  # Partner supplies to me (I buy). My best is min_p. Concede higher.
            # 伙伴向我供应 (我购买)。我的最优价是 min_p。让步到更高价格。
            return min(
                min_p * 1.2, max_p
            )  # Concede up to 20% more, capped by max_p / 最多让步20%，但不超过 max_p
        elif self.is_consumer(
            partner_id
        ):  # Partner consumes from me (I sell). My best is max_p. Concede lower.
            # 伙伴从我处消费 (我销售)。我的最优价是 max_p。让步到更低价格。
            return max(
                max_p * 0.7, min_p
            )  # Concede down to 30% less, floored by min_p / 最多让步30%，但不低于 min_p
        return None
