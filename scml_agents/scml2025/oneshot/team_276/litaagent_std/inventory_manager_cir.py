...
class InventoryManagerCIR:
    def __init__(
        ...
        self.processing_cost_per_unit: float = (
            processing_cost  # Cost to convert 1 raw to 1 product
        )
        ...
        self.pending_supply_contracts: List[
            IMContract
        ] = []  # Raw materials agent will receive
        self.pending_demand_contracts: List[
            IMContract
        ] = []  # Products agent must deliver
    ...
    def add_transaction(self, contract: IMContract) -> bool:
        if contract.delivery_time < self.current_day:
            if os.path.exists("env.test") and not self.is_deepcopy:
                pass  # print(f"Warning (add_transaction): Attempting to add past contract: {contract.contract_id} for day {contract.delivery_time} when current day is {self.current_day}")
            return False

        if contract.type == IMContractType.SUPPLY:
            if contract.material_type != MaterialType.RAW:
                if os.path.exists("env.test") and not self.is_deepcopy:
                    pass  # print(f"Warning (add_transaction): Supply contract {contract.contract_id} has incorrect material type {contract.material_type}. Expected RAW.")
                return False
            ...
            if contract.material_type != MaterialType.PRODUCT:
                if os.path.exists("env.test") and not self.is_deepcopy:
                    pass  # print(f"Warning (add_transaction): Demand contract {contract.contract_id} has incorrect material type {contract.material_type}. Expected PRODUCT.")
                return False
            self.pending_demand_contracts.append(contract)
            self.pending_demand_contracts.sort(key=lambda c: c.delivery_time)
        else:
            if os.path.exists("env.test") and not self.is_deepcopy:
                pass  # print(f"Warning (add_transaction): Unknown contract type for contract {contract.contract_id}")
            return False
    ...
    def get_inventory_summary(self, day: int, mtype: MaterialType) -> Dict[str, int]:
        ...
        batches_to_consider = (
            self.raw_material_batches
            if mtype == MaterialType.RAW
            else self.product_batches
        )
        storage_cost_per_unit = (
            self.raw_storage_cost_per_unit_per_day
            if mtype == MaterialType.RAW
            else self.product_storage_cost_per_unit_per_day
        )
        ...
        for batch in batches_to_consider:
            if batch.arrival_or_production_time < day:
                ...
                cost_for_batch = (
                    batch.unit_cost * batch.remaining_quantity
                    + storage_cost_per_unit
                    * storage_duration
        ...
        average_cost_of_current_stock = (
            (total_cost_of_current_stock / current_stock_qty)
        ...
        if mtype == MaterialType.RAW:
            ...
            for prod_day in range(self.current_day, day + 1):
                ...
                if (
                    current_stock_qty == 0
                ):  # Simplified: only estimate if no current stock
                    ...
                    cost_of_future_qty_added += planned_prod * (
                        proxy_raw_cost + self.processing_cost_per_unit
                    )
        ...
        if estimated_available_qty > 0:
            if (
                current_stock_qty > 0
            ):  # If there's current stock, its avg cost is a good base
                # Refined: weighted average of current stock cost and future items cost
                total_estimated_qty_for_cost = (
                    current_stock_qty + future_qty_added_for_estimation
                )
                if total_estimated_qty_for_cost > 0:
                    estimated_average_cost_val = (
                        total_cost_of_current_stock + cost_of_future_qty_added
                    ) / total_estimated_qty_for_cost
                else:  # Should not happen if estimated_available_qty > 0
                    estimated_average_cost_val = 0.0
            elif (
                future_qty_added_for_estimation > 0
            ):  # No current stock, but future items
                estimated_average_cost_val = (
                    cost_of_future_qty_added / future_qty_added_for_estimation
                )
            ...
            if contract.delivery_time == day_being_processed:
                if contract.material_type != MaterialType.RAW:
                    if os.path.exists("env.test") and self.is_deepcopy == False:
                        pass  # print(f"Warning (_receive_materials): Supply contract {contract.contract_id} for day {day_being_processed} has incorrect material type {contract.material_type}. Skipping.")
                    continue
                ...
                if os.path.exists("env.test") and self.is_deepcopy == False:
                    pass  # print(f"Info (_receive_materials): Received {contract.quantity} of RAW material from {contract.partner_id} on day {day_being_processed}.")
    ...
    def _execute_production(self, day_being_processed: int):
        ...
        available_raw_material = sum(
            b.remaining_quantity for b in self.raw_material_batches
        )

        actual_produced_qty = min(
            planned_qty_to_produce,
        ...
        if actual_produced_qty < planned_qty_to_produce:
            if os.path.exists("env.test") and self.is_deepcopy == False:
                pass  # print(f"Warning (_execute_production): Production shortfall on day {day_being_processed}. Planned: {planned_qty_to_produce}, Actual: {actual_produced_qty}. Available Raw: {available_raw_material}, Daily Capacity: {self.daily_production_capacity}")
        ...
        qty_to_consume_for_production = (
            actual_produced_qty  # 1 unit of raw makes 1 unit of product
        )
        ...
        for i, batch in enumerate(self.raw_material_batches):
            ...
            consume_from_this_batch = min(
                qty_to_consume_for_production, batch.remaining_quantity
            )
        ...
        production_cost_per_unit = (
            (consumed_raw_material_cost / actual_produced_qty)
            + self.processing_cost_per_unit
            if actual_produced_qty > 0
            else self.processing_cost_per_unit
        )
        ...
        if os.path.exists("env.test") and self.is_deepcopy == False:
            pass  # print(f"Info (_execute_production): Produced {actual_produced_qty} of PRODUCT on day {day_being_processed}.")
    ...
    def _deliver_products(self, day_being_processed: int):
        ...
        for i, contract in enumerate(self.pending_demand_contracts):
            if contract.delivery_time == day_being_processed:
                if contract.material_type != MaterialType.PRODUCT:
                    if os.path.exists("env.test") and self.is_deepcopy == False:
                        pass  # print(f"Warning (_deliver_products): Demand contract {contract.contract_id} for day {day_being_processed} has incorrect material type {contract.material_type}. Skipping.")
                    continue

                qty_to_deliver = contract.quantity
                available_product_stock = sum(
                    b.remaining_quantity for b in self.product_batches
                )
                ...
                if actual_delivered_qty < qty_to_deliver:
                    if os.path.exists("env.test") and self.is_deepcopy == False:
                        pass  # print(f"Warning (_deliver_products): Delivery shortfall for contract {contract.contract_id} on day {day_being_processed}. Needed: {qty_to_deliver}, Delivered: {actual_delivered_qty}. Available Stock: {available_product_stock}")
                ...
                if actual_delivered_qty <= 0:
                    processed_contracts_indices.append(
                        i
                    )  # Still mark as processed for the day
                    continue
                ...
                for batch_idx, batch in enumerate(self.product_batches):
                    ...
                    deliver_from_this_batch = min(
                        actual_delivered_qty - qty_fulfilled_for_contract,
                        batch.remaining_quantity,
                    )
                ...
                processed_contracts_indices.append(i)
                if os.path.exists("env.test") and self.is_deepcopy == False:
                    pass  # print(f"Info (_deliver_products): Delivered {actual_delivered_qty} of PRODUCT for contract {contract.contract_id} on day {day_being_processed}.")
    ...
    def process_day_end_operations(self, day_being_processed: int):
        if day_being_processed != self.current_day:
            if os.path.exists("env.test") and self.is_deepcopy == False:
                pass  # print(f"Warning (process_day_end_operations): Attempting to process day {day_being_processed} but current_day is {self.current_day}. This might indicate a simulation flow error.")
        ...
        if os.path.exists("env.test") and self.is_deepcopy == False:
            pass  # print(f"--- Day {day_being_processed} processed. CIM is now at the start of Day {self.current_day} ---")
    ...
    def get_today_insufficient_raw(self, day: int) -> int:
        ...
        raw_stock_today = self.get_inventory_summary(day, MaterialType.RAW)[
            "current_stock"
        ]
    ...
    def get_total_insufficient_raw(self, target_day: int, horizon: int) -> int:
        ...
        for (
            s_contract
        ) in self.pending_supply_contracts:  # Future stock arriving before target_day
        ...
        for i in range(horizon):
            ...
            for s_contract in self.pending_supply_contracts:
                if (
                    s_contract.delivery_time == d
                ):  # Arriving exactly on this simulated day 'd'
                    simulated_raw_stock += s_contract.quantity
            ...
            if production_needed > 0:
                if simulated_raw_stock >= production_needed:
                    ...
                    simulated_raw_stock = (
                        0  # All stock used up for this day's production
                    )
    ...
    def plan_production(self, up_to_day: Optional[int] = None):
        ...
        if up_to_day is None:
            up_to_day = (
                self.max_simulation_day - 1
            )  # max_simulation_day 是总步数 (e.g., 50), 所以最后一天索引是 max_simulation_day - 1 / max_simulation_day is the total number of steps (e.g., 50), so the last day index is max_simulation_day - 1
        ...
        for day_to_clear in range(
            self.current_day, up_to_day + 1
        ):  # up_to_day is inclusive
        ...
        for contract in self.pending_demand_contracts:
            ...
            if (
                self.current_day <= contract.delivery_time <= up_to_day
            ):  # up_to_day is inclusive
                demands_by_day[contract.delivery_time] = (
                    demands_by_day.get(contract.delivery_time, 0) + contract.quantity
                )
        ...
        for s_contract in self.pending_supply_contracts:
            if self.current_day <= s_contract.delivery_time <= up_to_day:
                ...
                simulated_raw_stock_by_day[delivery_day] = (
                    simulated_raw_stock_by_day.get(delivery_day, 0)
                    + s_contract.quantity
                )
        ...
        for day in range(self.current_day + 1, up_to_day + 1):
            ...
            simulated_raw_stock_by_day[day] += simulated_raw_stock_by_day.get(
                day - 1, 0
            )
        ...
        for demand_delivery_day in sorted_demand_delivery_days:
            ...
            for prod_day in range(demand_delivery_day, self.current_day - 1, -1):
                ...
                capacity_before_this_slice = (
                    self.daily_production_capacity
                    - self.production_plan.get(prod_day, 0)
                )
                ...
                can_plan_on_prod_day = min(
                    potential_to_plan, capacity_before_this_slice, raw_before_this_slice
                )
                ...
                if can_plan_on_prod_day < potential_to_plan and potential_to_plan > 0:
                    ...
                    is_capacity_bottleneck = (
                        capacity_before_this_slice <= raw_before_this_slice
                        and capacity_before_this_slice < potential_to_plan
                    )
                    is_raw_bottleneck = (
                        raw_before_this_slice < capacity_before_this_slice
                        and raw_before_this_slice < potential_to_plan
                    )
                    # This covers the case where both are equal and limiting
                    is_both_equally_limiting = (
                        raw_before_this_slice == capacity_before_this_slice
                        and raw_before_this_slice < potential_to_plan
                    )

                    if is_both_equally_limiting:
                        bottleneck_details_for_this_demand.append(
                            f"Day {prod_day}:Cap&Raw_Limit({raw_before_this_slice:.0f})"
                        )
                    elif is_capacity_bottleneck:
                        bottleneck_details_for_this_demand.append(
                            f"Day {prod_day}:Cap_Limit({capacity_before_this_slice:.0f})"
                        )
                    elif is_raw_bottleneck:
                        bottleneck_details_for_this_demand.append(
                            f"Day {prod_day}:Raw_Limit({raw_before_this_slice:.0f})"
                        )
                ...
                if can_plan_on_prod_day > 0:
                    ...
                    self.production_plan[prod_day] = (
                        self.production_plan.get(prod_day, 0) + can_plan_on_prod_day
                    )
                    ...
                    for future_day in range(prod_day + 1, up_to_day + 1):
                        if future_day in simulated_raw_stock_by_day:
                            simulated_raw_stock_by_day[future_day] -= (
                                can_plan_on_prod_day
                            )

            if (
                remaining_to_plan_for_this_demand > 0
                and os.path.exists("env.test")
                and not self.is_deepcopy
            ):
                reason_details_str = "; ".join(
                    list(set(bottleneck_details_for_this_demand))
                )  # Use set to show unique bottleneck messages
                reason_str = ""
                if reason_details_str:
                    reason_str = (
                        f"Primary bottlenecks encountered: [{reason_details_str}]"
                    )
                else:
                ...
                if os.path.exists("env.test") and self.is_deepcopy == False:
                    pass  # print(f"Warning (plan_production): Could not fully plan for demand of {demands_by_day[demand_delivery_day]} units "
...
if __name__ == "__main__":
    if os.path.exists("env.test") and self.is_deepcopy == False:
        pass  # print("--- Starting CustomInventoryManager Part 2 Test ---")
    ...
    if os.path.exists("env.test") and self.is_deepcopy == False:
        pass  # print(f"Initialized CIM at day {cim.current_day}. Daily prod capacity: {cim.daily_production_capacity}")
    ...
    if os.path.exists("env.test") and self.is_deepcopy == False:
        pass  # print(f"Initial production plan (no demands): {cim.production_plan}")

    # Add demands. add_transaction calls plan_production() internally.
    cim.add_transaction(
        IMContract("C1", IMContractType.DEMAND, 20, 10, 2, MaterialType.PRODUCT)
    )
    cim.add_transaction(
        IMContract("C2", IMContractType.DEMAND, 15, 11, 2, MaterialType.PRODUCT)
    )
    cim.add_transaction(
        IMContract("C3", IMContractType.DEMAND, 25, 12, 1, MaterialType.PRODUCT)
    )
    cim.add_transaction(
        IMContract("C4", IMContractType.DEMAND, 40, 13, 3, MaterialType.PRODUCT)
    )

    if os.path.exists("env.test") and self.is_deepcopy == False:
        pass  # print(f"Production plan after adding demands: {dict(sorted(cim.production_plan.items()))}")
    ...
    cim.add_transaction(
        IMContract("S1_new", IMContractType.SUPPLY, 15, 5, 0, MaterialType.RAW)
    )  # 15 on Day 0

    # Process day 0 to receive S1_new and produce based on Plan[0]=10
    cim.process_day_end_operations(
        0
    )  # current_day becomes 1. S1_new (15) received. 10 produced. Raw left: 5.

    if os.path.exists("env.test") and self.is_deepcopy == False:
        pass  # print(f"CIM at start of Day {cim.current_day}")
        pass  # print(f"Raw material batches: {[(b.remaining_quantity, b.arrival_or_production_time) for b in cim.raw_material_batches]}") # Expected: [(5, 0)]
        pass  # print(f"Pending supply: {[(s.quantity, s.delivery_time, s.partner_id) for s in cim.pending_supply_contracts]}") # Expected: S2 not added yet
    cim.add_transaction(
        IMContract("S2", IMContractType.SUPPLY, 5, 5.5, 1, MaterialType.RAW)
    )  # 5 on Day 1. Plan re-calculated.
    if os.path.exists("env.test") and self.is_deepcopy == False:
        pass  # print(f"Production plan after S2: {dict(sorted(cim.production_plan.items()))}") # Should be same: {0:10, 1:30, 2:30, 3:30}
        pass  # print(f"Pending supply after S2: {[(s.quantity, s.delivery_time, s.partner_id) for s in cim.pending_supply_contracts]}")
    ...
    if os.path.exists("env.test") and self.is_deepcopy == False:
        pass  # print(f"Insufficient raw for Day 1 (production P[1]=30, stock before D1 was 5): {insufficient_day1}") # Expected: 25
    ...
    if os.path.exists("env.test") and self.is_deepcopy == False:
        pass  # print(f"Total insufficient raw from Day 1 (horizon 3): {total_insufficient_d1_h3}") # Expected: 80

    # Test with more supply
    cim.add_transaction(
        IMContract("S3", IMContractType.SUPPLY, 100, 6, 2, MaterialType.RAW)
    )  # 100 on Day 2
    if os.path.exists("env.test") and self.is_deepcopy == False:
        pass  # print(f"Production plan after S3: {dict(sorted(cim.production_plan.items()))}") # Should still be {0:10,1:30,2:30,3:30}
    ...
    total_insufficient_d1_h3_with_S3 = cim.get_total_insufficient_raw(
        target_day=1, horizon=3
    )
    if os.path.exists("env.test") and self.is_deepcopy == False:
        pass  # print(f"Total insufficient raw from Day 1 (horizon 3) with S3 added: {total_insufficient_d1_h3_with_S3}") # Expected: 20
