...
class LitaAgentCIR(StdSyncAgent):
    ...
    def __init__(
        ...
        *args,
        ...
        combo_evaluation_strategy: str = "simulated_annealing",  # 可选 "k_max", "beam_search", "simulated_annealing", "exhaustive_search" / Options: "k_max", "beam_search", "simulated_annealing", "exhaustive_search" # MODIFIED
        ...
        over_procurement_max_factor: float = 0.5,     # 最大超采购因子 (例如，最多超采50%) / Maximum over-procurement factor (e.g., at most 50% over)
        over_procurement_success_lower_bound: float = 0.7, # 采购成功率低于此值则增加因子 / Increase factor if procurement success rate is below this
        over_procurement_success_upper_bound: float = 1.2, # 采购成功率高于此值则减少因子 / Decrease factor if procurement success rate is above this
        ...
        **kwargs,
        ...
        if os.path.exists("env.test"):
            print(f"🤖 LitaAgentCIR {self.id} initialized with: \n"\
                  # ... (其他参数日志) ...
                  f"  initial_over_procurement_factor={initial_over_procurement_factor}, \n"\
                  f"  over_procurement_adjustment_rate={self.over_procurement_adjustment_rate}, \n"\
                  f"  over_procurement_min_factor={self.over_procurement_min_factor}, \n"\
                  f"  over_procurement_max_factor={self.over_procurement_max_factor}")
        ...
        self.purchase_completed: Dict[int, int] = {} # 用于记录每日实际完成的采购总量（非超采购目标）
    ...
    def init(self) -> None:
        ...
        if os.path.exists("env.test"):
            pass # print(f"🤖 {self.id} CustomIM initialized. Daily Capacity: {self.im.daily_production_capacity}, Processing Cost: {self.im.processing_cost_per_unit}, Current Day (IM): {self.im.current_day}")
    ...
    def before_step(self) -> None:
        ...
        if self.awi.is_first_level:
            ...
            if exogenous_contract_quantity > 0:
                ...
                if os.path.exists("env.test"):
                    pass # print(f"📥 Day {current_day} ({self.id}): Added exogenous SUPPLY contract {exogenous_contract_id} to IM. Qty: {exogenous_contract_quantity}, Price: {exogenous_contract_price}")
            ...
            if exogenous_contract_quantity > 0:
                ...
                self.im.add_transaction(exogenous_contract)
                if os.path.exists("env.test"):
                    pass # print(f"📤 Day {current_day} ({self.id}): Added exogenous DEMAND contract {exogenous_contract_id} to IM. Qty: {exogenous_contract_quantity}, Price: {exogenous_contract_price}")
        ...
        if os.path.exists("env.test"):
            pass # print(f"🌞 Day {current_day} ({self.id}) starting. CIM Day: {self.im.current_day}. Today Insufficient Raw: {self.today_insufficient}, Total Insufficient Raw (14d): {self.total_insufficient} (calculated AFTER exogenous contracts and subsequent plan_production)")
            pass # print(f"   Current Over-Procurement Factor: {self.over_procurement_factor:.3f}")
    ...
    def step(self) -> None:
        ...
        targeted_today = self.daily_targeted_procurement_for_adjustment.get(current_day, 0)
        achieved_today = self.daily_achieved_procurement_for_adjustment.get(current_day, 0)

        original_over_proc_factor = self.over_procurement_factor

        if targeted_today > 0:
            success_ratio = achieved_today / targeted_today
            if success_ratio < self.over_procurement_success_lower_bound: # 例如，低于70%的目标
                self.over_procurement_factor += self.over_procurement_adjustment_rate
                if os.path.exists("env.test"):
                    pass # print(f"📈 Day {current_day} ({self.id}): Procurement success ratio {success_ratio:.2f} < {self.over_procurement_success_lower_bound}. Increasing over_proc_factor.")
            elif success_ratio > self.over_procurement_success_upper_bound: # 例如，超过120%的目标
                self.over_procurement_factor -= self.over_procurement_adjustment_rate * 0.5 # 减少时幅度小一些
                if os.path.exists("env.test"):
                    pass # print(f"📉 Day {current_day} ({self.id}): Procurement success ratio {success_ratio:.2f} > {self.over_procurement_success_upper_bound}. Decreasing over_proc_factor.")

            self.over_procurement_factor = max(self.over_procurement_min_factor, min(self.over_procurement_max_factor, self.over_procurement_factor))
            if os.path.exists("env.test") and abs(self.over_procurement_factor - original_over_proc_factor) > 1e-5 :
                pass # print(f"   Over-Procurement Factor adjusted from {original_over_proc_factor:.3f} to {self.over_procurement_factor:.3f}")
        ...
        if os.path.exists("env.test"):
             pass # print(f"🌙 Day {current_day} ({self.id}) ending. Market Material Avg Price: {self._market_material_price_avg:.2f}, Market Product Avg Price: {self._market_product_price_avg:.2f}. IM is now on day {self.im.current_day}.")
    ...
    def first_proposals(self) -> Dict[str, Outcome]:
        ...
        if not self.im:
            if os.path.exists("env.test"):
                pass # print(f"Error ({self.id} @ {current_day}): InventoryManager not initialized. Cannot generate first proposals.")
            return proposals

        # --- 1. 采购原材料以满足短缺 (Procure raw materials to meet shortfall) ---
        base_target_procurement_quantity = self.total_insufficient if self.total_insufficient is not None else 0

        adjusted_target_procurement_quantity = 0
        if base_target_procurement_quantity > 0:
             adjusted_target_procurement_quantity = int(round(base_target_procurement_quantity * (1 + self.over_procurement_factor)))
             if os.path.exists("env.test"):
                 pass # print(f"Debug ({self.id} @ {current_day}): FirstProposals (Supply) - Base need: {base_target_procurement_quantity}, Over-procurement factor: {self.over_procurement_factor:.3f}, Adjusted target: {adjusted_target_procurement_quantity}")

        # 记录当日的目标采购量（用于后续调整超采购因子）
        # Record today's target procurement quantity (for later adjustment of over-procurement factor)
        self.daily_targeted_procurement_for_adjustment[current_day] = adjusted_target_procurement_quantity
        ...
        if adjusted_target_procurement_quantity > 0 and supplier_negotiators:
            ...
            if os.path.exists("env.test"):
                pass # print(f"Debug ({self.id} @ {current_day}): FirstProposals (Supply) - Adjusted total raw material need: {adjusted_target_procurement_quantity}. "\
            ...
            for nid in sorted_supplier_nids:
                ...
                if propose_q_for_this_supplier <= 0 or propose_q_for_this_supplier > remaining_procurement_need :
                    ...
                    if os.path.exists("env.test") and propose_q_for_this_supplier > 0 :
                         pass # print(f"Debug ({self.id} @ {current_day}): FirstProposals (Supply) - Skipping supplier {nid}. "\
                ...
                if self.total_insufficient and self.total_insufficient > self.im.daily_production_capacity * 2 :
                ...
                if propose_q > 0:
                    ...
                    if os.path.exists("env.test"):
                        pass # print(f"Debug ({self.id} @ {current_day}): FirstProposals (Supply) - To {nid}: Q={propose_q}, T={propose_t}, P={propose_p:.2f}. "\
                              f"Remaining need: {remaining_procurement_need}")

            if remaining_procurement_need > 0 and os.path.exists("env.test"):
                pass # print(f"Warning ({self.id} @ {current_day}): FirstProposals (Supply) - Still have {remaining_procurement_need} unmet raw material need after proposing to all suppliers.")
        ...
        if not self.awi.is_last_level:
            ...
            if estimated_sellable_quantity > 0 and consumer_negotiators:
                ...
                if os.path.exists("env.test"):
                    pass # print(f"Debug ({self.id} @ {current_day}): FirstProposals (Demand) - Estimated sellable products (horizon {sellable_horizon}d): {estimated_sellable_quantity}. "\
                ...
                for nid in sorted_consumer_nids:
                    ...
                    if propose_q_for_this_consumer <= 0 or propose_q_for_this_consumer > remaining_sellable_quantity:
                        if os.path.exists("env.test") and propose_q_for_this_consumer > 0:
                            pass # print(f"Debug ({self.id} @ {current_day}): FirstProposals (Demand) - Skipping consumer {nid}. "\
                    ...
                    if propose_q > 0:
                        ...
                        if os.path.exists("env.test"):
                             pass # print(f"Debug ({self.id} @ {current_day}): FirstProposals (Demand) - To {nid}: Q={propose_q}, T={propose_t}, P={propose_p:.2f}. "\
                                   f"Remaining sellable: {remaining_sellable_quantity}")

                if remaining_sellable_quantity > 0 and os.path.exists("env.test"):
                    pass # print(f"Warning ({self.id} @ {current_day}): FirstProposals (Demand) - Still have {remaining_sellable_quantity} sellable products after proposing to all consumers.")
            elif os.path.exists("env.test") and consumer_negotiators:
                 pass # print(f"Debug ({self.id} @ {current_day}): FirstProposals (Demand) - No estimated sellable quantity or no consumers to propose to for sales.")

        if os.path.exists("env.test"):
            pass # print(f"Making first proposals on day {current_day}, to {proposals}")
        return proposals
    ...
    def score_offers(
        ...
        for negotiator_id, offer_outcome in offer_combination.items():
            if not offer_outcome:
                if os.path.exists("env.test"):
                    pass # print(f"Warning ({self.id} @ {today}): Null offer_outcome for negotiator {negotiator_id} in combination. Skipping.")
                continue
        ...
        if score_a < 0:
            if os.path.exists("env.test"):
                pass # print(f"Warning ({self.id} @ {today}): score_a (cost_before) is negative: {score_a:.2f}. Clamping to 0.")
            score_a = 0.0
        if score_b < 0:
            if os.path.exists("env.test"):
                pass # print(f"Warning ({self.id} @ {today}): score_b (cost_after) is negative: {score_b:.2f}. Clamping to 0.")
            score_b = 0.0
        ...
        if os.path.exists("env.test"):
            ...
            pass # print(f"ScoreOffers ({self.id} @ {today}): Combo Eval: [{offers_str}]\n"\
    ...
    def calculate_inventory_cost_score(
        ...
        if os.path.exists("env.test"):
            pass # print(f"Debug (calc inventory penalty): Checking disposal for day {day_for_disposal_check}. IM state day: {sim_eval_im_for_storage.current_day}, Last simulation day (param): {last_simulation_day}")
        ...
        if os.path.exists("env.test"):
            pass # print(f"Debug (calc inventory penalty): Day {day_for_disposal_check} - RemainRaw={remain_raw:.0f}, RemainProd={remain_product:.0f}, DisposalCost={inventory_penalty:.2f}")
    ...
    def _evaluate_offer_combinations_exhaustive(
        ...
        if num_offers_available > 15 and os.path.exists("env.test"):
            pass # print(f"Warning ({self.id} @ {awi.current_step}): Exhaustive search with {num_offers_available} offers. This might be very slow.")
        ...
        if os.path.exists("env.test") and best_combination_items:
            ...
            pass # print(f"Debug ({self.id} @ {awi.current_step}): ExhaustiveSearch Best Combo (by NormScore): NIDs: {best_combo_nids_str}, "\
             ...
             pass # print(f"Debug ({self.id} @ {awi.current_step}): ExhaustiveSearch: No suitable non-empty offer combination found (by NormScore).")
    ...
    def _evaluate_offer_combinations_k_max(
        ...
        if os.path.exists("env.test") and best_combination_items:
            ...
            pass # print(f"Debug ({self.id} @ {awi.current_step}): K-Max Best Combo (by NormScore): NIDs: {best_combo_nids_str}, "\
             ...
             pass # print(f"Debug ({self.id} @ {awi.current_step}): K-Max: No suitable non-empty offer combination found (by NormScore).")
        return best_combination_items, highest_norm_score
    ...
    def _evaluate_offer_combinations_beam_search(
        ...
        if final_best_combo_dict:
            ...
            if os.path.exists("env.test"):
                 pass # print(f"Debug ({self.id} @ {awi.current_step}): BeamSearch: No suitable non-empty offer combination found (by NormScore).")
            return None, -1.0
        ...
        if os.path.exists("env.test"):
            if best_solution_dict:
                ...
                pass # print(f"Debug ({self.id} @ {self.awi.current_step}): SA Best Combo (by NormScore): NIDs: {best_combo_nids_str}, "\
                 ...
                 pass # print(f"Debug ({self.id} @ {self.awi.current_step}): SA: No suitable non-empty offer combination found (by NormScore).")
    ...
    def _evaluate_offer_combinations(
        ...
        if self.combo_evaluation_strategy == "k_max":
            ...
            if os.path.exists("env.test"):
                pass # print(f"Warning ({self.id} @ {awi.current_step}): Unknown combo_evaluation_strategy '{self.combo_evaluation_strategy}'. Defaulting to 'k_max'.")
            best_combination_items, best_norm_score = self._evaluate_offer_combinations_k_max(offers, im, awi)
        ...
        if best_combination_items: # 确保找到了一个非空的最佳组合
            ...
            if os.path.exists("env.test"):
                pass # print(f"Debug ({self.id} @ {awi.current_step}): No non-empty best combination items found by strategy '{self.combo_evaluation_strategy}'.")
            return None, -1.0, 0.0
    ...
    def _calculate_combination_profit_and_normalize(
        ...
        for negotiator_id, outcome in offer_combination.items():
            ...
            if nmi is None and os.path.exists("env.test"):  # Log if NMI was missing and fallback was used
                pass # print(f"Warning ({self.id} @ {awi.current_step}): NMI missing for {negotiator_id}. Using heuristic price bounds: min={min_est_price:.2f}, max={max_est_price:.2f}")
        ...
        if os.path.exists("env.test"):
            pass # print(f"Debug ({self.id} @ {awi.current_step}): ProfitCalcNorm[-1,1] (NMI-based) for Combo: "\
    ...
    def _generate_counter_offer(
        ...
        if not nmi:
             if os.path.exists("env.test"):
                 pass # print(f"Warning ({self.id} @ {self.awi.current_step}): NMI missing for {negotiator_id}. Cannot generate counter-offer.")
             return None
        ...
        for t_candidate in candidate_times:
            ...
            if os.path.exists("env.test"):
                pass # print(f"Debug ({self.id} @ {self.awi.current_step}): TimeEval for NID {negotiator_id}: CandT={t_candidate}, Q={new_q}, P={price_for_time_eval:.2f}, Score={current_sim_score:.3f}")
        ...
        opponent_last_price = opponent_last_offer_outcome[UNIT_PRICE] if opponent_last_offer_outcome else orig_p
        price_towards_opponent = (1.0 - self.price_concession_opponent_factor) * base_target_p + \
                                 self.price_concession_opponent_factor * opponent_last_price
        inventory_pressure = 0.0
        if optimize_for_inventory:
            if is_buying:
                raw_stock = self.im.get_inventory_summary(self.awi.current_step, MaterialType.RAW).get('current_stock', 0)
                if raw_stock < self.inventory_pressure_threshold_raw:
                    inventory_pressure = 1.0 - (raw_stock / self.inventory_pressure_threshold_raw if self.inventory_pressure_threshold_raw > 0 else 1.0)
                    inventory_pressure = max(0.0, min(1.0, inventory_pressure))
            else:
                product_stock = self.im.get_inventory_summary(self.awi.current_step, MaterialType.PRODUCT).get('current_stock', 0)
                if product_stock > self.inventory_pressure_threshold_product:
                    inventory_pressure = (product_stock - self.inventory_pressure_threshold_product) / self.inventory_pressure_threshold_product if self.inventory_pressure_threshold_product > 0 else 0.0
                    inventory_pressure = max(0.0, inventory_pressure)
        final_target_p = price_towards_opponent
        concession_magnitude = (round_concession_factor + inventory_pressure) * self.price_concession_round_factor
        if is_buying:
            max_possible_concession_price = max_p_nmi
            final_target_p = price_towards_opponent + (max_possible_concession_price - price_towards_opponent) * concession_magnitude
        else:
            min_possible_concession_price = min_p_nmi
            final_target_p = price_towards_opponent - (price_towards_opponent - min_possible_concession_price) * concession_magnitude
        if new_t != orig_t:
        ...
        if new_q == orig_q and new_t == orig_t and abs(new_p - orig_p) < 1e-5:
            if os.path.exists("env.test"):
                pass # print(f"Debug ({self.id} @ {self.awi.current_step}): Counter for {negotiator_id} resulted in same as original. No counter generated.")
            return None
        if os.path.exists("env.test"):
             pass # print(f"Debug ({self.id} @ {self.awi.current_step}): Generated Counter for {negotiator_id}: Q={new_q}, T={new_t}, P={new_p:.2f}. Orig P={orig_p:.2f}. OptInv={optimize_for_inventory}, OptProf={optimize_for_profit}")
    ...
    def counter_all(
        ...
        if not self.im or not self.awi:
            if os.path.exists("env.test"):
                pass # print(f"Error ({self.id} @ {self.awi.current_step}): IM or AWI not initialized. Rejecting all offers.")
        ...
        for nid, offer_outcome in offers.items():
            if offer_outcome:
                ...
                if os.path.exists("env.test"):
                    pass # print(f"Debug ({self.id} @ {self.awi.current_step}): Recorded offer from {nid}: Q={offer_outcome[QUANTITY]}, T={offer_outcome[TIME]}, P={offer_outcome[UNIT_PRICE]:.2f}")
        ...
        if os.path.exists("env.test"):
            pass # print(f"CounterAll ({self.id} @ {current_day}): Dynamic Thresholds: p_thresh={dynamic_p_threshold:.3f}, q_thresh={dynamic_q_threshold:.3f} (Base: p={self.p_threshold}, q={self.q_threshold}, DecayFactor Applied: {time_decay_factor:.3f})")
        ...
        if os.path.exists("env.test"):
            ...
            pass # print(f"CounterAll ({self.id} @ {self.awi.current_step}): Best combo NIDs: {nids_in_best_str}, norm_score: {norm_score:.3f}, norm_profit: {norm_profit:.3f}")
        ...
        if norm_score > dynamic_p_threshold and norm_profit > dynamic_q_threshold:
            ...
            # 默认情况下，所有报价都将保持被拒绝状态。
            if os.path.exists("env.test"):
                pass # print(f"Warning ({self.id} @ {self.awi.current_step}): counter_all logic fell through. All offers rejected by default.")
    ...
    def on_negotiation_success(self, contract: Contract, mechanism: StdAWI) -> None:
        ...
        self.today_insufficient = self.im.get_today_insufficient_raw(current_day)
        self.total_insufficient = self.im.get_total_insufficient_raw(current_day, horizon=14)

        # 更新每日采购/销售完成量
        if is_supply:  # 原材料采购合同
            self.purchase_completed[current_day] = self.purchase_completed.get(current_day, 0) + agreement["quantity"]
            # 更新用于调整超采购因子的实际达成量
            # Update actual achieved quantity for over-procurement factor adjustment
            self.daily_achieved_procurement_for_adjustment[
                current_day] = self.daily_achieved_procurement_for_adjustment.get(current_day, 0) + agreement[
                "quantity"]

        elif not is_supply:  # 产成品销售合同
            self.sales_completed[current_day] = self.sales_completed.get(current_day, 0) + agreement["quantity"]

        if os.path.exists("env.test"):
            pass # print(f"✅ [{current_day}] ({self.id}) Contract {contract.id} added to IM: {new_c}")
            if is_supply:
                print(
                    f"   Updated daily_achieved_procurement_for_adjustment[{current_day}]: {self.daily_achieved_procurement_for_adjustment.get(current_day, 0)}")
