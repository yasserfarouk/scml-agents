...
class TestCustomInventoryManager(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        self.assertEqual(
            self.im.raw_storage_cost_per_unit_per_day, self.raw_storage_cost
        )
        self.assertEqual(
            self.im.product_storage_cost_per_unit_per_day, self.product_storage_cost
        )
    ...
    def test_add_supply_contract(self):
        """测试添加供应合同"""
        contract = IMContract(
            "supply1", "partnerA", IMContractType.SUPPLY, 100, 5.0, 5, MaterialType.RAW
        )
    ...
    def test_add_demand_contract(self):
        ...
        self.assertGreaterEqual(
            planned_qty_for_demand, 30, "Production plan should cover the demand"
        )

    def test_add_contract_past_date(self):
        """测试添加交货日期已过的合同"""
        self.im.current_day = 2
        past_contract = IMContract(
            "past_supply",
            "partnerC",
        ...
        self.assertFalse(
            self.im.add_transaction(past_contract),
            "Should not add contract with past delivery date",
        )
    ...
    def test_plan_production_capacity_constrained(self):
        ...
        demand_contract = IMContract(
            "demand_cap", "c1", IMContractType.DEMAND, 25, 30.0, 2, MaterialType.PRODUCT
        )
    ...
    def test_execute_production_sufficient_raw(self):
        ...
        self.im.raw_material_batches.append(
            InventoryBatch(20, 1.0, day_to_produce - 1)
        )  # 假设前一天就有
        ...
        self.assertAlmostEqual(
            self.im.product_batches[0].unit_cost, expected_product_cost
        )
    ...
    def test_deliver_products_sufficient_stock(self):
        ...
        self.im.add_transaction(
            demand_contract
        )  # 不会再触发plan_production，因为库存已模拟存在
    ...
    def test_deliver_products_insufficient_stock(self):
        """测试库存不足时的产品交付（部分交付）"""
        day_to_deliver = 3
        self.im.product_batches.append(
            InventoryBatch(5, 15.0, day_to_deliver - 1)
        )  # 只有5单位库存
        demand_contract = IMContract(
            "demand_part_deliv",
            "c3",
        ...
        self.assertNotIn(
            demand_contract,
            self.im.pending_demand_contracts,
    ...
    def test_process_day_end_operations_full_cycle(self):
        """测试完整的日终操作流程"""
        # Day 0: 添加供应和需求
        supply_day1 = IMContract(
            "s_eod", "sup_eod", IMContractType.SUPPLY, 20, 2.0, 1, MaterialType.RAW
        )
        demand_day1 = IMContract(
            "d_eod", "cus_eod", IMContractType.DEMAND, 10, 20.0, 1, MaterialType.PRODUCT
        )
        self.im.add_transaction(supply_day1)
        self.im.add_transaction(
            demand_day1
        )  # 这会触发plan_production, 计划在第1天生产10

        self.assertEqual(
            self.im.production_plan.get(1, 0), 10, "Should plan to produce 10 on day 1"
        )
        ...
        # 1. 原材料接收
        self.assertEqual(len(self.im.pending_supply_contracts), 0)
        raw_summary_after_d1 = self.im.get_inventory_summary(
            1, MaterialType.RAW
        )  # 查看第1天结束时的状态
        # 20单位原料进来，生产用掉10单位
        self.assertEqual(
            raw_summary_after_d1["current_stock"],
            10,
            "Raw stock should be 10 after production",
        )
        ...
        # 生产10单位，交付10单位
        self.assertEqual(
            prod_summary_after_d1["current_stock"],
            0,
            "Product stock should be 0 after delivery",
        )
        ...
        self.assertEqual(
            self.im.production_plan.get(2, 0),
            0,
    ...
    def test_get_inventory_summary_costs_and_values(self):
        ...
        self.assertAlmostEqual(
            raw_summary0["estimated_average_cost"], 2.5
        )  # (10*2 + 10*3) / 20
        ...
        self.assertAlmostEqual(
            prod_summary0["estimated_average_cost"], expected_prod_cost
        )
        ...
        self.assertEqual(
            raw_summary_day1_start["current_stock"], 8
        )  # 剩余第二批的8单位
        self.assertAlmostEqual(
            raw_summary_day1_start["estimated_average_cost"], 3.0
        )  # 剩余都是3元成本的
        ...
        self.assertAlmostEqual(
            prod_summary_day1_start["estimated_average_cost"],
            (10 * (2.0 + self.processing_cost) + 2 * (3.0 + self.processing_cost))
            / 12.0,
        )
    ...
    def test_get_insufficient_raw(self):
        ...
        self.assertEqual(
            self.im.get_total_insufficient_raw(0, horizon=1), 30
        )  # horizon 1 = today only
        self.assertEqual(
            self.im.get_total_insufficient_raw(0, horizon=2), 50
        )  # today + next day
        ...
        self.assertEqual(
            self.im.get_total_insufficient_raw(0, horizon=2), 40
        )  # (30-10) for day 0 + 20 for day 1

    def test_get_available_production_capacity(self):
        """测试可用生产能力的计算"""
        self.assertEqual(
            self.im.get_available_production_capacity(0), self.daily_capacity
        )
        self.im.production_plan[0] = 10
        self.assertEqual(
            self.im.get_available_production_capacity(0), self.daily_capacity - 10
        )
        self.im.production_plan[0] = (
            self.daily_capacity + 5
        )  # 超额计划 (plan_production应该避免，但测试_get_available...)
        self.assertEqual(self.im.get_available_production_capacity(0), 0)  # 不应为负

    def test_deepcopy_inventory_manager(self):
        """测试库存管理器的深拷贝功能"""
        self.im.add_transaction(
            IMContract("s1", "p1", IMContractType.SUPPLY, 10, 1, 1, MaterialType.RAW)
        )
        ...
        self.assertIsNot(
            self.im.pending_supply_contracts, im_copy.pending_supply_contracts
        )
        ...
        # 验证内容相同
        self.assertEqual(
            len(self.im.pending_supply_contracts), len(im_copy.pending_supply_contracts)
        )
        self.assertEqual(
            self.im.pending_supply_contracts[0].id,
            im_copy.pending_supply_contracts[0].id,
        )
        self.assertEqual(
            len(self.im.raw_material_batches), len(im_copy.raw_material_batches)
        )
        self.assertEqual(
            self.im.raw_material_batches[0].quantity,
            im_copy.raw_material_batches[0].quantity,
        )

        # 修改原对象，拷贝不应受影响
        self.im.add_transaction(
            IMContract("s2", "p2", IMContractType.SUPPLY, 20, 2, 2, MaterialType.RAW)
        )
        self.im.raw_material_batches[0].quantity = 99

        self.assertEqual(len(im_copy.pending_supply_contracts), 1)  # 拷贝仍为1个合同
        self.assertEqual(
            im_copy.raw_material_batches[0].quantity, 5
        )  # 拷贝的批次数量不变
