...
# required for typing
from negmas import Contract, SAOResponse
import random
import math
...
class EpsilonGreedyAgent(OneShotSyncAgent):
    ...
    def before_step(self):
        if len(self._first_proposal_accepted_ratio) == 0:
            if self.awi.is_first_level:
                ...
                self._first_proposal_accepted_ratio = {k : 0 for k in self.awi.my_consumers}

            if self.awi.is_last_level:
                self._first_proposal_accepted = {k : 0 for k in self.awi.my_suppliers}
                self._first_proposal_accepted_ratio = {k : 0 for k in self.awi.my_suppliers}
    ...
    def distribute_needs_smartly(self) -> dict[str, int]:
        ...
        for i,(needs, all_partners) in enumerate([
            ...
            accepted_ratio_list = [self._first_proposal_accepted_ratio[k] for k in partner_ids]
            accepted_ratio_sum = sum(accepted_ratio_list)
            # ゼロ除算回避
            if accepted_ratio_sum == 0:
                accepted_ratio_sum = 1
            # 最低1は入れる
            accepted_ratio_list = [max(1 , math.ceil(needs*ratio/accepted_ratio_sum)) for ratio in accepted_ratio_list]
    ...
    def counter_all(self, offers, states):
        ...
        for needs, all_partners, issues in [
            ...
            for i, partner_ids in enumerate(plist):
                ...
                # ec -> estimated_cost
                log_message(f"estimated_cost: {estimated_cost:>5.3f} utility: {utility_value:>8.3f} shortfall: {shortfall} offers",str([offers[_] for _ in partner_ids]))
            ...
            if len(others) == 0 or (current_time >time_threshold and len(partner_ids) >0):
                if len(others) == 0:
                    log_message("agreement with all partners")
                elif current_time > time_threshold:
                    log_message("time is over. So accept or end negotiation with all offers.")
                else:
                ...
                if excess_needs < 0 :
                    ...
                    distribution = dict(zip(others, distribute(int(excess_needs), len(others))))
                    #log_message("distribution!!!!!!!!!!!!!!!!:",distribution)
    ...
    def step(self):
        ...
        for partner_id, one_if_accepted in self._first_proposal_accepted.items():
            self._first_proposal_accepted_ratio[partner_id] += (one_if_accepted - self._first_proposal_accepted_ratio[partner_id])/(self.awi.current_step+1)
        ...
        log_message("first_proposal_accepted_ratio:",self._first_proposal_accepted_ratio)
        # 辞書のvalueをゼロにする
        self._first_proposal_accepted = {k : 0 for k,v in self._first_proposal_accepted.items()}
