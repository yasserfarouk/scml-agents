...
class S5s(StdAgent):
        ...
        if self.awi.is_first_level:
            ...
            for t in range(T, self.awi.n_steps):
                    if t != self.awi.n_steps-1:
                        self.raw_inventory[t] = sum(self.buy_num[:t+1])-sum(self.sell_num[:t+1])
                    else:
    ...
    def step(self):
        ...
        if self.raw_inventory[self.awi.current_step] < 0:
            self.buy_num[self.awi.current_step] += -self.raw_inventory[self.awi.current_step]
            self.buy_day[self.awi.current_step] += self.ip * -self.raw_inventory[self.awi.current_step]
    ...
    def respond(self, negotiator_id: str, state: SAOState,  source="") -> ResponseType:
        ...
        with self.used:
            ...
            # print("完成品の販売")
                ...
                #材料購入
                if sum(self.buy_num) - sum(self.sell_num) + Q  > sum(self.fac_lines_cap[self.awi.current_step:]) * 0.6:
                    return ResponseType.REJECT_OFFER
                
                for t in range(T, min(int(T + self.awi.n_steps/10),self.awi.n_steps)):
                ...
                if T < self.awi.n_steps*2/3:
                    max_q = max(self.raw_inventory[T:int(self.awi.n_steps*2/3)+1]) 
        ...
        with self.used:
            ...
            if negotiator_id in self.awi.my_suppliers:
                ...
                if need_num == 0:
                    ...
                    if need_num > 0:
                        need_num = min(need_num, self.awi.n_lines/2)
                    else:
                ...
                if self.awi.current_step < self.awi.n_steps*2/3:
                    ...
                    for time in range(self.awi.current_step, int(self.awi.current_step + self.awi.n_steps/5)):
                        if self.raw_inventory[time] == 0:
                            ...
                            emp_lines[time] = min (self.fac_lines_cap[time], self.raw_inventory[time])
                    ...
                    if q == 0:
                        return None
                    p = int(-state.step/20 * (((self.awi.catalog_prices[self.awi.my_output_product] + self.awi.trading_prices[self.awi.my_output_product])/2) - self.op)+((self.awi.catalog_prices[self.awi.my_output_product] + self.awi.trading_prices[self.awi.my_output_product])/2))
