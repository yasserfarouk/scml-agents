...
class S7s(StdSyncAgent):
        ...
        for date, o_list in buy_date.items():
            if len(o_list) == 1:
                type = self.responsses(
                    o_list[0].negotiator_id,
                    states[o_list[0].negotiator_id],
                    inventory,
                    fac_cap,
                )
                if type == ResponseType.ACCEPT_OFFER:
                    ...
                    inventory, fac_cap = self.Update_inventory(
                        o_list[0].offer, inventory, fac_cap, o_list[0].negotiator_id
                    )
                else:
                    ...
                    if new_offer is not None:
                        inventory, fac_cap = self.Update_inventory(
                            new_offer, inventory, fac_cap, o_list[0].negotiator_id
                        )
                    respond_offer[o_list[0].negotiator_id] = SAOResponse(
                ...
                for i in range(len(o_list)):
                    if (
                        self.responsses(
                            o_list[i].negotiator_id,
                            states[o_list[i].negotiator_id],
                            inventory,
                            fac_cap,
                        )
                        ...
                        for j in range(i + 1, len(o_list)):
                            new_offer = self.make_offer(
                                o_list[j].negotiator_id,
                                states[o_list[j].negotiator_id],
                                inventory,
                                fac_cap,
                            )
                            if new_offer is not None:
                        ...
                        if new_offer is not None:
                            inventory, fac_cap = self.Update_inventory(
                                new_offer, inventory, fac_cap, o_list[i].negotiator_id
                            )
                        respond_offer[o_list[i].negotiator_id] = SAOResponse(
                            ResponseType.REJECT_OFFER, new_offer
                        )
        ...
        if negotiator_id in self.awi.my_suppliers:
            ...
            if self.awi.is_last_level:
                if inventory[self.awi.current_step] < 0:
                    ...
                    return tuple([need_num, p, date_time])
    ...
    def responsses(
        ...
        if negotiator_id in self.awi.my_consumers:
            ...
            if self.awi.current_step < self.awi.n_steps * 0.7:
                if time == 0:
                    ...
                    if (
                        ...
                        < sum(fac_cap[self.awi.current_step :]) * 0.6
                    ):
                        if price > p:
                            return ResponseType.ACCEPT_OFFER
            ...
            for t in range(
                time, min(int(time + self.awi.n_steps / 5), self.awi.n_steps)
            ):
                if inventory[t] < 0:
                    if inventory[t] + quantity < need_buy:
                        if price < p:
                            return ResponseType.ACCEPT_OFFER
                    else:
            ...
            if self.sell_q_num + quantity > self.awi.n_steps * self.awi.n_lines * 0.6:
                # print("too much")
                return ResponseType.REJECT_OFFER
            if self.awi.is_last_level:
                if inventory[t] < 0:
            ...
            if time < self.awi.n_steps * 2 / 3:
                ...
                if max_q + quantity < 2 * need_buy:
                    if price < p:
                        return ResponseType.ACCEPT_OFFER
            else:
                max_q = max(inventory[time : self.awi.n_steps])
                if max_q + quantity < need_buy:
                    if price < p:
                        return ResponseType.ACCEPT_OFFER
            return ResponseType.REJECT_OFFER
    ...
    def step(self):
        if self.awi.current_step == self.awi.n_steps - 1:
            # print(self.buy_num)
            # print(self.sell_num)
            pass  # print(sum(self.buy_num)-sum(self.sell_num))
