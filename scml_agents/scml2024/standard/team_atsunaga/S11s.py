...
class S7s(StdSyncAgent):
    ...
    def first_proposals(self) -> dict[str, Outcome | None]:
        offers = {}
        self.in_scedule = copy.deepcopy(self.buy_num)
        self.out_schedule = copy.deepcopy(self.sell_num)
        for neg_id in self.awi.my_suppliers:
            new_offer = self.make_offer(neg_id, SAOState(step=0))
    ...
    def counter_all(
        ...
        for date, o_list in buy_date.items():
            if len(o_list) == 1:
                type = self.responsses(
                    o_list[0].negotiator_id, states[o_list[0].negotiator_id]
                )
                if type == ResponseType.ACCEPT_OFFER:
                    respond_offer[o_list[0].negotiator_id] = SAOResponse(
                        ResponseType.ACCEPT_OFFER, None
                    )
                    self.Update_inventory(o_list[0].offer, o_list[0].negotiator_id)
                else:
                    new_offer = self.make_offer(
                        o_list[0].negotiator_id, states[o_list[0].negotiator_id]
                    )
                    if new_offer is not None:
                        self.Update_inventory(new_offer, o_list[0].negotiator_id)
                    respond_offer[o_list[0].negotiator_id] = SAOResponse(
                ...
                for i in range(len(o_list)):
                    if (
                        self.responsses(
                            o_list[i].negotiator_id, states[o_list[i].negotiator_id]
                        )
                        == ResponseType.ACCEPT_OFFER
                    ):
                        respond_offer[o_list[i].negotiator_id] = SAOResponse(
                            ResponseType.ACCEPT_OFFER, None
                        )
                        inventory, fac_cap = self.Update_inventory(
                            o_list[0].offer, o_list[0].negotiator_id
                        )
                        for j in range(i + 1, len(o_list)):
                            new_offer = self.make_offer(
                                o_list[j].negotiator_id, states[o_list[j].negotiator_id]
                            )
                            if new_offer is not None:
                                self.Update_inventory(
                                    new_offer, o_list[j].negotiator_id
                                )
                            respond_offer[o_list[j].negotiator_id] = SAOResponse(
                                ResponseType.REJECT_OFFER, new_offer
                            )
                        break
                    else:
                        ...
                        if new_offer is not None:
                            self.Update_inventory(new_offer, o_list[i].negotiator_id)
                        respond_offer[o_list[i].negotiator_id] = SAOResponse(
                            ResponseType.REJECT_OFFER, new_offer
                        )
        ...
        for date, o_list in sell_date.items():
            if len(o_list) == 1:
                ...
                o_list = sorted(o_list, key=lambda x: x.price, reverse=True)
                for i in range(len(o_list)):
                    if (
                        self.responsses(
                            o_list[i].negotiator_id, states[o_list[i].negotiator_id]
    ...
    def Update_inventory(self, contract, neg_id) -> None:
        # 交渉が成功したときに呼び出される
    ...
    def make_offer(self, negotiator_id: str, state) -> Outcome | None:
        neg_info = None
        ...
        if negotiator_id in self.awi.my_suppliers:
            # 材料購入戦略
            need_num = 0
            if sum(self.in_scedule) - sum(self.out_schedule) > 2 * self.awi.n_lines:
                return None
            if state.step > 10:
                inde = 0
                for t_offer in neg_info.t_dif:
                    for time in range(
                        t_offer,
                        min(int(t_offer + self.awi.n_steps / 10), self.awi.n_steps - 1),
                    ):
                        if (
                            sum(self.in_scedule[: time + 1])
                            - sum(self.out_schedule[: time + 1])
                            < 0
                        ):
            ...
            if need_num == 0:
                ...
                for t in range(
                    ...
                    if (
                        sum(self.in_scedule[: t + 1]) - sum(self.out_schedule[: t + 1])
                        < 0
                    ):
                        if first_loss == 0:
                            first_loss = t
                        if need_num < -sum(self.in_scedule[: t + 1]) - sum(
                            self.out_schedule[: t + 1]
                        ):
                            need_num = -sum(self.in_scedule[: t + 1]) - sum(
                                self.out_schedule[: t + 1]
                            )
                if need_num > 0:
            ...
            if self.awi.current_step < self.awi.n_steps * 2 / 3:
                ...
                for time in range(
                    ...
                    emp_lines[time] = min(
                        self.awi.n_lines - self.out_schedule[time],
                        sum(self.in_scedule[: time + 1])
                        - sum(self.out_schedule[: time + 1]),
                    )
                ...
                for t in range(self.awi.current_step, self.awi.n_steps):
                    if (
                        ...
                        time = t
                        q = min(
                            self.awi.n_lines - self.out_schedule[t],
                            sum(self.in_scedule[: t + 1])
                            - sum(self.out_schedule[: t + 1]),
                        )
                        break
                p = int(
                    state.step
    ...
    def responsses(self, negotiator_id: str, state: SAOState) -> ResponseType:
        ...
        if negotiator_id in self.awi.my_consumers:
            ...
            p = int(
                state.step
                / 20
                * ((self.awi.catalog_prices[self.awi.my_output_product]) - self.op)
                + self.op
            )
            if self.awi.n_lines - self.out_schedule[time] < quantity:
                return ResponseType.REJECT_OFFER
            if self.awi.current_step < self.awi.n_steps * 0.7:
                ...
                elif time < self.awi.current_step + self.awi.n_steps / 10:
                    if quantity < sum(self.in_scedule[: time + 1]) - sum(
                        self.out_schedule[: time + 1]
                    ):
                        if price > p:
                            return ResponseType.ACCEPT_OFFER
                ...
                if (
                    sum(self.in_scedule[:time]) - sum(self.out_schedule[:time])
                    > quantity
                ):
                    return ResponseType.ACCEPT_OFFER
            ...
            if (
                sum(self.in_scedule) + quantity
                > self.awi.n_steps * self.awi.n_lines * 0.6
            ):
                # print("too much")
                return ResponseType.REJECT_OFFER
            ...
            if self.awi.current_step < self.awi.n_steps * 0.3:
                need_buy = int(self.awi.n_lines / 3)

            elif self.awi.current_step != 0:
                need_buy = (
                    sum(self.sell_num[: self.awi.current_step]) / self.awi.current_step
                )
                # need_buy = sum(self.buy_num[self.awi.current_step - int(self.awi.n_step * 0.9]):self.awi.current_step])/(self.awi.current_step - int(self.awi.n_step * 0.9)

            for t in range(
                time, min(int(time + self.awi.n_steps / 5), self.awi.n_steps - 1)
            ):
                if sum(self.in_scedule[: t + 1]) - sum(self.out_schedule[: t + 1]) < 0:
                    if (
                        sum(self.in_scedule[: t + 1])
                        - sum(self.out_schedule[: t + 1])
                        + quantity
            ...
            if time < self.awi.n_steps * 2 / 3:
                ...
                if sum(self.out_schedule) - sum(self.in_scedule) + quantity < need_buy:
                    if price < p:
                        return ResponseType.ACCEPT_OFFER
            return ResponseType.REJECT_OFFER
