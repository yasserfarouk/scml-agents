...
class S7s(StdSyncAgent):
        ...
        for date,o_list in buy_date.items():
            if len(o_list) == 1:
                ...
                for i in range(1,len(o_list)):
                    respond_offer[o_list[i].negotiator_id] = SAOResponse(ResponseType.END_NEGOTIATION,None)
        ...
        if negotiator_id in self.awi.my_suppliers:
            ...
            if self.awi.is_last_level:
                if inventory[self.awi.current_step] < 0:
                    ...
                    p = int(state.step/20 * (self.ip - self.awi.trading_prices[self.awi.my_input_product]*0.9) + self.awi.trading_prices[self.awi.my_input_product]*0.9) * 1.2
    ...
    def responsses(self, negotiator_id: str, state: SAOState, inventory,fac_cap) -> ResponseType:
        ...
        # print("完成品の販売")
            ...
            for t in range(time, min(int(time + self.awi.n_steps/5),self.awi.n_steps)):
                if inventory[t] < 0:
