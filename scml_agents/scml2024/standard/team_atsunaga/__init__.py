from .S24s import *

MAIN_AGENT = S5s
__all__ = S24s.__all__

__author__ = ""
__team__ = ""
__email__ = ""
__version__ = "0.1.0"

# --- BEGIN generated metadata (scmlweb update_agents_repo.py) ---
ID = None
NAME = 'S5s'
CLASS_NAME = 'S5s'
VERSION = ''
TEAM = 'Team ATSUNAGA'
AUTHOR = 'Atsunaga Sadahiro'
MEMBERS = [{'name': 'Sadahiro Atsunaga', 'institution': 'Tokyo University of Agriculture and Technology', 'country': 'Japan'}, {'name': 'Atsunaga Sadahiro', 'institution': 'Tokyo University of Agriculture and Technology', 'country': 'Japan'}]
COUNTRY = 'Japan'
INSTITUTION = 'Tokyo University of Agriculture and Technology'
TAGS = []
USES_LLM = False
DESC = ''
METADATA = {
    'id': ID, 'name': NAME, 'class_name': CLASS_NAME,
    'version': VERSION, 'team': TEAM, 'author': AUTHOR,
    'members': MEMBERS, 'country': COUNTRY,
    'institution': INSTITUTION, 'tags': TAGS,
    'uses_llm': USES_LLM, 'description': DESC,
}
# --- END generated metadata ---
