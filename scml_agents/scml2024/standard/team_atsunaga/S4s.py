...
class S4s(StdAgent):
    def init(self):
        ...
        ]  # 日にちごとの工場の生産ラインのキャパシティ
        self.inventory = [
            [] for _ in range(self.awi.n_steps)
        ]  # 日にちごとの製品の未契約在庫
        self.all_inventory = [
        ...
        self.buy_day = self.awi.catalog_prices[
        ...
        self.ave_buy_price = self.awi.catalog_prices[
            self.awi.my_input_product
        ]  # 平均購入価格
        self.ave_sell_price = self.awi.catalog_prices[
    ...
    def before_step(self):
        ...
        self.storage_num += (
            self.awi.current_inventory_input + self.awi.current_inventory_output
        )
        ...
        if self.awi.is_first_level:
            self.raw_inventory[self.awi.current_step] += (
            ...
            if (
                ...
                self._update_sell_contract(
                    self.awi.current_inventory_output, self.awi.current_step
                )
                self.need_quantity = (
                    self.awi.current_exogenous_output_quantity
                    - self.awi.current_inventory_output
                ...
                self._update_sell_contract(
                    self.awi.current_exogenous_output_quantity, self.awi.current_step
                )
            pass
    ...
    def respond(self, negotiator_id: str, state: SAOState, source="") -> ResponseType:
        ...
        if negotiator_id in self.awi.my_consumers:
            ...
            if T == 0:
                ...
                if (
                    self.ave_buy_price + self.awi.profile.cost + self.ave_storage_cost
                    < 0.9 * P
                ):
    ...
    def propose(self, negotiator_id: str, state) -> Contract:
        ...
        if self.awi.current_step / self.awi.n_steps < 0.5:
            ...
            if negotiator_id in self.awi.my_consumers:
                ...
                if t == -1:
                    t = (
                        int(self.awi.n_steps - self.awi.current_step) / 2
    ...
    def on_negotiation_success(self, contract: Contract, mechanism: SAONMI) -> None:
        ...
        if contract.annotation["buyer"] == self.id:
            ...
            self.ave_buy_price = (
                self.ave_buy_price * (self.buy_num - quantity) + price * quantity
            ) / self.buy_num
        ...
        # 販売契約の場合は，出荷日の製品の在庫を更新する
            ...
            self.ave_sell_price = (
                self.ave_sell_price * (self.sell_num - quantity) + price * quantity
            ) / self.sell_num
        ...
        if buyer:
            ...
            if len(self.awi.my_consumers) == 1:
                offer = Contract(
                    {
                        "price": (
                            self.awi.current_exogenous_output_price - self.process_cost
                        )
                        * 0.95,
                        "quantity": self.need_quantity,
            ...
            return Contract(
                {
                    "price": (self.ave_sell_price - self.process_cost) * 0.95,
                    "quantity": int(self.awi.n_lines / 5),
            ...
            return Contract(
                {
                    "price": (self.ave_buy_price + self.process_cost) * 1.05,
                    "quantity": int(self.awi.n_lines / 2),
                    "time": min(self.awi.n_steps - 1, self.awi.current_step + 4),
                }
            )
