...
class S5s(StdAgent):
    ...
    def respond(self, negotiator_id: str, state: SAOState, source="") -> ResponseType:
        ...
        if negotiator_id in self.awi.my_consumers:
            ...
            if T > self.awi.n_steps / 3 * 2:
                if (
                    state.current_offer[UNIT_PRICE]
                    > self.awi.trading_prices[self.awi.my_input_product] - 5
                ):
                    return ResponseType.REJECT_OFFER
                else:
                    return ResponseType.ACCEPT_OFFER
    ...
    def propose(self, negotiator_id: str, state):
        ...
        if negotiator_id in self.awi.my_suppliers:
            ...
            # 販売時
            much_inve = sorted(
                self.inventory[self.awi.current_step :], key=len, reverse=True
            )
            indexed_lists = [
                (index, sublist)
                for index, sublist in enumerate(self.inventory[self.awi.current_step :])
            ]
