...
class RTAgent(SimpleAgent):
    ...
    def propose(
        self, negotiator_id: str, state
    ) -> "Outcome":  # 提案戦略 相手の提案履歴をもとに作成
        # print('提案')
                ...
                if average == 0:  # 相手の要求数が多すぎる 売りすぎない
                    ...
                    for i in range(-2, 0):
                        ...
                        if new_offer_q >= nmi.issues[UNIT_PRICE].min_value:
                            q_evaluation = math.exp(
                                -((new_offer_q - my_needs) ** 2) / 2 * RTAgent.DIST
                            ) / math.sqrt(2 * math.pi * (RTAgent.DIST**2))
                            if (
                                q_evaluation * nmi.issues[UNIT_PRICE].max_value
                                > average
                            ):  # 少なくてもいいから少しでも良くなるように売る
    ...
    def respond(self, negotiator_id, state):  # 受け入れ戦略
        ...
        if response == ResponseType.REJECT_OFFER:
            q_evaluation = (
                math.exp(-((offer[QUANTITY] - self.q_need) ** 2) / 2 * RTAgent.DIST)
                / math.sqrt(2 * math.pi * (RTAgent.DIST**2))
                if offer[QUANTITY] - self.q_need <= 1
                else 0
            )  # 多すぎるオファーは拒否
            if self._is_selling(nmi):  # 売るときは高いほうがいい
                ...
                if partner not in self.buyer_list:
                    ...
                    self.buyer_list[partner].add_evalation(
                        q_evaluation * offer[UNIT_PRICE]
                    )
                    return response
                else:
                    ...
                    if (
                        self.buyer_list[partner].get_average() > last_ave
                    ):  # 平均がよくなる→相手が譲歩した
                ...
                if partner not in self.seller_list:
                    ...
                    self.seller_list[partner].add_evalation(
                        -1 * (1 - q_evaluation) * offer[UNIT_PRICE]
                    )
                    return response
                else:
