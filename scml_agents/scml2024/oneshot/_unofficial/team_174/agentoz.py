...
class AgentOZ(OneShotAgent):
    ...
    def best_offer(self, negotiator_id, state):
        ...
        offer[QUANTITY] = max(
            min(my_needs, ami.issues[QUANTITY].max_value),
            ami.issues[QUANTITY].min_value,
        )
        offer[TIME] = self.awi.current_step

        if self._is_selling(ami):
            offer[UNIT_PRICE] = self._find_good_price_according_to_time_for_seller(
                ami, state, offer[QUANTITY]
            )
        else:
            offer[UNIT_PRICE] = self._find_good_price_according_to_time_for_buyer(
                ami, state, offer[QUANTITY]
            )
    ...
    def _find_good_price_according_to_time_for_seller(self, ami, state, offer_quantity):
        ...
        return max_price - (
            state.step * ((max_price - min_price) / self.max_negotiation_rounds)
        )
