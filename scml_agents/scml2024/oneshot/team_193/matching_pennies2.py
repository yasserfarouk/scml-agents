    ...
    **Please change the name of this class to match your agent name**
    ...
    def counter_all(self, offers, states):
        ...
        for needs, all_partners, issues in [
            ...
            total_offer_quant = sum(offers[p][QUANTITY] for p in list(partners))  # type: ignore
            ...
            self.expected_contract_quantity[self.negotiation_steps] /= (  # type: ignore
                self.expected_contract_step[self.negotiation_steps]
            )
            ...
            if (sum(offers[p][QUANTITY] for p in partner_ids) > (needs + 1)) or (  # type: ignore
                best_diff < needs
            ...
            response |= {
                ...
                k: SAOResponse(
                    ...
                    (
                        ...
                        self.best_price if best_diff > 1 else offers[k][UNIT_PRICE],  # type: ignore
                    ),  #  self.best_price, offers[k][UNIT_PRICE]
                )
                for k, q in dict_response.items()
                # k: SAOResponse(ResponseType.END_NEGOTIATION, None)
    ...
    def step(self):
        ...
        if self.verbose:
            print(
                f"Today I need {self.q} contracts, I managed to get {self.todays_contracts_num} "
                f"contracts with the price of {uprice} each. Today I accumulated a profit of "
                f"{self.ufun.from_contracts(self.contracts)}."  # type: ignore
            )
