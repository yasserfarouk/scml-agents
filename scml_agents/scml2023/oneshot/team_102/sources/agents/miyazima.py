...
class PPOTrainAgent(OneShotAgent):
    ...
    def init(self):
        ...
        self.lr_actor, self.lr_critic = 3e-6, 1e-5  # dafault:0.0003(3e-4),0.001(1e-3), 割と上手くいってた3e-5,1e-4
        ...
        self.ppo = PPO_Emb(self.state_dim, self.action_dim, self.emb_dim, self.lr_actor, self.lr_critic, self.lr_emb,
                           self.gamma, self.K_epochs, self.eps_clip, True,
        ...
        env_name = "OneShot-v3-" + (f"seller" if self.awi.my_suppliers[0] == "SELLER" else f"buyer")
        ...
        self.checkpoint_path = directory + "PPO_{}_{}_{}".format(env_name, random_seed, PPOTrainAgent.dt_now)
    ...
    def before_step(self):
        ...
        if self.awi.my_suppliers[0] == "SELLER":
            for negotiator_id in self.my_negotiators:
                self.obs[negotiator_id[2:-2]] = [self.awi.current_step / (self.awi.n_steps - 1), 0, 1,
                                                 0]  # 日,交渉ラウンド,前回の自分のオファーの単価,直前の相手のオファーの単価
            self.contracts['EXOGENOUS'] = {
                'unit_price': self.awi.current_exogenous_input_price / self.awi.current_exogenous_input_quantity if self.awi.current_exogenous_input_quantity != 0 else -1,
                'quantity': self.awi.current_exogenous_input_quantity}
        else:
            for negotiator_id in self.my_negotiators:
                self.obs[negotiator_id[2:-2]] = [self.awi.current_step / (self.awi.n_steps - 1), 0, 0, 1]
            self.contracts['EXOGENOUS'] = {
                'unit_price': self.awi.current_exogenous_output_price / self.awi.current_exogenous_output_quantity if self.awi.current_exogenous_output_quantity != 0 else -1,
        ...
        for negotiator_id in self.my_negotiators:
            ...
            if negotiator_id in self.ppo.episode_buffer.keys() and len(self.ppo.episode_buffer[negotiator_id]) > 0:
                self.embs[negotiator_id] = self.ppo.get_emb(self.ppo.episode_buffer[negotiator_id][-1])
    ...
    def propose(self, negotiator_id: str, state) -> "Outcome":
        ...
        if self.first_proposer[negotiator_id] == None:  # 自分が先手で提案する日の第一ラウンド
            ...
            self.action[negotiator_id] = self.ppo.select_action(self.obs[negotiator_id] + self.embs[negotiator_id],
                                                                negotiator_id)
        ...
        offer[UNIT_PRICE] = self.action[negotiator_id] * (
                    ami.issues[UNIT_PRICE].max_value - ami.issues[UNIT_PRICE].min_value) + ami.issues[
    ...
    def respond(self, negotiator_id, state, offer):
        ...
        if negotiator_id in self.opp_pre_offer.keys() and not self.ppo.buffer[negotiator_id].is_terminals[-1]:
            if offer[UNIT_PRICE] > self.opp_pre_offer[negotiator_id][UNIT_PRICE]:
                self.ppo.buffer[negotiator_id].rewards[-1] = self.reward_for_opp_concession if self._is_selling(
                    ami) else -self.reward_for_opp_concession
        ...
        # 状態を記録
        self.obs[negotiator_id][3] = (offer[UNIT_PRICE] - ami.issues[UNIT_PRICE].min_value) / (
                    ami.issues[UNIT_PRICE].max_value - ami.issues[UNIT_PRICE].min_value)  # 直前の相手のオファー価格
    ...
    def on_negotiation_success(self, contract, mechanism):
        ...
        # 報酬の算出
        if self._is_selling(ami):
            reward = ((contract.agreement['unit_price'] - ami.issues[UNIT_PRICE].min_value) / (
                        ami.issues[UNIT_PRICE].max_value - ami.issues[UNIT_PRICE].min_value)) ** 1.0
        else:
            reward = ((ami.issues[UNIT_PRICE].max_value - contract.agreement['unit_price']) / (
                        ami.issues[UNIT_PRICE].max_value - ami.issues[UNIT_PRICE].min_value)) ** 1.0
    ...
    def step(self):
        ...
        for negotiator_id in self.my_negotiators:
            ...
            if self.ppo_action_flag[negotiator_id]:
                self.opp_step_rewards[negotiator_id].append(self.ppo.buffer[negotiator_id].rewards[-1])

                episode = []
                for i, (state, action, is_terminal) in enumerate(zip(reversed(self.ppo.buffer[negotiator_id].states),
                                                                     reversed(self.ppo.buffer[negotiator_id].actions),
                                                                     reversed(self.ppo.buffer[
                                                                                  negotiator_id].is_terminals))):
        ...
        for negotiator_id in self.my_negotiators:
            if negotiator_id[2:-2] in self.contracts.keys():
                ...
                PPOTrainAgent.writer.add_scalar(f'agreement_price/{negotiator_id}', \
                                                (self.contracts[negotiator_id]['unit_price'] - ami.issues[
                                                    UNIT_PRICE].min_value) / (
                                                            ami.issues[UNIT_PRICE].max_value - ami.issues[
                                                        UNIT_PRICE].min_value), \
        ...
        if self.awi.current_step == self.awi.n_steps - 1:
            ...
            losses = {}
            if (
                    PPOTrainAgent._count + 1) % 10 == 0 and self.emb_dim != 0:  # and PPOTrainAgent._count<0.5*N_SIMULATIONS: #10シミュレーションに1回表現関数のUpdate
            ...
            if False not in [len(self.ppo.buffer[negotiator_id[2:-2]].rewards) > 1 for negotiator_id in
                             self.my_negotiators]:
            ...
            for i, negotiator_id in enumerate(self.my_negotiators):
                ...
                PPOTrainAgent.writer.add_scalar(f'simulation_reward/{negotiator_id}',
                                                sum(self.opp_step_rewards[negotiator_id]), PPOTrainAgent._count)
                PPOTrainAgent.writer.add_scalar(f'n_agreements/{negotiator_id}', self.n_agreements[negotiator_id],
            ...
            if (PPOTrainAgent._count + 1) % 500 == 0:
                ...
                for negotiator_id in self.my_negotiators:
                    ...
                    for j, (action, reward, is_terminal) in enumerate(
                            zip(reversed(self.ppo.buffer[negotiator_id].actions),
            ...
            if PPOTrainAgent._count + 1 in N_SIMULATIONS_SHORTS:
                if not os.path.exists(self.checkpoint_path + f'/{PPOTrainAgent._count + 1}'):
            ...
            if PPOTrainAgent._count + 1 == N_SIMULATIONS:
                ...
                for negotiator_id in self.my_negotiators:
                    ...
                    ax.set_ylim([-25.1, 50.1])
                    ax.plot(range(len(PPOTrainAgent.opp_simulation_total_rewards[negotiator_id])),
                            PPOTrainAgent.opp_simulation_total_rewards[negotiator_id], color='tab:blue', alpha=0.3,
                            lw=0.7)

                    smoothed_rewards = [PPOTrainAgent.opp_simulation_total_rewards[negotiator_id][0]]
                    smoothing_rate = 0.95
                    for total_reward in PPOTrainAgent.opp_simulation_total_rewards[negotiator_id][1:]:
                        smoothed_rewards.append(smoothed_rewards[-1] * 0.9 + total_reward * 0.1)
                    ax.plot(range(len(smoothed_rewards)), smoothed_rewards, color='tab:blue')

                    ax.tick_params(labelsize=20)
                    fig.tight_layout()
                    # fig.savefig(f"{''.join(self.my_negotiators)}.png")
                    fig.savefig(self.checkpoint_path + f"/totalreward-{negotiator_id}.png")
                plt.show()
                pass
...
class PPOTestAgent(OneShotAgent):
    ...
    def init(self):
        ...
        self.lr_emb = 1e-6
        self.ppo = PPO_Emb(self.state_dim, self.action_dim, self.emb_dim, self.lr_actor, self.lr_critic, self.lr_emb,
                           self.gamma, self.K_epochs, self.eps_clip, True, self.my_negotiators)
        self.ppo_action_flag = {}  # self.ppo.select_actionを呼び出したか否かk
        self.embs = {}
        self.all_embs = {}
    ...
    def before_step(self):
        ...
        if self.awi.my_suppliers[0] == "SELLER":
            ...
            for negotiator_id in self.my_negotiators:
                self.obs[negotiator_id] = [self.awi.current_step / (self.awi.n_steps - 1), 0, 1,
                                           0]  # 日,交渉ラウンド,前回の自分のオファーの単価,直前の相手のオファーの単価
            self.contracts['EXOGENOUS'] = {
                'unit_price': self.awi.current_exogenous_input_price / self.awi.current_exogenous_input_quantity if self.awi.current_exogenous_input_quantity != 0 else -1,
                'quantity': self.awi.current_exogenous_input_quantity}
            ...
            for negotiator_id in self.my_negotiators:
                self.obs[negotiator_id] = [self.awi.current_step / (self.awi.n_steps - 1), 0, 0, 1]
            self.contracts['EXOGENOUS'] = {
                'unit_price': self.awi.current_exogenous_output_price / self.awi.current_exogenous_output_quantity if self.awi.current_exogenous_output_quantity != 0 else -1,
                'quantity': self.awi.current_exogenous_output_quantity}
    ...
    def on_negotiation_success(self, contract, mechanism):
        ...
        if self._is_selling(ami):
            reward = ((contract.agreement['unit_price'] - ami.issues[UNIT_PRICE].min_value) / (
                        ami.issues[UNIT_PRICE].max_value - ami.issues[UNIT_PRICE].min_value)) ** 1
        else:
            reward = ((ami.issues[UNIT_PRICE].max_value - contract.agreement['unit_price']) / (
                        ami.issues[UNIT_PRICE].max_value - ami.issues[UNIT_PRICE].min_value)) ** 1
        if self.ppo_action_flag[negotiator_id]:
    ...
    def step(self):
        ...
        if self.awi.current_step == self.awi.n_steps - 1:
            ...
            my_negotiator_types = list(set([negotiator_id[2:-2] for negotiator_id in self.my_negotiators]))

            # 交渉相手ごと，相手の戦略(種類)ごとに合意率と合意効用を算出し，クラス変数AGREE_UTLSとAGREE_RATESに追加
            agent_agree_rate = {k: 1 - v.count(self.reward_for_neg_failure) / (len(v) - v.count(None)) for k, v in
                                self.agreement_utils.items()}
            agent_util_mean = {k: 0 if len([v_suc for v_suc in v if v_suc and v_suc >= 0]) == 0 else sum(
                [v_suc for v_suc in v if v_suc and v_suc >= 0]) / len([v_suc for v_suc in v if v_suc and v_suc >= 0])
            ...
            for negotiator_id in self.my_negotiators:
                type_agree_rate[negotiator_id[2:-2]].append(agent_agree_rate[negotiator_id])
                type_util_mean[negotiator_id[2:-2]].append(agent_util_mean[negotiator_id])
            type_agree_rate = {k: (0 if len(v) == 0 else sum(v) / len(v)) for k, v in type_agree_rate.items()}
            type_util_mean = {k: (0 if len(v) == 0 else sum(v) / len(v)) for k, v in type_util_mean.items()}
            ...
            if n_types >= 2:
                ...
                if '-'.join([negotiator_id[2:-2] for negotiator_id in self.my_negotiators]) in PPOTestAgent.IICR.keys():
                    PPOTestAgent.IICR['-'.join([negotiator_id[2:-2] for negotiator_id in self.my_negotiators])].append(
                        iicr)
                else:
                    PPOTestAgent.IICR['-'.join([negotiator_id[2:-2] for negotiator_id in self.my_negotiators])] = [iicr]
            ...
            fig, ax = plt.subplots()
            ax.set_xlabel(f'First Principal Component ({pca.explained_variance_ratio_[0]:.3f})', fontsize=20)
            ax.set_ylabel(f'Second Principal Component ({pca.explained_variance_ratio_[1]:.3f})', fontsize=20)
            ax.tick_params(labelsize=20)
            pos = 0
            """for i,negotiator_id in enumerate(self.my_negotiators):
            ...
            for i, agent_type in enumerate(my_negotiator_types):
                ax.scatter(feature[pos:pos + len(type_all_embs[agent_type]), 0],
                           feature[pos:pos + len(type_all_embs[agent_type]), 1], label=agent_type)
