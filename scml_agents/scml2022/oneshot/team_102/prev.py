    ...
    def _price_range(self, ami):
        ...
        if self._is_selling(ami):
            ...
            mn = min(
                ...
                worst_opp_acc_price_(ami, is_selling, self.success_contracts)
...
class AdaptiveSyncAgent(OneShotSyncAgent, AdaptiveAgent):
...
class LearningSyncAgent_(OneShotSyncAgent):
    ...
    def counter_all(self, offers, states):
        ...
        print_log(
            "my name",
            self.get_ami(n).negotiator_names[0]
            if self._is_selling(self.get_ami(n))
            else self.get_ami(n).negotiator_names[1],
        )
    ...
    def detect_max_min_utility(self, need, ami):
        ...
        return self.from_offers(
            [max_offer], [True] if self._is_selling(ami) else [False]
    ...
    def print_analyze(self):
        pass  # print("\n ~~~~  analyze  ~~~~")
        pass  # print("day", self.awi.current_step, " agent name:", self.short_type_name)
        # print("exogenous contract summary", self.awi.exogenous_contract_summary)
        pass  # print("exogenous input", self.awi.current_exogenous_input_quantity, self.awi.current_exogenous_input_price)
        pass  # print("exogenous output", self.awi.current_exogenous_output_quantity, self.awi.current_exogenous_output_price)
        ...
        pass  # print("current balance", self.awi.current_balance)
        ...
        pass  # print("ufun", self.ufun.max_utility, self.ufun.min_utility)
...
class LearningSyncAgent(OneShotSyncAgent, LearningAgent):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
...
class LearningSyncAgentT(LearningSyncAgent):
    def __init__(
    ...
    def on_negotiation_end(self, negotiator_id: str, state: MechanismState) -> None:
        ...
        if self.success_list:
            ...
            is_selling = [
                self._is_selling(self.get_ami(negotiator_id)) for _ in agreements
            ]
            if agreements:
    ...
    def counter_all(self, offers, states):
        ...
        print_log("my name", self.nego_info["my_name"])
